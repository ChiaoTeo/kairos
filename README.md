...
| 模式 | 数据/时钟 | 执行 | 适用场景 |
| --- | --- | --- | --- |
| `study` | 冻结数据 / 分析时钟 | 无 | 探索假设、构建和验证 Factor |
...
```text
study -> backtest -> historical-simulation -> shadow -> paper-kairos/testnet -> live
```
...
更完整的边界说明见 [系统架构](docs/architecture.md)，Study 数据使用见 [数据指南](docs/study_data_guide.md)，首次 Study 的逐步操作见 [新人教程](docs/tutorial_first_study.md)。

## 功能与命令参考

期权研究框架正按 `Market Data -> Pricing/Vol Engine -> Strategy -> Backtest -> Risk Analytics`
建设。目标架构、接口约束和分阶段验收标准见
[`docs/options_study_architecture.md`](docs/options_study_architecture.md)。
...
- [`examples/massive_study_diagnostics.ipynb`](examples/massive_study_diagnostics.ipynb)：MarketSnapshot、报价覆盖、标的价格、内部 IV/Greeks 和 feature。
...
```bash
./pyenv/bin/kairos study capture --host 127.0.0.1 --port 4001
./pyenv/bin/kairos study capture-series \
