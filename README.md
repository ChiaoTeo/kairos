...
所有命令均在仓库根目录执行，要求 Python 3.11 或更高版本。

```bash
python3 -m venv pyenv
./pyenv/bin/pip install -e '.[data,query,notebook]'
./pyenv/bin/python -m trading --help
./pyenv/bin/python -m trading tutorial sma
```
...
| `ProductType` / `ProductSpec` | 这是什么经济合约？ | 描述股票、现货、期货、永续、期权等产品属性；不保存 Venue symbol、tick 或账户状态。 |
...
| `ListingDefinition` | 该标的在某个 Venue 如何挂牌和交易？ | 保存 Venue symbol、external id、tick、lot、最小名义金额等。Adapter 下单必须通过 Catalog 查它，不能猜 symbol。 |
...
| 名称 | 定义 | 不应被当成什么 |
| --- | --- | --- |
| `DatasetProduct` / `DatasetKey` | 稳定的逻辑数据名，如 `market.ohlcv.crypto.binance.btc-usdt.1h` | 不是某次下载，也不保证内容永远不变 |
...
| `live-paper` | 实时数据 / 系统时钟 | 模拟执行 | 在真实行情下验证运行与监控，不产生真实成交 |
| `live` | 实时数据 / 系统时钟 | 真实执行 Adapter | 真实交易；必须经过对账、安全门禁和显式确认 |

`paper/testnet` 是外部环境或账户语义，`live-paper` 是系统运行模式，两者不要仅凭名称互换。Backtest 通过不代表 Live 安全；首次使用的推荐晋级顺序是：

```text
research -> backtest -> historical-simulation -> shadow -> live-paper/testnet -> live
```
...
| 目标 | 推荐入口 |
| --- | --- |
| 第一次了解系统 | `./pyenv/bin/python -m trading tutorial sma` |
| 查找和检查历史数据 | `trading data search`、`trading data describe` |
| 做确定性策略回测 | `trading run backtest --strategy ...` |
| 运行历史仿真 | `trading run simulate-sma ...` |
| 用 Capture 做零发单影子运行 | `trading run shadow-sma ...` |
| 检查/晋级策略版本 | `trading strategy status`、`trading strategy check-promotion`、`trading strategy promote` |
| 提交人工运维订单 | `trading order submit` |
| 验证内部期权定价/IV | `trading pricing option` |
| 校准波动率曲面 | `trading vol calibrate` |
| 做情景风险重估 | `trading risk scenario` |
| 同步产品与挂牌定义 | `trading catalog sync` |
| 接入外部账户前检查 | `trading account reconcile` |
| 外部 Paper/Testnet 验收 | `trading runtime l4-preflight` 后按 runbook 执行 soak |
| 从运行成交生成执行校准 | `trading runtime calibrate-execution` |
| 兼容旧人工订单入口 | `trading trade run`；新代码不要继续扩展它 |
...
./pyenv/bin/python -m trading --format json run backtest \
  --strategy sma-cross-v1@1.2.0 --fixture --fast 5 --slow 15
./pyenv/bin/python -m trading --format json run shadow-sma \
...
```bash
./pyenv/bin/python -m trading --format json strategy check-promotion sma-cross-v1 \
  --version 1.2.0 \
  --to RESEARCH_VALIDATED \
  --evidence data/studies/<study>/<version>/results.json

./pyenv/bin/python -m trading --format json strategy promote sma-cross-v1 \
  --version 1.2.0 \
  --to RESEARCH_VALIDATED \
  --evidence data/studies/<study>/<version>/results.json \
...
./pyenv/bin/python -m trading runtime l4-preflight \
  --venue binance --environment testnet \
...
```bash
./pyenv/bin/python -m trading order submit \
...
```bash
./pyenv/bin/python -m trading runtime calibrate-execution \
...
```bash
./pyenv/bin/python -m trading run backtest \
  --strategy sma-cross-v1@1.2.0 \
  --fixture --fast 5 --slow 15 \
