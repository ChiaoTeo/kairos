...
这个目录展示 Adapter 的语言边界，而不是复制 Python 领域模型。
...
```bash
./pyenv/bin/python examples/adapters/reference_adapter/verify_contract.py \
  --command './target/release/trader-binance-gateway --golden-vectors examples/adapters/reference_adapter/golden_vectors.json'
```
...
- Rust gateway 必须能被 Python reference 替换，上层 Strategy/Projection 不改代码。
