...
from kairos.market_data import CanonicalCaptureWriter, CapturedCanonicalEventSource
from kairos.contracts import BarPayload
...
from kairos.features import OptionFearCoolingFactorRuntime
from kairos.reference import ReferenceCatalog
from kairos.execution.policy import ExecutionMode,ExecutionPolicy
from kairos.execution.calibration import load_execution_calibration_release
...
from kairos.strategies.registry import PromotionEvidence
...
def create_study(args) -> dict[str, object]:
    ...
    return {"study_id": args.study_id, "version": args.version, "status": "sandbox", "workspace": str(path),
            **metadata, "next": f"./pyenv/bin/kairos --lake-root {args.lake_root} study freeze {args.study_id}"}
...
def _plan_governed_study(args, stop_event: Event) -> dict[str, object]:
    ...
    from kairos.connectors.binance.historical_archive import BinanceUsdmPerpetualHourlyArchiveProvider
    ...
    result = {
        ...
        **{key: value for key, value in plan.items() if key != "matrix"},
        "next": f"./pyenv/bin/kairos study start {args.study_id} --start {args.start} --end {args.end}",
    }
...
def _plan_us_equity_momentum_study(args) -> dict[str, object]:
    ...
    return {
        ...
        "next": (
            f"./pyenv/bin/kairos --lake-root {args.lake_root} study start {args.study_id} "
...
def start_sma_tutorial(args) -> dict[str, object]:
    ...
    if target.exists():
        ...
        created = True
    command = f"./pyenv/bin/kairos --lake-root {root} study data {args.study_id} --head 10"
    return {
...
def preview_study_data(args) -> dict[str, object]:
    ...
    return {
        ...
        "rows": selected,
        "next": f"./pyenv/bin/kairos --lake-root {args.lake_root} study profile {args.study_id}",
    }
...
def profile_study(args) -> dict[str, object]:
    ...
    return {
        ...
        **session.profile().as_dict(),
        "next": f"./pyenv/bin/kairos --lake-root {args.lake_root} study scaffold {args.study_id}",
    }
...
def register_btc_iron_condor_candidate(args)->dict[str,object]:
    ...
    source=Path(__file__).parent/"strategies"/"btc_iron_condor.py";implementation=StrategyImplementation(
        "kairos.strategies.btc_iron_condor:BtcIronCondorStrategy",sha256(source.read_bytes()).hexdigest())
