...
def _next_steps(payload: Mapping[str, object]) -> tuple[str, ...]:
    ...
    if not artifact:
        return ()
    steps = [f"kairos run inspect --artifact {artifact}", f"kairos run artifact-replay --artifact {artifact} --fixture"]
    if capture:
        steps.append(f"kairos run capture-replay --artifact {artifact} --capture {capture}")
    return tuple(steps)
