...
from .option_snapshot_analysis import OptionSnapshotAnalysis


def write_csv(result: OptionSnapshotAnalysis, path: Path) -> Path:
...
def summarize(result: OptionSnapshotAnalysis) -> str:
    return "\n".join(
        (
            f"Run: {result.run_id}",
