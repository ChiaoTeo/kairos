...
GOLDEN_SCENARIO_ID = "runtime-l2-spot-target-position-v1"
...
def run_runtime_golden(root: str | Path) -> RuntimeGoldenResult:
    ...
    work_root = output_root / ".runtime-golden-work"
    if work_root.exists():
    ...
    venue = SimulatedExecutionAccountAdapter(VenueId("simulated"), account, clock=clock)
    ...
    restarted_venue = SimulatedExecutionAccountAdapter(
    ...
    artifact = artifact_root / GOLDEN_SCENARIO_ID / "manifest.json"
    ...
    restarted.stop()
    return RuntimeGoldenResult(GOLDEN_SCENARIO_ID, audit_hash, artifact, payload)
...
def _strategy_decision(quote: Quote) -> TargetPositionIntent:
    ...
    return TargetPositionIntent(
        uuid5(NAMESPACE_URL, f"{GOLDEN_SCENARIO_ID}:{quote.instrument_id.value}"),
