...
from trading.adapters.base import OrderAck, RecoveredExecution, VenueOrderRecovery, VenueOrderStatus
...
from .runtime_golden import STARTED_AT, _catalog


FAILURE_MATRIX_ID = "runtime-l3-failure-matrix-v1"


def run_runtime_failure_matrix(root: str | Path) -> dict[str, object]:
    output_root = Path(root)
    work = output_root / ".runtime-failure-matrix-work"
    if work.exists():
        shutil.rmtree(work)
    ...
    artifact = output_root / "artifacts" / FAILURE_MATRIX_ID / "manifest.json"
...
def _request():
    ...
    account = AccountKey(InstitutionId("simulated"), "failure-matrix", AccountType.CRYPTO_SPOT)
    intent = TargetPositionIntent(
        UUID("00000000-0000-0000-0000-00000000f301"), "failure-matrix-strategy",
...
def _adapter(clock: FixedClock, *, balances=(), positions=()):
    order = _request()
    return SimulatedExecutionAccountAdapter(
        VenueId("simulated"), order.account, balances=balances, positions=positions, clock=clock,
    )


def _coordinator(path: Path, store: SQLiteRuntimeStore, adapter, clock: FixedClock, injector=None):
    order = _request()
    paths = RuntimePaths(path, path / "catalog.json", path, store.path, path / "artifacts")
    application = TradingApplication(
        ApplicationConfig(adapter.environment, paths), store,
        runtime_id=f"failure-matrix-{path.name}", accounts=(order.account,), clock=clock,
        recovery=RuntimeRecoveryService(store, _catalog(), AssetId("USDT"), {order.account: adapter}),
    )
    coordinator = TradingCoordinator(
        ExecutionRouter(_catalog(), (adapter,)),
        {order.account: ReconciliationService(store.load_ledger(), adapter, clock=clock)},
        KillSwitch((adapter,), clock, store), PersistentEventLog(path / "events.jsonl"),
...
def _acknowledged(store: SQLiteRuntimeStore):
    ...
    store.transition_order(order.client_order_id, DurableOrderStatus.ACKNOWLEDGED, STARTED_AT, ack=OrderAck(
        ...
        order.correlation_id, "failure-matrix-venue-order", STARTED_AT,
    ))
    return order
...
def _before_venue(path: Path) -> dict[str, object]:
    clock = FixedClock(STARTED_AT); store = SQLiteRuntimeStore(path / "runtime.sqlite3"); adapter = _adapter(clock)
    try:
        _coordinator(path, store, adapter, clock, OneShotRuntimeFaultInjector(
    ...
    safely_blocked = record is not None and record.status is DurableOrderStatus.SUBMITTING and not adapter.orders and not store.load_ledger().transactions
    resolution = store.resolve_unresolved_order(
        ...
        actor="failure-matrix", reason="checkpoint proves transport was not called",
...
def _after_venue_before_ack(path: Path) -> dict[str, object]:
    ...
    recovery = VenueOrderRecoveryService(
        store, {_request().account: adapter},
        DurableExecutionIngestionService(LedgerService(store.load_ledger(), _catalog()), store),
    ).recover(STARTED_AT + timedelta(seconds=1))
    record = store.order(_request().client_order_id)
    passed = len(adapter.orders) == 1 and not recovery.unresolved and record is not None and record.status is DurableOrderStatus.ACKNOWLEDGED
...
def _partial_fill_restart(path: Path) -> dict[str, object]:
    ...
    DurableExecutionIngestionService(LedgerService(Ledger(), _catalog()), store).ingest(
        fill, external_key="matrix:partial:2", client_order_id=order.client_order_id, fully_filled=False,
    )
...
class _BackfillRecoveryAdapter:
...
def _websocket_disconnect_backfill(path: Path) -> dict[str, object]:
    ...
    adapter = _BackfillRecoveryAdapter(VenueOrderRecovery(
        ...
        executions=(RecoveredExecution(
            "matrix:disconnect-backfill:5", fill, True,
            "matrix:rest-backfill", "5",
        ),),
    ))
    report = VenueOrderRecoveryService(
        store, {order.account: adapter},
        DurableExecutionIngestionService(LedgerService(store.load_ledger(), _catalog()), store),
    ...
    passed = (
        ...
        and len(store.load_ledger().transactions) == 1 and store.cursor("matrix:rest-backfill") == "5"
    )
...
def _duplicate_transport(path: Path) -> dict[str, object]:
    ...
    first = ingestion.ingest(fill, external_key="matrix:transport:3", client_order_id=order.client_order_id, fully_filled=True)
    second = ingestion.ingest(fill, external_key="matrix:transport:3", client_order_id=order.client_order_id, fully_filled=True)
...
def _ledger_transaction_interrupt(path: Path) -> dict[str, object]:
    ...
    try:
        DurableExecutionIngestionService(LedgerService(Ledger(), _catalog()), store).ingest(
            fill, external_key="matrix:atomic:4", client_order_id=order.client_order_id,
            fully_filled=True, cursor_name="matrix:fills", cursor_value="4",
        )
    except InjectedRuntimeFailure:
        failed = True
    restarted = SQLiteRuntimeStore(path / "runtime.sqlite3"); record = restarted.order(order.client_order_id)
    rolled_back = failed and record is not None and record.status is DurableOrderStatus.ACKNOWLEDGED and not restarted.load_ledger().transactions and restarted.cursor("matrix:fills") is None
    retried = DurableExecutionIngestionService(LedgerService(Ledger(), _catalog()), restarted).ingest(
        fill, external_key="matrix:atomic:4", client_order_id=order.client_order_id, fully_filled=True,
    )
...
def _kill_switch_restart(path: Path) -> dict[str, object]:
    clock = FixedClock(STARTED_AT); store = SQLiteRuntimeStore(path / "runtime.sqlite3"); adapter = _adapter(clock)
    KillSwitch((adapter,), clock, store).trigger((), "matrix drill")
    switch = KillSwitch((adapter,), clock, SQLiteRuntimeStore(path / "runtime.sqlite3"))
    blocked = False
    try:
        paths = RuntimePaths(path, path / "catalog.json", path, store.path, path / "artifacts")
        application = TradingApplication(
            ApplicationConfig(adapter.environment, paths), store,
            runtime_id="failure-matrix-kill-switch", accounts=(_request().account,), clock=clock,
            recovery=RuntimeRecoveryService(store, _catalog(), AssetId("USDT"), {_request().account: adapter}),
        )
        coordinator = TradingCoordinator(
            ExecutionRouter(_catalog(), (adapter,)), {_request().account: ReconciliationService(Ledger(), adapter, clock=clock)},
        ...
        blocked = True
    return _case("kill_switch_restart", switch.triggered and blocked and not adapter.orders, "persistent reduce-only state blocks risk expansion")
...
def _reconciliation_mismatch(path: Path) -> dict[str, object]:
    ...
    adapter = _adapter(clock, balances=((AssetId("USDT"), Decimal("1")),)); blocked = False
    app = TradingApplication(
        ApplicationConfig(adapter.environment, paths), store, runtime_id="matrix-mismatch",
