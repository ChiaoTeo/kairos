...
def merge_datasets(existing: HistoricalDataset, chunk: HistoricalDataset) -> HistoricalDataset:
    ...
    for item in chunk.slices:
        ...
        if previous is not None and previous != item:
            raise ValueError(f"conflicting market slice at {item.timestamp} sequence={item.sequence}")
        slices[key] = item
