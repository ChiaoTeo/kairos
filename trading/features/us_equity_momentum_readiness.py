...
from trading.data.models import DatasetStatus, QualityLevel
...
class UsEquityMomentumReadiness:
    """Local acceptance report for the US equity momentum data package."""
        ...
        for logical_key in self._release_check_products(catalog):
            try:
                ...
                checks.append(UsEquityReadinessCheck(
                    ...
                    "Build features with trader features build --feature-set us-equity-momentum-v1",
                ))
                continue
            checks.append(UsEquityReadinessCheck(
            ...
            checks.append(UsEquityReadinessCheck(
                ...
                f"Run trader data validate --release {release.release_id}",
            ))
            directory = self.root / release.relative_path
            required_metadata = (
                ("manifest", "quality", "events")
                if logical_key == "reference.corporate_actions.equity.us.massive"
                else ("manifest", "lineage", "coverage", "quality", "schema")
            )
            checks.append(UsEquityReadinessCheck(
    ...
    def _release_set(self, catalog: DataCatalog) -> UsEquityReadinessCheck:
        ...
        return UsEquityReadinessCheck(
            ...
            "Run trader features build --feature-set us-equity-momentum-v1 after preparing OHLCV input",
        )
    ...
    def _identity_mapping(self) -> UsEquityReadinessCheck:
        ...
        if not manifests:
            return UsEquityReadinessCheck(
                ...
                "Run trader data build-massive-equity-identity with active/inactive reference rows and ticker events",
            )
    ...
    def _study_snapshot(self, study_id: str, version: str) -> UsEquityReadinessCheck:
        ...
        if not path.exists():
            return UsEquityReadinessCheck(
                ...
                "Run trader study start us-equity-momentum --dataset features.momentum.equity.us.1d",
            )
        ...
        return UsEquityReadinessCheck(
            ...
            "error",
            values,
            "Study workspace pins returns, universe, liquidity, momentum and available corporate action release hashes",
    ...
    def _corporate_action_evidence(self, catalog: DataCatalog) -> UsEquityReadinessCheck:
        try:
            ...
            return UsEquityReadinessCheck(
                ...
                "Build returns with trader data prepare-us-equity-momentum",
            )
    ...
    def _universe_missing_status(self, catalog: DataCatalog) -> UsEquityReadinessCheck:
        ...
        try:
            ...
            return UsEquityReadinessCheck(
                ...
                "Build market.universe.equity.us.1d with trader features build --feature-set us-equity-momentum-v1",
            )
        try:
    ...
    def _release_check_products(self, catalog: DataCatalog) -> tuple[str, ...]:
        try:
            ...
            return self.required_products
        return (*self.required_products, "reference.corporate_actions.equity.us.massive")
