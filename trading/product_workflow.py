...
from trading.market_data import CanonicalCaptureWriter, CapturedCanonicalEventSource
from trading.contracts import BarPayload
...
from trading.features import OptionFearCoolingFactorRuntime
from trading.reference import ReferenceCatalog
from trading.execution.policy import ExecutionMode,ExecutionPolicy
from trading.execution.calibration import load_execution_calibration_release
...
from trading.strategies.registry import PromotionEvidence
...
def create_study(args) -> dict[str, object]:
    ...
    return {"study_id": args.study_id, "version": args.version, "status": "sandbox", "workspace": str(path),
            **metadata, "next": f"./pyenv/bin/python -m trading --lake-root {args.lake_root} study freeze {args.study_id}"}
...
def _plan_governed_study(args, stop_event: Event) -> dict[str, object]:
    ...
    result = {
        ...
        **{key: value for key, value in plan.items() if key != "matrix"},
        "next": f"./pyenv/bin/python -m trading study start {args.study_id} --start {args.start} --end {args.end}",
    }
...
def _start_governed_study(args, stop_event: Event) -> dict[str, object]:
    ...
    from trading.data.acquisition import AcquisitionLimits
...
def _plan_us_equity_momentum_study(args) -> dict[str, object]:
    ...
    return {
        ...
        "next": (
            f"./pyenv/bin/python -m trading --lake-root {args.lake_root} study start {args.study_id} "
...
def start_sma_tutorial(args) -> dict[str, object]:
    ...
    if target.exists():
        ...
        created = True
    command = f"./pyenv/bin/python -m trading --lake-root {root} study data {args.study_id} --head 10"
    return {
...
def preview_study_data(args) -> dict[str, object]:
    ...
    return {
        ...
        "rows": selected,
        "next": f"./pyenv/bin/python -m trading --lake-root {args.lake_root} study profile {args.study_id}",
    }
...
def profile_study(args) -> dict[str, object]:
    ...
    return {
        ...
        **session.profile().as_dict(),
        "next": f"./pyenv/bin/python -m trading --lake-root {args.lake_root} study scaffold {args.study_id}",
    }
...
def register_sma_strategy(args) -> dict[str, object]:
    ...
    source = Path(__file__).parent/"strategies"/"sma_strategy.py"
    implementation = StrategyImplementation(
        "trading.strategies.sma_strategy:SmaCrossStrategy", sha256(source.read_bytes()).hexdigest(),
    )
...
def register_btc_iron_condor_candidate(args)->dict[str,object]:
    ...
    source=Path(__file__).parent/"strategies"/"btc_iron_condor.py";implementation=StrategyImplementation(
        "trading.strategies.btc_iron_condor:BtcIronCondorStrategy",sha256(source.read_bytes()).hexdigest())
...
def run_sma_paper_workflow(args) -> dict[str, object]:
    ...
    result=asyncio.run(run_sma_historical_simulation(root=args.run_root,events=events,catalog=catalog,
        ...
        mode="live-paper",environment=Environment.PAPER))
    artifact=RunArtifactRepository(_artifact_root(args)).write(mode="live-paper",input_identity=identity,
        strategy_id="sma-cross-v1",strategy_version="1.2.0",config=_run_config(args),result=result.strategy_run,
        execution={"driver":"simulated-live-paper","capture":str(capture),"orders":result.orders,
        "fills":result.fills,"runtime_database":str(result.runtime_database),"restart_ready":result.restart_ready},
        attribution=build_run_attribution(result.strategy_run,starting_equity=args.initial_cash,
        ending_equity=result.final_cash+result.final_position*bars[-1].close,
        orders=result.orders,fills=result.fills,fees=result.fees))
    return {"mode":"live-paper","capture":str(capture),"input_identity":identity,"bars":len(bars),
...
def replay_sma_capture(args) -> dict[str, object]:
    ...
    replay=asyncio.run(_governed_run(identity,bars,int(config["fast"]),int(config["slow"]),
...
async def _governed_run(identity, bars, fast, slow, capital, fee_bps=Decimal("10")):
    ...
    return await GovernedStrategyRunLoop(
        IterableEventSource(tuple(canonical_bar_events(bars))),
