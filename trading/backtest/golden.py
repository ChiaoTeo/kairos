...
from trading.data import DatasetStatus, QualityLevel, ResearchDataClient, RunMode
from trading.risk.limits import RiskLimits
from trading.strategies.bull_put_spread import BullPutSpreadConfig


def build_spxw_golden_pipeline(
    lake_root: str | Path,
    ...
    *,
    ...
    if curated_release.quality_level not in {QualityLevel.BACKTEST, QualityLevel.PRODUCTION}:
        raise ValueError("SPXW Golden Pipeline requires a Q3/Q4 MarketSlice Release")
    if curated_release.status not in {DatasetStatus.APPROVED_FOR_BACKTEST, DatasetStatus.APPROVED_FOR_PRODUCTION}:
        raise ValueError("SPXW Golden Pipeline requires a backtest-approved MarketSlice Release")
    ...
    if {event_release.release_id, source_release.release_id} - input_ids:
        raise ValueError("curated SPXW Release lineage does not freeze Event and source MarketSlice inputs")
    feed = client.replay_slices(curated_release.release_id)
    dataset = feed.dataset
    repository = BacktestRepository(backtest_root)
    results = BacktestService(repository).run_suite(
        feed,
    ...
    payload = {
        ...
        "pipeline": "massive-spxw-event-market-slice-bull-put-v1",
        "inputs": {
    ...
    directory = Path(backtest_root) / "golden" / payload["pipeline"]
