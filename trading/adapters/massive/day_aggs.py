...
from .source import MassiveFlatFileBatchDownloader, MassiveFlatFileClient, request_fingerprint
...
class OptionDayAggPipeline:
    """Governed OPRA day-aggregate inventory and one option-root Parquet dataset."""
    ...
    def prepare(self, dataset_id: str, start: date, end: date) -> dict[str, object]:
        ...
        for entry in inventory:
            with gzip.open(entry.path, "rt", encoding="utf-8", newline="") as handle:
                ...
                if not _REQUIRED_COLUMNS <= set(reader.fieldnames or ()):
                    raise ValueError(f"OPRA day aggregates schema mismatch: {entry.path}")
                for source_row, row in enumerate(reader, start=2):
                    ...
                    try:
                        ...
                        if len(invalid) >= 100:
                            raise ValueError(f"{self.option_root} day aggregates exceeded 100 invalid rows")
                        continue
        ...
        if invalid:
            ...
            write_json(quarantine / "invalid-day-aggs.json", {"rows": invalid})
            raise ValueError(f"{self.option_root} day aggregates contain {len(invalid)} invalid rows")
        if not option_rows:
            raise ValueError(f"OPRA inventory contains no {self.option_root} day aggregates")
        ...
        if not quality["publishable"]:
            raise ValueError(f"{self.option_root} day aggregate dataset failed publish gate")
        ...
        write_json(target / "lineage.json", {
            ...
            "visibility": {"event_clock": "window_start", "available_time": "next natural day 11:00 America/New_York per Massive Day Aggregates plan recency"},
        })
    ...
    def build_inventory(self, start: date, end: date) -> tuple[tuple[OpraInventoryEntry, ...], Path, str]:
        ...
        if missing:
            raise FileNotFoundError(f"OPRA day aggregate inventory is missing {len(missing)} trading days; first={missing[0]}")
        primitive = [{
        ...
        path = self.root / "reference" / "provider=massive" / "opra_day_aggs" / f"year={start.year}" / f"inventory-{inventory_hash}.json"
        if not path.exists():
...
class SpxwDayAggPipeline(OptionDayAggPipeline):
...
def _pyarrow():
    try:
        ...
        raise RuntimeError("OPRA day aggregates require the 'data' optional dependency") from error
    return pa, pq
