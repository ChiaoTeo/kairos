...
from trading.adapters.binance.adapter import BinanceSigner, BinanceTransport, RateLimiter
...
from trading.treasury.models import (
...
class BinanceTransferAdapter:
    """Dedicated Binance asset-movement adapter.

    Withdrawal capability is disabled by default and intentionally separate from
    trading and user-stream adapters.
    """
    ...
    def submit(self, instruction: TransferInstruction) -> TransferSubmission:
        ...
        raise TypeError(f"Binance transfer adapter does not support {type(instruction).__name__}")
