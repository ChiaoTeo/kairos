...
from trading.treasury.adapter import TransferSubmission
from trading.treasury.models import BankTransferInstruction, TransferInstruction, TransferStatus


class BankTransferGateway(Protocol):
...
class BankTransferAdapter:
    ...
    def __init__(self, gateway: BankTransferGateway, *, enable_transfers: bool = False) -> None:
    ...
    def submit(self, instruction: TransferInstruction) -> TransferSubmission:
        if not isinstance(instruction, BankTransferInstruction):
            raise TypeError(f"bank adapter does not support {type(instruction).__name__}")
        if not self.enable_transfers:
            raise PermissionError("bank transfers are disabled")
        row = self.gateway.create_transfer({
    ...
    def status(self, provider_reference: str) -> TransferSubmission:
        return _submission(self.gateway.get_transfer(provider_reference))
