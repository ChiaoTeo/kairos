...
from .models import (
    DataView, DatasetKey, DatasetLayer, DatasetProduct, DatasetProductSpec, DatasetRelease, DatasetStatus,
...
class DataCatalog:
    ...
    def __init__(self, root: str | Path = "data", registry_path: str | Path | None = None) -> None:
        ...
        self._products: dict[str, DatasetProduct] = {}
        self._specs: dict[str, DatasetProductSpec] = {}
    ...
    def register_product(self, product: DatasetProduct, *, enrich: bool = False) -> None:
    ...
    def register_product_spec(self, spec: DatasetProductSpec, *, enrich: bool = False) -> None:
        key = str(spec.key)
        ...
        if previous is not None and previous != spec:
            ...
            if not enrich or not contract_unchanged:
                raise ValueError(f"conflicting dataset product spec: {spec.key}")
        self.register_product(spec.product, enrich=enrich)
        self._specs[key] = replace(spec, product=self._products[key])

    def product_spec(self, key: DatasetProduct | DatasetKey | str) -> DatasetProductSpec:
        product = self.product(key)
        try:
            return self._specs[str(product.key)]
        except KeyError as error:
            raise KeyError(f"dataset product has no complete ProductSpec: {product.key}") from error

    def product_specs(self) -> tuple[DatasetProductSpec, ...]:
        return tuple(sorted(self._specs.values(), key=lambda item: str(item.key)))

    def update_product_spec(self, spec: DatasetProductSpec, *, actor: str, reason: str) -> None:
        if not actor.strip() or not reason.strip():
            raise ValueError("ProductSpec update requires actor and reason")
        key = str(spec.key)
        previous = self._specs.get(key)
        if previous is None:
    ...
    def product(self, key: DatasetProduct | DatasetKey | str) -> DatasetProduct:
        value = str(key.key) if isinstance(key, DatasetProduct) else str(key)
        direct = self._products.get(value)
        if direct is not None:
            return direct
        return self._products[str(self.release(value).product_key)]

    def release(self, value: DatasetProduct | DatasetKey | str, *, version: str | None = None,
                provider: str | None = None, venue: str | None = None) -> DatasetRelease:
        name = str(value.key) if isinstance(value, DatasetProduct) else str(value)
        direct = None if isinstance(value, (DatasetProduct, DatasetKey)) else self._releases.get(self._aliases.get(name, name))
    ...
    def products(self) -> tuple[DatasetProduct, ...]:
        return tuple(sorted(self._products.values(), key=lambda item: str(item.key)))

    def releases(self, product: DatasetProduct | DatasetKey | str | None = None) -> tuple[DatasetRelease, ...]:
        values = self._releases.values()
        if product is not None:
            key = str(product.key) if isinstance(product, DatasetProduct) else str(product)
    ...
    def search(self, **dimensions: str) -> tuple[DatasetProduct, ...]:
    ...
    def discover(self) -> tuple[DatasetRelease, ...]:
        ...
        for pointer in sorted(pointers):
            ...
            if dataset_id in self._releases:
                continue
            market_type = str(manifest.get("market_data_type") or "market_slices")
            underlying = dataset_id.lower().split(".")[0]
            logical = f"curated.market_slices.options.us.{underlying}" if "massive" in dataset_id.lower() else f"curated.{market_type}"
            ...
            release = DatasetRelease(
                ...
                "historical_dataset.v2", "2", "historical_dataset", "2",
                ...
                DatasetStorageKind.MARKET_SLICES, "1",
            )
            self.register_release(release)
            discovered.append(release)
...
def _product_primitive(item: DatasetProduct) -> dict[str, object]:
    return {
...
def _spec_primitive(item: DatasetProductSpec) -> dict[str, object]:
    return {
...
def _product_from_primitive(raw: dict[str, object]) -> DatasetProduct:
...
def _spec_from_primitive(raw: dict[str, object]) -> DatasetProductSpec:
    product_raw = raw.get("product")
    if not isinstance(product_raw, dict):
        raise ValueError("dataset product spec requires a product document")
...
def _storage_kind(relative_path: str, schema_id: str) -> DatasetStorageKind:
    ...
    if "historical_dataset" in schema or normalized.endswith("/dataset.json"):
        return DatasetStorageKind.MARKET_SLICES
...
def _discovered_product(logical: str, layer: DatasetLayer, provider: str) -> DatasetProduct:
    parts = logical.split(".")
    dimensions = {}
    ...
    return DatasetProduct(DatasetKey(logical), logical, layer, dimensions=dimensions,
