...
from trading.backtest.feed import (
    ContractMetadata, DatasetManifest, HistoricalDataset, MarketSlice, build_manifest,
)
from trading.reference.repository import instrument_from_primitive, instrument_to_primitive
from trading.reference.models import EconomicProduct, InstrumentReference, SettlementTermsDefinition
from trading.storage.codec import from_primitive, to_primitive


class MarketSliceStorageDriver:
    """Internal physical driver for governed MarketSlice Releases."""

    def __init__(self, root: str | Path) -> None:
        self.root = Path(root)

    def save(self, dataset: HistoricalDataset, *, directory_name: str | None = None) -> Path:
    ...
    def load(self, release_directory: str | Path) -> HistoricalDataset:
        ...
        if value.get("format") == "parquet":
            try:
                ...
                raise RuntimeError("loading Parquet MarketSlice Release requires the data optional dependency") from error
            table = pq.read_table(directory / "slices.parquet", columns=["slice_json"])
            slices = tuple(from_primitive(json.loads(item), MarketSlice) for item in table.column("slice_json").to_pylist())
        else:
            slices = tuple(from_primitive(item, MarketSlice) for item in value["slices"])
        dataset = HistoricalDataset(manifest, slices, contracts, definitions, products, references, settlements)
        ...
        if rebuilt.content_hash != manifest.content_hash:
            raise ValueError("market slice release content hash mismatch")
        return dataset
