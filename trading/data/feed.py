...
from trading.contracts import CanonicalEventEnvelope, canonicalize_market_event
from trading.domain.identity import InstrumentId
from trading.market_data import MarketEventEnvelope, MarketEventType, ParquetMarketEventRepository
from trading.backtest.feed import HistoricalDataset, HistoricalFeed, MarketSlice

from .models import DataView, DatasetRelease
...
class ReplaySliceFeed:
    """Deterministic MarketSlice feed tied to a Catalog release and dataset hash."""

    def __init__(self, release: DatasetRelease, dataset: HistoricalDataset) -> None:
        if release.content_hash is None:
            raise ValueError("backtest replay requires a release with a frozen content hash")
        if dataset.manifest.dataset_id != release.release_id:
            raise ValueError("HistoricalDataset identity does not match the frozen Catalog release")
        if dataset.manifest.content_hash != release.content_hash:
            raise ValueError("HistoricalDataset content hash does not match the frozen Catalog release")
        self.release, self.dataset = release, dataset
        self._feed = HistoricalFeed(dataset)
    ...
    def between(self, start: datetime, end: datetime) -> Iterator[MarketSlice]:
