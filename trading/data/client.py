...
from .feed import ReplayEventFeed, ReplaySliceFeed, replay_spec
from .models import (
    AcquirePolicy, DataView, DatasetLike, DatasetProduct, DatasetStatus, DatasetStorageKind, FieldLike, OutputFormat,
...
class DataUnavailableError(RuntimeError):
    def __init__(self, plan: AcquisitionPlan) -> None:
        ...
        if plan.selected is not None:
            ...
            command = (
                f"; next: trader data acquire --dataset {plan.logical_key} "
...
class ResearchDataClient:
    ...
    HistoricalDataset.  Arrow is the native return type; pandas/polars are adapters.
    """
    ...
    def resolve(self, dataset: DatasetLike, *, version: str | None = None):
        name = str(dataset.key) if isinstance(dataset, DatasetProduct) else str(dataset)
    ...
    def replay_slices(self, dataset: DatasetLike, *, version: str | None = None,
                      provider: str | None = None, venue: str | None = None) -> ReplaySliceFeed:
        release = self.catalog.release(dataset, version=version, provider=provider, venue=venue)
        self._require_release_for_mode(release)
        release = self.resolve(release.release_id)
        directory = self.root / release.relative_path
        if release.storage_kind is not DatasetStorageKind.MARKET_SLICES:
            raise ValueError(f"release {release.release_id!r} is not a HistoricalDataset")
        historical = MarketSliceStorageDriver(directory.parent).load(directory)
        return ReplaySliceFeed(release, historical)
    ...
    def _require_release_for_mode(self, release) -> None:
        ...
        if self.run_mode in {RunMode.LIVE_PAPER, RunMode.LIVE}:
