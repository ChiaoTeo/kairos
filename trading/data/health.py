...
class DataHealthService:
    ...
    def audit(self) -> dict[str, object]:
        ...
        for product in products:
            ...
            if not product.description.strip():
                issues.append(DataHealthIssue("missing_product_description", "error", "product description is required", key))
            if not product.owner:
                issues.append(DataHealthIssue("missing_product_owner", "error", "product owner is required", key))
            if not product.dimensions:
                issues.append(DataHealthIssue("missing_product_dimensions", "warning", "product dimensions are empty", key))
            if not self.catalog.releases(product):
                issues.append(DataHealthIssue("product_without_release", "warning", "product has no published release", key))
            try:
    ...
    def _release_issues(self, release: DatasetRelease) -> list[DataHealthIssue]:
        key = str(release.product_key)
        result: list[DataHealthIssue] = []
        try:
            spec = self.catalog.product_spec(release.product_key)
        except KeyError:
            spec = None
        if spec is not None and release.storage_kind is not spec.storage_kind:
            result.append(DataHealthIssue(
                "release_storage_kind_mismatch", "error",
                f"release storage {release.storage_kind.value} differs from ProductSpec {spec.storage_kind.value}",
                key, release.release_id,
            ))
        if spec is not None and release.layout_version != spec.layout_version:
            result.append(DataHealthIssue(
                "release_layout_version_mismatch", "error",
                f"release layout {release.layout_version} differs from ProductSpec {spec.layout_version}",
                key, release.release_id,
            ))
        ...
        if not release.content_hash:
            result.append(DataHealthIssue("missing_content_hash", "error", "release content hash is required", key, release.release_id))
    ...
    def _next_action(*, product: str, releases: tuple[DatasetRelease, ...],
                     issues: list[DataHealthIssue]) -> str:
        if not releases:
            return f"trader data plan --dataset {product} --start <UTC> --end <UTC>"
        ...
        return f"trader data describe --dataset {product}"
