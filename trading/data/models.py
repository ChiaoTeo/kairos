...
class DatasetProductSpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not path.strip() or path.startswith("/") or ".." in path.split("/"):
            raise ValueError("dataset product spec requires a safe lake-relative path")
        if not self.schema_id.strip() or not self.layout_version.strip() or not self.quality_profile.strip():
            raise ValueError("dataset product spec schema, layout, and quality profile cannot be empty")
        if self.product.primary_time.strip() == "":
            raise ValueError("dataset product spec requires a primary time field")
...
DatasetLike = DatasetProduct | DatasetKey | str
