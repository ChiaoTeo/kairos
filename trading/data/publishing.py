...
from .models import DatasetProduct, DatasetRelease, DatasetStatus, DatasetStorageKind, QualityLevel
...
def publish_release(root: str | Path, product: ManagedDataset, release_id: str, manifest: dict[str, object], *,
    ...
    if levels.index(quality_level) < levels.index(product.minimum_publication_level):
        raise ValueError(
            f"release quality {quality_level.value} is below ProductSpec minimum "
...
def register_historical_dataset(root: str | Path, dataset, directory: str | Path, product: DatasetProduct, *,
    ...
    try:
        ...
        raise ValueError("HistoricalDataset storage must be inside the governed lake root") from error
    status = DatasetStatus.APPROVED_FOR_BACKTEST
    release = DatasetRelease(
        dataset.manifest.dataset_id, product.key, dataset.manifest.end.isoformat(), "historical_dataset.v2", "2",
        "synthetic_fixture" if synthetic else "historical_dataset", "2", relative_path, "parquet",
