...
from .market_slice_storage import MarketSliceStorageDriver
from .models import DatasetProductSpec, DatasetStorageKind, QualityLevel
from .publishing import register_historical_dataset
from .quality import DatasetQualityService


def curate_complete_market_slices(
    root: str | Path,
    ...
    *,
    ...
    source = ResearchDataClient(lake).replay_slices(source_release.release_id).dataset
    slices = tuple(
    ...
    if len(slices) < 2:
        raise ValueError("curated MarketSlice Release requires at least two complete slices")
    ...
    base = Path("curated") / "market_slices" / f"product={product.key}"
    try:
        spec = catalog.product_spec(product)
    except KeyError:
        spec = DatasetProductSpec(
            product, base.as_posix(), "historical_dataset.v2", {}, DatasetStorageKind.MARKET_SLICES,
            "1", "market_slice", QualityLevel.BACKTEST,
        )
        catalog.register_product_spec(spec, enrich=True)
    else:
        if spec.quality_profile != "market_slice" or spec.relative_path != base.as_posix():
            catalog.update_product_spec(
                replace(spec, quality_profile="market_slice", relative_path=base.as_posix()),
                actor="market-slice-curation",
                reason="publish complete point-in-time MarketSlice Release",
            )
    catalog.save()
    directory = MarketSliceStorageDriver(lake / base).save(dataset, directory_name=f"release={release_id}")
    write_json(directory / "lineage.json", {
        "lineage_version": 2,
        "producer": {"name": "curate_complete_market_slices", "version": 1},
        "inputs": [
    ...
    release = register_historical_dataset(
        lake,
        dataset,
        directory,
        product,
    ...
    if not assessment.passed or assessment.level is not QualityLevel.BACKTEST:
        raise RuntimeError(f"curated MarketSlice Release failed Q3 quality: {release.release_id}")
