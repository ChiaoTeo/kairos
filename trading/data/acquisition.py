...
from .models import DatasetLike, DatasetProductSpec, DatasetRelease, SourceBinding
...
class ProviderRegistry:
    ...
    def register(self, connector: ProviderConnector, specs: tuple[DatasetProductSpec, ...] = ()) -> None:
        ...
        for spec in specs:
            ...
            if not connector.supports(key):
                raise ValueError(f"connector {provider!r} does not support declared ProductSpec {key!r}")
            if not any(source.provider == provider for source in spec.product.sources):
                raise ValueError(f"ProductSpec {key!r} does not declare provider {provider!r}")
            previous = self._specs.get(key)
            if previous is not None and previous != spec:
                raise ValueError(f"conflicting provider ProductSpec registration: {key}")
    ...
    def product_spec(self, logical_key: str) -> DatasetProductSpec:
        try:
            return self._specs[logical_key]
        except KeyError as error:
            raise KeyError(f"provider registry has no ProductSpec for {logical_key!r}") from error
