...
from .models import FeePolicy, TransferInstruction, TransferOperation, TransferStatus
from .policy import TransferPolicy
from .state_machine import TransferOperationStore


class TreasuryCoordinator:
    def __init__(self, store: TransferOperationStore, adapter: TransferAdapter, policy: TransferPolicy) -> None:
        self.store = store
    ...
    def submit(self, transfer_id: str, instruction: TransferInstruction, at: datetime) -> TransferOperation:
        ...
        if result.status is not TransferStatus.SUBMITTED:
            raise ValueError("adapter submission did not return submitted status")
        return self.store.transition(
