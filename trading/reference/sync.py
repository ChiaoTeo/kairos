...
class ReferenceSyncService:
    """Merge normalized adapter reference facts into the authoritative catalog."""

    def __init__(self, catalog: ReferenceCatalog) -> None:
        self.catalog = catalog

    def sync(self, adapter, request) -> ReferenceSyncResult:
        published = adapter.sync(request)
