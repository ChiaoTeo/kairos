...
```bash
./pyenv/bin/python -m kairospy tutorial sma
```

它会在 `example-output/first-study` 创建隔离的 Study Sandbox，自动解析教学 Dataset 的身份、真实内容 hash 和覆盖区间，并在输出的 `next` 字段告诉你下一条命令。该命令可安全重复运行，不会覆盖已有研究。

CLI 默认输出面向人的本地化字段视图和表格，而不是 JSON。可以显式选择语言：

```bash
./pyenv/bin/python -m kairospy --lang zh-CN tutorial sma
./pyenv/bin/python -m kairospy --lang en-US tutorial sma
```

脚本和 CI 才应显式请求稳定 JSON 契约：

```bash
./pyenv/bin/python -m kairospy --format json tutorial sma
```
...
```bash
export FIRST_STUDY_ROOT="$PWD/example-output/first-study"
mkdir -p "$FIRST_STUDY_ROOT"
```
...
```bash
./pyenv/bin/python -m kairospy --help
```
...
系统会在 Workspace 中生成 `study.py`。运行输出中的下一条命令，或直接使用 Python API：
...
现在可以在生成的 `study.py` 中自由增加 DataFrame 计算。例如：
...
```text
$FIRST_STUDY_ROOT/study-candidates/btc-sma-first/1.0.0/
...
```bash
python -m json.tool \
  "$FIRST_STUDY_ROOT/factors/sma-spread/1.0.0/factor_spec.json"
```
...
./pyenv/bin/python -m kairospy \
  ...
  --initial-cash 100000 \
  --fee-bps 10 \
  --artifact-root "$FIRST_STUDY_ROOT/artifacts"
```
...
./pyenv/bin/python -m kairospy run inspect --artifact "$RUN_ARTIFACT"
./pyenv/bin/python -m kairospy run artifact-replay --artifact "$RUN_ARTIFACT" --fixture
```
...
只有真实数据上的 Study Validation 和 Backtest 门槛通过后，才继续：
...
./pyenv/bin/kairospy \
  ...
  --run-root "$FIRST_STUDY_ROOT/runs/sma"
```
...
当前 CLI 已打通 Study、Factor、Strategy 和 Run，但还没有一个面向研究者的统一命令来提交并展示“统计验证结果、样本切分、通过门槛和最终结论”。因此目前第 2 步的探索结果与第 8 步的研究结论仍需研究者在文档或现有 Study Validation API 中记录。

下一阶段产品改造应优先补齐：

```text
kairospy study validate
kairospy study inspect
kairospy study conclude --decision accept|reject
```

并让 Strategy promotion 强制引用通过的 Study Validation Evidence。否则系统在工程链路上已经贯通，但对第一次使用者来说，研究过程仍会显得在 `study freeze` 与 `factor register` 之间“跳了一步”。
