...
Examples 必须让开发者从公开入口理解并运行本次改造，而不是只能阅读内部测试。覆盖范围包括：治理回测、Canonical Replay、公共实时 Quote、实时 OrderBook、策略 Live/Replay、运行模式组合、长跑运维和 Python/Rust connector 契约。
...
| Python/Rust connector boundary | `examples/connectors/reference_connector/verify_contract.py` | Python reference 无 | `test_examples_suite` |
