...
状态：Proposed  
适用范围：`trading.domain`、`trading.data`、`trading.market_data`、`trading.reference`、`trading.research`、`trading.backtest`、数据 CLI 与 `data/` 目录  
...
1. `ResearchDataClient`、`ParquetMarketEventRepository`、`HistoricalDataset/DatasetRepository`、`BarRepository` 等多套读取和存储模型并存；
...
- Domain 不依赖 `trading.data`、`trading.reference`、`trading.storage`、`trading.research` 或 `trading.backtest`；
...
- `ResearchDataClient.get()`、`replay()`、`replay_slices()`、`plan()`、`acquire()` 和研究冻结能力；
...
| `HistoricalDataset + DatasetRepository` | MarketSlice 回放 | 迁入统一 Release storage contract，Repository 降为内部 driver |
| `ResearchDatasetStore` | MarketSlice append session | 合并到统一 Publisher/Collection Service |
...
当前部分代码通过目录名判断数据类型，例如识别 `canonical/market/dataset=*` 后选择 Event Repository，或通过 `dataset.json` 判断 HistoricalDataset。这会导致：
...
```yaml
storage:
  kind: tabular | market_events | market_slices | reference
  format: parquet
  layout_version: 1
...
当前逻辑产品定义、物理布局、Schema、Capabilities 和动态 Provider 配置分散在多个位置。目标是统一为一个 `DatasetProductSpec`：
...
```text
domain <- application services <- data/adapters/storage
...
- `InstrumentDefinition`、`ListingDefinition`、`ProductSpec`；
...
- Adapter 负责将 Provider Payload 转为 Canonical Record；
...
当前 `domain.strategy.StrategyContext` 直接引用具体 `ReferenceCatalog`，并通过类型引用 Backtest、Research 和 Volatility 对象。建议将 StrategyContext 移出 Domain，放入 `trading.strategies.runtime` 或应用层。
...
| 对象 | 迁移目标 | 删除验收 |
|---|---|---|
| `trading.history.BarRepository` | Canonical OHLCV Release | 全仓无生产 import；CLI 已替换；数据已核对 |
| `trader history *` | `trader data prepare/query` 与正式 backtest | CLI help 中不再出现 history |
| `strategies.sma_cross -> BarDataset` | Arrow rows 或统一 Bar Series Port | 策略不再 import `trading.history` |
...
- 将 `DatasetRepository` 变成 Data 内部 `MarketSliceStorageDriver`；
...
trader data query <product-or-release> --start ... --end ...
trader data replay <product-or-release> --start ... --end ...
...
Provider 运维命令可以保留在 `trader provider massive ...` 或内部运维组，不应与普通数据产品命令混在同一层级。
...
- Schema 字段、类型、单位和主键；
- 质量等级、最近一次质量报告及失败检查；
- 是否支持 point-in-time、SQL、事件 replay 和 MarketSlice replay；
- 已知限制；
...
- CI 可检测新增的 `trading.history`、直接 `DatasetRepository` 和硬编码 `data/...` 读取；
...
- Event、Tabular、MarketSlice Reader 实现统一内部 Protocol；
- Product 定义合并为 DatasetProductSpec；
...
- `trading/domain` 中无 `from trading.*` 的跨包实现依赖；
- Domain 单元测试不需要文件系统、Catalog JSON、Arrow 或 Provider；
- Adapter contract 测试证明相同 Provider payload 可稳定映射到 Canonical Record；
...
- CLI golden tests 覆盖成功、缺数据、质量不足、无凭证和禁止网络获取。
...
- Provider Adapter 不直接修改 Catalog Registry；
- 只有 Data Publishing Service 可以发布 Release；
...
- Massive SPXW Events -> MarketSlice -> Backtest。
...
控制：统一的是公开契约，不要求 Event、Tabular 和 MarketSlice 使用相同物理实现。内部 storage driver 可以针对 Arrow、Parquet 和确定性重放分别优化。
...
6. 迁移 Bar、MarketSlice、Surface 和旧 Studies 数据；
...
- [ ] `BarRepository` 和 `trader history` 已删除；
