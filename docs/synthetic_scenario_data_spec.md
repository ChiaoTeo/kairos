# Backtest Synthetic Scenario 数据规范

本规范定义可长期用于回测单元测试、状态机测试、场景测试和确定性回放测试的合成数据。Synthetic Scenario 只验证软件行为，不作为策略有效性的证据。
...
旧 synthetic compatibility 入口仍保留，但新代码不再使用。
...
├── lifecycles: tuple[InstrumentLifecycleSnapshot, ...]
...
- code version；
- 对 definitions+lifecycles+slices 的 SHA-256；
- `split`：development、validation 或 test；
- `synthetic=true`。

### InstrumentDefinition 与 InstrumentLifecycleSnapshot

每个数据集必须带历史有效的 `InstrumentDefinition`，其中 `InstrumentId` 只保存稳定内部身份，产品字段位于 tagged `InstrumentContractSpec`，Venue symbol/tick/lot/min notional 位于 `ListingDefinition`。

每个可交易合约必须提供 `InstrumentLifecycleSnapshot`：
...
- SPXW synthetic fixture 使用 `America/New_York`；
- 默认合约乘数为 100；
- 默认组合为卖出 6000 Put、买入 5950 Put；
- 默认入场自然信用为 2.80；
- 新订单只允许在后续 MarketSnapshot 成交。
...
- test：策略与参数冻结后才运行；
- synthetic fixture 的三种 split 只验证流程隔离，不提供统计意义；
...
Synthetic scenario 数据若用于长期 golden test，内容哈希应纳入断言。只有经过评审的语义变更才能更新 golden hash。
