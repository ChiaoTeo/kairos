...
本文给出 Trader 项目从当前多资产交易框架演进为稳定、可扩展、可审计的交易系统框架的完整改造方案，集中解决两类问题：
...
- 旧版 Catalog 确定性迁移、V2 JSON 持久化、CLI migration 和 adapter sync bridge；
...
- Binance 内部划转/链上提现 adapter 与 provider-neutral Bank transfer adapter；
...
- 旧版 IBKR/Binance adapter 内部参数从 `InstrumentDefinition` 改为解析后的轻量 contract；
- 旧 ResearchSnapshot builder 和少量 mock/compatibility 分支移除；
- 旧版 `trading.reference.ReferenceCatalog`、嵌套 `ListingDefinition` 和兼容 facade 的最终删除；
...
| 产品定义 | `InstrumentDefinition + ProductSpec` | 表达产品类型和经济条款 |
...
- `ProductType` 和 `ProductSpec` 可能构造非法组合；
...
- Execution route：某账户通过哪个 Broker/Adapter 交易该 Listing；
...
在 `trading.reference.identity` 中增加：
...
Pricing model 只消费标准 PricingContext，不再直接读取多种 ProductSpec 并使用 `getattr` 猜测 underlying、multiplier 或 settlement asset。
...
5. Market/reference adapters 改为写新 mapping；
...
3. 实现 CryptoTransferAdapter 接口和 Binance adapter；
...
1. ExecutionRoute 全量接管 Instrument -> Adapter 解析；
...
内部逐步代理到新 repositories 和 TreasuryService，同时记录 deprecated metrics。调用量归零后删除。
