...
本文定义 Kairos 下一阶段的两条产品主线：

1. 为策略开发者提供便捷、统一、低样板代码的回测开发体验；
2. 系统化管理研究、回测、历史模拟、Paper Trading 和 Live 之间的数据与运行差异。
...
6. Bar、MarketSnapshot、Option Snapshot 等在线/离线 Builder 尚未全部共享实现；
7. Paper Trading 的本地验收偏重 Capture Replay，真实持续流、背压、迟到事件和恢复证据不足；
...
- 为 Bar、Quote State、OrderBook、MarketSnapshot、Option Snapshot 建立共享增量 Builder；
...
> Kairos 既提供便捷的回测开发体验，也能以可验证、可解释的方式管理研究、回测、模拟和实盘之间不可避免的差异。
