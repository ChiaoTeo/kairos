...
版本：1.0  
基线日期：2026-07-17  
上位设计：`docs/async_dataflow_trading_system_refactor.md`  
...
本文是实施清单和验收依据，不重复讨论为什么选择异步数据流、为什么不迁移 `kairos_v2`，相关决策以上位设计为准。
...
| W6 | 市场数据 Adapter 收敛 | Massive/Binance/IBKR 统一接入 | P1 |
...
- 存在唯一正式组合根定义；
- Domain 不依赖 Data、Adapter、Runtime、Storage；
- Strategy 不依赖具体 Venue Adapter；
- Risk 不依赖 WebSocket/HTTP/SQLite 实现；
- Adapter 不直接修改 Ledger、Portfolio 或 Strategy State；
...
3. Planner 通过 Adapter capability 验证支持程度；
4. 合并多个策略的重复需求；
...
1. 保留 `TradingApplication` 的状态机和持久状态；
...
## 11. W6：市场数据 Adapter 收敛

### 11.1 要完成的工作

- 将 Massive、Binance 和 IBKR 接入统一生命周期；
- 分离 Transport、Venue Codec、Normalizer 和 Port Adapter；
- 统一 RawFrame、sequence、reconnect 和 fault；
- 统一 health/readiness；
- 清除 Adapter 直接调用上层 Consumer 的特殊路径。
...
- Port Adapter 只负责生命周期、capability 和 EventSource/Sink。

### 11.4 验收边界

每个 Adapter 必须通过同一 Contract Test：
...
- Adapter 直接更新 Strategy State；
- Adapter 直接写 Ledger；
- Venue raw object 离开 Adapter；
...
- Live Paper 使用实时 EventSource + 模拟 Execution Driver；
- Testnet/Live 只替换 Adapter 和 Safety Policy；
...
#### Historical Simulation -> Live Paper
...
-至少一个 Python Adapter 通过完整 Contract Test；
-W10 可以定位跨进程事件。
...
-Adapter 内建立第二套 Order State；
-Adapter 内决定全局 readiness；
-Adapter 直接提交 Ledger。
...
| 模块 | Domain | Contracts | Catalog | Data | Market Runtime | Strategy | Risk | Execution | Accounting | Application | Adapters | Storage |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
...
| Market Runtime | 是 | 是 | Port | Capture Port | 内部 | 否 | 否 | 否 | 否 | Supervisor Port | Market Adapter Port | 可选 |
...
| Adapters | 是 | 是 | Listing 输入 | 否 | Port 实现 | 否 | 否 | Port 实现 | 否 | 生命周期 Port | 内部 | 否 |
...
| 错误位置 | Retry owner |
|---|---|
| Transport reconnect | Adapter/Transport |
...
包含：W4、W5、Massive Adapter、W7、W10 基础。  
...
包含：W9。  
退出：同一策略通过 Research、Backtest、Historical Sim 和 Live Paper。
...
11. 更新实施进度文档，再开始 Massive Adapter 迁移。
