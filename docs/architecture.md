...
旧 `trading/core`、旧单币种 Portfolio 和旧 broker facade 已删除。

## 2. 产品与 Venue

| 产品 | ProductSpec | Reference/Market | Execution/Account | Lifecycle |
|---|---|---|---|---|
...
- `IbkrReferenceAdapter` 通过 `ReferenceDataRequest.product_type` 分别同步股票、ETF 和上市期权；
- `IbkrMarketDataAdapter`、`IbkrExecutionAdapter`、`IbkrAccountAdapter` 消费通用定义；行情适配器提供 quote、recent trade 和 historical bar；
- Reference、MarketData、Execution 分别声明 capability，指数可查询行情但不会被误路由到执行层；
- `IbkrSpxwResearchAdapter` 的命名明确表明它只负责 SPX/SPXW option-chain 研究切片；
- 通用 `MarketSlice` 不包含强制 `underlying_price`，股票、ETF 和加密产品直接保存 instrument quote；确有参考价格时使用 `reference_prices`。

Capability 以 adapter 端口为边界，而不是描述 Venue 的能力全集：
...
- 不保留宽泛的 `VenueCapabilities` 兼容类型，新增 adapter 必须在类型层面选择所属端口。
...
- execution adapter product/order capability；
...
- Binance adapter 包含 rate limit、clock sync、WebSocket reconnect、REST backfill、账户/订单/持仓恢复；
...
| 验收面 | 自动化证据 |
|---|---|
| Catalog 历史版本、ProductSpec、capability | `test_multi_asset_domain.py` |
| IBKR 股票/期权分离、股票 quote/trade/bar 与 adapter capability | `test_ibkr_adapters.py` |
...
| 股票/期权/现货/永续/交割合约/加密期权 Conservative/Stress 与无前视 | `test_backtest_models.py`, `test_multi_asset_backtest.py`, `test_reference_scenarios.py` |
| SPXW golden、split、replay/audit hash | `test_backtest_engine.py`, `test_settlement.py` |
| 股票+加密通用 MarketSlice | `test_normalized_series.py` |
