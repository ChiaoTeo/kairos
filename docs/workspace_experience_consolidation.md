...
kairospy workspace attach alpha --name bars --dataset bars.us.equity.1d --view history
```
...
如果需要治理或发布可运行入口，在 RunConfig 里记录 workspace 与 strategy entrypoint：

```toml
[run]
workspace = "my_workspace:build_workspace"
strategy = "my_strategies.sma_cross:SmaCross"
```
...
```bash
kairospy workspace attach alpha --name quote --dataset market.quote.crypto.binance.btc-usdt --view live
...
- 策略返回 `strategy.intents.Intent`；Intent 是唯一输出合同，并通过 `reason` 同时承载审计/解释记录。
...
| `decide(context) -> Intent | list[Intent]` | 简单 research / signal 入口，可进入统一 intent 审计链 |
...
from kairospy.strategy import TargetPositionIntent
...
kairospy workspace attach NAME --name LOCAL --dataset DATASET --view history|live|both
kairospy workspace inspect NAME
kairospy workspace inspect-code module:callable --mode backtest|paper|live
...
```text
study add-data      -> workspace attach --view history|both
strategy add-data   -> workspace attach --view history|both
strategy set-model  -> RunConfig [run].strategy
...
- `Intent` 作为唯一 strategy output contract。
...
```text
kairospy workspace create alpha
kairospy workspace attach alpha --name bars --dataset ... --view history
python src/my_script.py
kairospy run start --config configs/runs/backtest.toml
```
...
- 新增统一 `kairospy workspace create/attach/inspect/inspect-code`。
...
- 已完成：统一 `workspace create/attach/inspect`，历史和实时通过 `workspace attach --view history|live|both` 表达，不再保留 `bind-data` / `bind-live` 两条用户入口。
