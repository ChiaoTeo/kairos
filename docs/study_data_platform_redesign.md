...
状态：Draft
适用范围：`kairospy.data`、市场数据适配器、数据湖、研究和回测模块
目标读者：研究员、数据工程人员、策略开发人员和平台维护者

## 1. 背景与结论

当前项目已经具备 Parquet、Dataset Catalog、`DatasetClient`、point-in-time 事件读取、
...
标准 Alias 为 `@latest`、`@latest-validated`、`@study` 和 `@production`。
...
```bash
kairospy data plan --dataset market.option_quotes.us.spxw --start 2025-01-01 --end 2025-02-01
kairospy data acquire --plan PLAN_ID
```
...
draft → registered → validating → validated
      → approved_for_study → approved_for_backtest → approved_for_production
...
逐个迁移现有 study 模块；Repository 降级为内部接口；废弃物理目录 fallback；验证后删除
...
| RFC 范围 | 实现 | 主要验证 |
|---|---|---|
| 4、10 身份与 Catalog V3 | `kairospy/data/contracts.py`、`catalog.py`、`publishing.py` | V1/V2 迁移、V3 round-trip、Release 不可变、promotion audit |
...
| 8 统一查询 | `DatasetClient.get/stream/sql/replay/replay_snapshots` | Arrow/Polars/Pandas/rows、字段校验、半开时间范围 |
...
| 13 用户用例 | `examples/data/` 与 `study_data_guide.md` | 发现、读取、plan、acquire、多来源、跨 venue、replay、freeze、compare |
...
python3 -m compileall -q kairospy examples/data tests studies
```
