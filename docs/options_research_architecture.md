...
```text
valuation: OptionValuationService
surface: SurfaceView
...
- 调用 connector 或 execution gateway；
- 在本地重新拟合曲面；
- 读取决策时点之后的数据；
- 根据成交结果反向修改历史信号；
...
现有 `MarketSnapshot` 和确定性引擎继续作为核心。数据格式升级后，每个 slice 必须描述当时可见的动态合约宇宙。
...
normalized/   point-in-time MarketDataSnapshot / MarketSnapshot
