...
- [x] 新增 `kairospy.contracts`，建立 transport-independent `CanonicalEventEnvelope`。
...
- `kairospy/contracts/market_data.py`
- `kairospy/market_data/stream.py`
- `kairospy/market_data/projections.py`
- `kairospy/data/feed.py`
...
- `kairospy/application/service_supervisor.py`
- `kairospy/application/async_runtime.py`
- `kairospy/market_data/subscriptions.py`
- `kairospy/connectors/massive/websocket.py`
...
- `kairospy/execution/command.py`
- `kairospy/execution/outbox.py`
- `kairospy/orchestration/runtime_store.py`
...
- [x] 新增 `kairospy data live-binance`，无需账户凭据即可采集公开 Binance Quote/Trade/Depth，输出 Raw 与 Canonical Journal。
- [x] 真实公网 `BTCUSDT@bookTicker` 集成验证通过：2 raw messages、2 canonical Quote、Capture hash、Replay 和 Quote Projection 完全一致。
- [x] 新增 `kairospy data soak-binance` 和可审计 Soak Artifact，显式验收持续时间、事件数、重连、序列回退、最大静默、尾部静默、生产者错误、Raw/Canonical 数量和未规范化消息。
...
- `kairospy/application/modes.py`
- `kairospy/connectors/binance/stream.py`
- `kairospy/connectors/binance/order_book.py`
- `kairospy/market_data/capture.py`
- `kairospy/market_data/soak.py`
- `kairospy/market_data/projections.py`
...
- [x] 将 `Strategy`、`StrategyContext`、`StrategyDecision` 从 `kairospy.domain` 移到 `kairospy.strategies.strategy_protocols`。
- [x] 删除 `kairospy/domain/strategy.py`。
- [x] 新增 AST 架构测试，禁止 Domain 依赖 Data、Catalog、Backtest、Study、Storage 等上层模块。

证据：

- `tests/test_architecture_boundaries.py`
- 全仓无 `kairospy.domain.strategy` 运行契约引用。
...
- [x] `kairospy trade` 先经过 KairosApplication Probe，再启动 Coordinator。
...
- [x] 新增正式 `kairospy runtime reference-artifact --root <isolated-root>` Application 场景，贯通 Market Data → Strategy → Intent → Risk → Order → Fill → Ledger → Portfolio → Reconciliation → Restart READY。
- [x] Runtime Golden 保存固定 Ledger hash 和全链路 audit hash，并由自动化测试锁定。

证据：

- `kairospy/application/clock.py`
- `kairospy/application/config.py`
...
- [x] 新增账户控制锁的基础数据库契约。
- [x] `kairospy trade` 已接入 SQLite Runtime Store。
...
- [x] `kairospy trade` 已组合 VenueOrderRecoveryService，启动时先恢复 crash-window order 再执行 Projection/Reconciliation。
...
- [x] 删除 `kairospy.history`、`BarRepository`、旧顶级 `kairospy history` 和旧测试。
...
- [x] 从 `kairospy.backtest.feed` 删除公开 DatasetRepository，物理读写迁入 Data 内部 MarketSnapshotStorageDriver。
...
- `kairospy/data/contracts.py`
- `kairospy/data/catalog.py`
- `kairospy/data/client.py`
...
可使用 `kairospy runtime l4-preflight --venue ... --environment ... --strategy ... --instrument ...`
重复检查，输出不会包含凭证内容。
...
Command: kairospy runtime reference-artifact --root data/runtime-reference-artifact
...
Command: kairospy runtime failure-policy --root data/runtime-reference-artifact
