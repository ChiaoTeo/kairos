...
- [x] 新增 `trading.contracts`，建立 transport-independent `CanonicalEventEnvelope`。
...
- [x] 新增异步 `EventSource` 和有限 iterable adapter。
...
- `trading/contracts/market_data.py`
- `trading/market_data/stream.py`
- `trading/market_data/projections.py`
- `trading/data/feed.py`
- `tests/test_async_market_stream.py`

### 异步数据流 M2 Runtime 与实时试点

- [x] 新增 `AsyncTaskSupervisor`，统一拥有长期任务，记录状态、attempt、restart count 和结构化 fault。
- [x] Managed Task 支持 criticality、有限 restart、正常完成声明和 bounded shutdown。
- [x] 新增 `AsyncTradingRuntime`，将异步任务健康状态绑定到既有持久 `TradingApplication` 生命周期。
...
- [x] `SubscriptionPlanner` 使用 Catalog Listing 和 Adapter Capability 在联网前验证需求，并合并多个策略的重复订阅。
...
- `trading/application/task_supervisor.py`
- `trading/application/async_runtime.py`
- `trading/market_data/subscriptions.py`
- `trading/adapters/massive/websocket.py`
...
- [x] `DurableOrderDispatcher` 作为 Async Runtime 长期任务运行，并使用线程卸载兼容现有同步 Venue Adapter。
...
- `trading/execution/command.py`
- `trading/execution/outbox.py`
- `trading/orchestration/runtime_store.py`
- `tests/test_durable_order_outbox.py`

### 异步数据流 M4 模式组合与 M5 Binance 公共实时行情

- [x] 新增可审计 `RunModeComposition`，显式声明 Research、Backtest、Historical Simulation、Live Paper 和 Live 的 EventSource、Clock、Execution Driver、Persistence、Safety 与 Capture。
...
- [x] 新增 `trader data live-binance`，无需账户凭据即可采集公开 Binance Quote/Trade/Depth，输出 Raw 与 Canonical Journal。
- [x] 真实公网 `BTCUSDT@bookTicker` 集成验证通过：2 raw messages、2 canonical Quote、Capture hash、Replay 和 Quote Projection 完全一致。
- [x] 新增 `trader data soak-binance` 和可审计 Soak Artifact，显式验收持续时间、事件数、重连、序列回退、最大静默、尾部静默、生产者错误、Raw/Canonical 数量和未规范化消息。
...
- [x] 新增 `CanonicalQuoteSliceProjection`，将 Canonical Quote 转为正式 StrategyContext 使用的 point-in-time MarketSlice，Live 与 Backtest/Replay 不需要两套策略入口。
...
- [x] 新增 `examples/README.md`，按 Governed Backtest、Live Quote、OrderBook、Replay、Run Mode、Adapter Contract 和长跑 Soak 组织入口。
...
- [x] 新增 Live-vs-Replay Strategy example、五模式 Composition example 和 Python/Rust 共用 Adapter golden-vector verifier。
...
- `trading/application/modes.py`
- `trading/adapters/binance/stream.py`
- `trading/adapters/binance/order_book.py`
- `trading/market_data/capture.py`
- `trading/market_data/soak.py`
- `trading/market_data/projections.py`
- `trading/strategies/event_session.py`
...
- [x] 将 `Strategy`、`StrategyContext`、`StrategyDecision` 从 `trading.domain` 移到 `trading.strategies.base`。
- [x] 删除 `trading/domain/strategy.py`。
- [x] 新增 AST 架构测试，禁止 Domain 依赖 Data、Catalog、Backtest、Research、Storage 等上层模块。

证据：

- `tests/test_architecture_boundaries.py`
- 全仓无 `trading.domain.strategy` 运行契约引用。
...
- [x] 新增 TradingApplication 生命周期：STARTING、RECOVERING、RECONCILING、READY、RUNNING、REDUCE_ONLY、STOPPED。
- [x] Persistence 和可组合 Readiness Probe 在 READY 前执行。
- [x] `trader trade` 先经过 TradingApplication Probe，再启动 Coordinator。
...
- [x] 新增正式 `trader runtime golden --root <isolated-root>` Application 场景，贯通 Market Data → Strategy → Intent → Risk → Order → Fill → Ledger → Portfolio → Reconciliation → Restart READY。
- [x] Runtime Golden 保存固定 Ledger hash 和全链路 audit hash，并由自动化测试锁定。

证据：

- `trading/application/clock.py`
- `trading/application/config.py`
...
- [x] 新增账户控制锁的基础数据库契约。
- [x] `trader trade` 已接入 SQLite Runtime Store。
...
- [x] `trader trade` 已组合 VenueOrderRecoveryService，启动时先恢复 crash-window order 再执行 Projection/Reconciliation。
...
- `tests/test_trading_application.py`
...
- [x] 支持 tabular、market_events、market_slices、reference。
- [x] ResearchDataClient 按 storage kind 分发 Event 和 MarketSlice Reader。
...
- [x] 新增统一 DatasetProductSpec，覆盖 Product 身份、Schema、物理布局、storage kind、能力、质量 Profile 和最低发布等级。
- [x] Catalog schema v4 持久化 ProductSpec；当前 15 个 Product 全部拥有统一 Spec。
- [x] 内置产品、动态 Massive 配置、Provider Registry、Catalog Bootstrap 和 Publisher 消费同一 ProductSpec。
- [x] 删除 `models.Datasets` 的平行产品定义；兼容 Datasets handle 由权威 ProductSpec 派生。
- [x] ManagedDataset 降为 DatasetProductSpec 的兼容别名，不再是第二种产品模型。
- [x] ProductSpec 发布门禁阻止低于声明最低质量等级的 Release。
...
- [x] Quality Engine 只按 ProductSpec 选择 Profile，不再由目录或字段猜测正式产品类型。
...
- [x] MarketSlice Profile：Manifest/Release 身份、切片顺序、Universe Definition、合约/报价覆盖、staleness、future fact、crossed quote、critical issue 和冻结输入。
- [x] 从 Massive SPXW Event 与旧 MarketSlice 双重冻结输入生成新的完整 Curated MarketSlice Release。
- [x] 新 SPXW Release 过滤首个缺失行情切片后达到 4 slices、100% contract/quote coverage、0 stale、0 future fact、0 critical issue，并晋级 Q3。
- [x] 新增 `backtest golden-spxw`，固定 Event、Source Slice、Curated Slice、Quality Report 和 conservative/stress audit hash。
...
- [x] 删除 `trading.history`、`BarRepository`、旧顶级 `trader history` 和旧测试。
- [x] README 和 History Notebook 改用 ResearchDataClient 和治理回测。
- [x] 旧 `data/history` 已不存在；迁移报告记录 Release、行数、质量与 report hash。

### MarketSlice 与 Surface 旧路径收敛

- [x] 从 `trading.backtest.feed` 删除公开 DatasetRepository，物理读写迁入 Data 内部 MarketSliceStorageDriver。
- [x] 正式 Research、Volatility 和 Backtest CLI 只通过 ResearchDataClient 解析不可变 Release。
- [x] ResearchDatasetStore 更名并收敛为 MarketSliceCollectionPublisher。
- [x] 7 条旧 `data/datasets` Release 在核验 Manifest/Release hash 后迁至统一 Curated Release 布局。
- [x] 所有迁移后的 MarketSlice 均通过 ResearchDataClient 重放，并与原 content hash 一致。
...
- `trading/data/models.py`
- `trading/data/catalog.py`
- `trading/data/client.py`
...
- [x] 正式 Trading CLI 只通过 TradingApplication/RuntimeSupervisor 启动；Coordinator 不再以布尔参数激活正式路径。
...
- [x] 合并 Datasets、ManagedDataset 和动态配置为 DatasetProductSpec。
...
- [x] DatasetRepository 删除，MarketSliceStorageDriver 仅作为 Data 内部物理 Driver。
...
- [x] Massive SPXW Event -> MarketSlice -> Strategy Golden Pipeline。
...
4. 根据真实 L4 结果修复 Adapter、Runtime 或运维缺口，全部退出条件有证据后再关闭 M5。
...
可使用 `trader runtime l4-preflight --venue ... --environment ... --strategy ... --instrument ...`
重复检查，输出不会包含凭证内容。
...
Command: trader runtime golden --root data/runtime-golden
...
Artifact: data/runtime-golden/artifacts/runtime-l2-spot-target-position-v1/manifest.json
```

该场景使用正式 TradingApplication、TradingCoordinator、ExecutionRouter、SQLite Runtime Store、
...
```text
Matrix: runtime-l3-failure-matrix-v1
Command: trader runtime failure-matrix --root data/runtime-golden
...
Artifact: data/runtime-golden/artifacts/runtime-l3-failure-matrix-v1/manifest.json
```
...
Source MarketSlice: spxw.massive.synthetic-forward.20251103.https.v1
Source hash: bbd9a2c79ae2b0f9b4f3099be4b8a108c385270ed6f2e1cb803501243d258d4a
Curated Q3 MarketSlice: ds_c5cd07c6a542b4af665709b6
...
该 Pipeline 证明 Event → Curated MarketSlice → Strategy Backtest 的技术复现闭环。其样本仅包含 4 个一分钟切片，只能作为 Golden mechanics evidence，不能作为策略统计有效性或 Live 晋级证据。
