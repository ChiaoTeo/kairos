...
状态：Draft  
适用范围：`trading.data`、市场数据适配器、数据湖、研究和回测模块  
...
4. Event Dataset、HistoricalDataset 和普通 Canonical Dataset 仍暴露不同读取方式；
...
公共 API 接受 `DatasetProduct | DatasetKey | str`，文档优先展示 `DatasetProduct`。
...
```bash
trader data plan --dataset market.option_quotes.us.spxw --start 2025-01-01 --end 2025-02-01
trader data acquire --plan PLAN_ID
```
...
兼容 CSV；合并重复 HistoricalDataset 抽象。
...
- 策略接收的 `MarketView`/`MarketSlice` 契约；
- 订单、风控、组合和估值领域模型。
...
```text
                          ┌─ HistoricalReplayAdapter ─ 回测
Canonical Event Contract ├─ PaperStreamAdapter      ─ 模拟
                          ├─ LiveStreamAdapter       ─ 实盘
                          └─ BatchQueryAdapter       ─ 研究
...
现有 `BacktestClock`、确定性 ID 和 Dataset hash 是正确基础，但 HistoricalDataset 的生成必须继续
...
1. **Historical simulation**：与回测共享 Replay Adapter，但使用更接近实盘的异步调度和执行模型；
2. **Live paper trading**：消费与实盘相同的实时市场流，但订单发送到模拟执行 adapter。
...
现有 Massive live journal、sequence-gap 和 reconnect-backfill hook 可以成为 Live Adapter 的起点，
...
运行模式决定允许的数据来源、Alias、acquisition policy、时钟、执行 adapter、账户类型和质量门槛。
...
规定：共享事件和 MarketView 契约，分别实现 batch、replay 和 stream adapter。
...
Catalog V3、类型生成、多来源 policy 和运行时 adapter 不应一次性构建成通用分布式平台。每一阶段必须
...
| RFC 范围 | 实现 | 主要验证 |
|---|---|---|
| 4、10 身份与 Catalog V3 | `trading/data/models.py`、`catalog.py`、`publishing.py` | V1/V2 迁移、V3 round-trip、Release 不可变、promotion audit |
...
| 7 类型化接口 | `Datasets`、`DatasetProduct`、Enum、FieldRef | Product 常量与受管 Product 使用同一对象，不要求背 key 字符串 |
| 8 统一查询 | `ResearchDataClient.get/stream/sql/replay/replay_slices` | Arrow/Polars/Pandas/rows、字段校验、半开时间范围 |
...
python3 -m trading data catalog
python3 -m trading data doctor
python3 -m compileall -q trading research examples/data tests
```
