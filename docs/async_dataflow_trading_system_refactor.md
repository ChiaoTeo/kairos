...
本文专门用于指导 Trader 下一阶段的系统级改造，统一以下目标：
...
- `InstrumentDefinition + ProductSpec + ListingDefinition + Capability`；
...
- Market Data Adapter 的正式端口仍以 `snapshot()` 为主；
...
- Raw Journal、sequence 检查和 reconnect backfill 只在部分 Adapter 中成立；
...
`VenueMessage` 只在 Adapter 内部存在，用于表达 Provider 原生协议。
...
Runtime 汇总全部 Strategy Requirement，并与 Adapter capability、Catalog Listing 和当前订阅状态求交集，生成 `SubscriptionPlan`。
...
只替换 Execution Adapter 和环境安全配置。Strategy 不允许基于环境分叉业务逻辑；环境差异通过 capability、limits、fees、latency、listing 和 deployment config 表达。
...
每个 Adapter，包括 Rust sidecar，必须通过同一套 contract tests：
...
```text
trader runtime tasks
trader runtime streams
trader runtime queues
trader runtime trace <trace-id>
trader data trace --instrument ... --start ... --end ...
trader data explain-event <event-id>
trader data inspect-gap <stream-id>
trader data replay-session <session-id>
trader data compare-session <live-session> <replay-run>
trader execution explain-order <client-order-id>
```
...
同一套 Adapter contract test 运行于：

- Python fake；
- Simulated Adapter；
- Binance/IBKR/Massive Adapter；
-未来 Rust gateway。
...
-Python/Rust Adapter 通过同一语言无关契约接入；
...
3. 定义异步 `EventSource`、`EventSink`、`CommandSink` 和 Adapter lifecycle；
...
8. 将 Massive stream 作为首个统一异步 Adapter 试点；
