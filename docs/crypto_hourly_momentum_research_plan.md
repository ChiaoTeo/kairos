...
```text
research/crypto_hourly_cross_sectional_momentum/
  README.md
  protocol.yaml
  universe.py
  data_quality.py
...
1. 创建 `research/crypto_hourly_cross_sectional_momentum/` 和研究协议；
...
```bash
./pyenv/bin/python -m trading study start crypto-hourly-momentum \
...
```bash
./pyenv/bin/python -m trading study plan crypto-hourly-momentum \
  --start 2020-01-01T00:00:00+00:00 \
