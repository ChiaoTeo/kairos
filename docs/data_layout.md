...
`trader data migrate-parquet --delete-csv` 删除经过行数校验的 CSV 副本。
...
- `trading.data`：Dataset Catalog、Product/Release、查询、Feed、获取计划和元数据；
- `trading.adapters.*.datasets`：Provider Connector，执行 Source 到 Canonical 发布；
- `trading.features`：point-in-time safe 的跨策略特征，不允许保存未来标签；
- `research/*`：生成未来标签、样本切分、假设检验和报告，只通过 Dataset ID 或逻辑名称读取受管数据。
...
```python
from trading.data import DataView, OptionQuoteFields, OutputFormat, ResearchDataClient
from trading.data.models import Datasets
...
`replay()`，定频 MarketSlice 回测使用 `replay_slices()`，SQL 分析使用 `sql()`。
...
```bash
python3 -m trading data plan --dataset market.ohlcv.crypto.binance.btc-usdt.1d --provider binance --venue binance --start 2021-03-24T00:00:00+00:00 --end 2026-07-15T00:00:00+00:00
...
python3 -m trading data plan --dataset market.ohlcv.crypto.binance.btc-usdt.1d --start 2025-01-01T00:00:00+00:00 --end 2025-02-01T00:00:00+00:00
python3 -m trading data migrate-parquet --dry-run
