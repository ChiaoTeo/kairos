...
### 2.1 Study 应该“探索灵活，晋级固定”

Study 不应被完全固定，否则会失去提出假设、尝试新数据、使用新统计方法和快速失败的能力；Study 也不能完全自由，否则研究结论无法复现，因子无法进入回测，回测无法证明运行的是同一套逻辑。
...
- Study Validation 和 Promotion Gate。
...
项目已经定义 Study、Backtest、Historical Simulation、Live Paper 和 Live 的组合差异，但尚未让这些组合真正构造同一个 Strategy Run Loop。
...
study readiness/governance-audit
...
|---|---|---|---|---|---|
| Study | Frozen Release/Batch Query | Analysis | None/Label Proxy | Study Artifact | 预测证据 |
...
1. 明确 Sandbox Study 与 Governed Study；
...
| 能力 | Study | Backtest | Historical Simulation | Paper/Testnet | Live |
|---|:---:|:---:|:---:|:---:|:---:|
