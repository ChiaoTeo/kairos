...
```python
feed = data.replay_slices(Datasets.CURATED_MARKET_SLICES_OPTIONS_US_SPXW)

for market in feed.between(start, end):
    ...
```

`replay()` 和 `replay_slices()` 在构造时冻结 Release ID 与内容哈希，之后不再解析 Alias、访问
...
```python
from trading.data import AcquirePolicy
...
```bash
trader data plan --connector-config examples/data/massive_connector.example.json \
...
trader data acquire --connector-config examples/data/massive_connector.example.json \
  --dataset market.events.options.us.spxw --provider massive --venue opra \
  --start 2026-07-17T13:30:00+00:00 --end 2026-07-17T20:00:00+00:00
```
...
```bash
trader data compare --first ds_old --second ds_new
```
...
研究只使用 `get()`、`iter_rows()`、`replay()` 和 `replay_slices()`；底层 Repository 仅供 Connector
和存储实现使用。旧的 `load()`、`scan_events()`、`load_historical_dataset()` 与重复 Historical Feed
已经删除，避免用户判断多套同义接口。
...
trader data catalog
trader data migrate-parquet --dry-run
trader data migrate-parquet
```
