...
```python
from kairos.data import AcquirePolicy
...
```bash
kairos data plan --connector-config examples/data/massive_connector.example.json \
...
kairos data acquire --connector-config examples/data/massive_connector.example.json \
  --dataset market.events.options.us.spxw --provider massive --venue opra \
  --start 2026-07-17T13:30:00+00:00 --end 2026-07-17T20:00:00+00:00
```
...
```bash
kairos data compare --first ds_old --second ds_new
```
...
kairos data catalog
kairos data migrate-parquet --dry-run
kairos data migrate-parquet
```
