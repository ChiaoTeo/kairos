...
- `kairospy.domain` 中统一的 `InstrumentDefinition`、Quote、Trade、Bar 和 OrderBook 类型；
- `kairospy.reference` 中内部 `InstrumentId` 与外部 listing 的映射；
- `kairospy.data` 中 Dataset Catalog、canonical pipeline 和 metadata；
- `kairospy.storage` 中原子写入、SHA-256、coverage 和 manifest；
- `kairospy.study_platform` 中 snapshot、series capture、追加式 session 和质量问题；
- `kairospy.backtest` 中确定性 feed、point-in-time universe、审计哈希和 replay；
