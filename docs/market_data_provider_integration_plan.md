...
本文定义 Trader 接入外部研究数据源时的供应商选择、目标架构、数据契约、模块边界、实施阶段和验收标准。当前决策是：
...
- `trading.backtest` 中确定性 feed、point-in-time universe、审计哈希和 replay；
...
| Trader Dataset ID | 一个经过版本控制、可追溯、可复现的内部数据资产 |
...
Massive 的 nanosecond 时间、condition code、exchange code、分页字段和可空 snapshot 字段只允许在 adapter/decoder 中解释。Notebook、feature 和 strategy 不得自行处理供应商编码规则。
...
实时接入应在历史链路稳定后复用各自供应商 adapter 和同一个 canonical envelope：
...
现有 `HistoricalFeed.between()` 使用包含结束点的判断。它应与数据湖契约统一为：
...
### Phase 1：Massive Historical Source Adapter
...
| Notebook 直接下载 | lineage、身份和质量约束失效 | 只允许 adapter 下载、Dataset ID 消费 |
...
| Source 不可变与幂等 | `MassiveSourceArchive`、内容 fingerprint、gzip pages、receipt 和 HTTPS cache gate |
...
| OPRA 年度 inventory | `SpxwDayAggPipeline.build_inventory` 以交易日历为预期集合，冻结 date/key/fingerprint/path/bytes/SHA-256 映射并阻断缺日或 hash 冲突 |
| SPXW 日聚合 Curated | `prepare-spxw-day-aggs` 流式过滤 `O:SPXW`，校验 OCC/OHLC/window_start，写月度 ZSTD Parquet 和 0DTE/热门 Call-Put 每日滚动代表序列 |
| 通用期权根日聚合 | `prepare-option-day-aggs --option-root` 复用同一 OPRA inventory，为 NVDA 等 OCC root 生成隔离的月度 Parquet |
| 股票日线与期权 IV | `prepare-massive-equity-day-aggs` 归档/转换调整后股票日线；`prepare-option-day-iv` 为全部日聚合保留求解状态并物化内部 close-based IV |
...
| 研究与回测桥接 | `MassiveCuratedSliceBuilder` 生成现有 `HistoricalDataset/MarketSlice` |
