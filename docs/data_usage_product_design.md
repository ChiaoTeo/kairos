...
`kairospy run backtest/simulate/shadow/paper --strategy ...`、`kairospy study create/start/freeze`、
`kairospy factor register-sma` 等过渡入口。
...
```bash
kairospy data download us-equity-momentum-data

kairospy study open us-equity-momentum
kairospy study add-data --workspace us-equity-momentum --name returns --dataset market.returns.equity.us.1d
kairospy study add-factor --workspace us-equity-momentum --name momentum_12_1 --file factors/momentum_12_1.py
kairospy run start --study us-equity-momentum --mode study
kairospy study freeze us-equity-momentum --version 1.0.0

kairospy strategy open us-equity-momentum-long-only --from-study us-equity-momentum@1.0.0
kairospy strategy bind-factor --workspace us-equity-momentum-long-only --name primary --study-factor momentum_12_1
kairospy strategy freeze us-equity-momentum-long-only --version 1.0.0
kairospy run start --snapshot us-equity-momentum-long-only@1.0.0 --mode backtest
```
...
| 职责 | 用户动作 | 输入 | 产物 |
|---|---|---|---|
| 下载内置/已注册数据 | `kairospy data download <data_key>` | data product key、时间范围、质量目标 | Historical Release、quality report、download report |
| 写入外部数据到契约 | `kairospy data write ... --as <dataset_id> --contract <contract>` | 文件、批处理目录、用户 connector 或实时流 | Data Release 或 Live View、manifest、quality/freshness report |
...
```bash
kairospy data list
kairospy data search equity momentum
kairospy data describe market.returns.equity.us.1d

kairospy data download us-equity-momentum-data
kairospy data register-download --key us-equity-momentum-data --spec us-equity-momentum-data.yaml

kairospy data write --file analyst_estimates.csv --as reference.estimates.equity.us --contract estimates.contract.yaml
kairospy data write --live --connector user_data_sources/realtime_quotes.py --as market.quotes.equity.us --contract quotes.contract.yaml

kairospy data quality market.returns.equity.us.1d
kairospy data compare returns_a returns_b
```
...
`kairospy data download` 的主参数应该是 data product key，而不是 YAML 文件路径：
...
| 设计概念 | 当前实现 |
|---|---|
| Source Transport | `kairospy.connectors.binance.market_stream`、`kairospy.connectors.massive.websocket` |
...
| Data CLI | `kairospy data live-binance`、`kairospy data soak-binance`、`kairospy data write --live --connector ...` |
...
kairospy study open us-equity-momentum
kairospy study add-data --workspace us-equity-momentum --name returns --dataset market.returns.equity.us.1d
kairospy study add-data --workspace us-equity-momentum --name universe --dataset market.universe.equity.us.1d
kairospy study add-data --workspace us-equity-momentum --name estimates --dataset reference.estimates.equity.us
kairospy study add-factor --workspace us-equity-momentum --name momentum_12_1 \
  --file factors/momentum_12_1.py \
  --metadata factors/momentum_12_1.metadata.yaml
kairospy study inspect us-equity-momentum
kairospy study freeze us-equity-momentum --version 1.0.0
```
...
```python
data_ref = data.dataset("market.returns.equity.us.1d")
study_data_ref = study.data("us-equity-momentum", "returns", version="1.0.0")
study_factor_ref = study.factor("us-equity-momentum", "momentum_12_1", version="1.0.0")
rows = study_data_ref.rows(columns=("decision_date", "instrument_id", "return_1d"))
run_manifest = run.start("us-equity-momentum-long-only@1.0.0", mode="backtest", execute_strategy=True)
```

这些入口返回冻结产物引用和 evidence；Data/Study data ref 已支持最小 `rows()` / `pandas()` / `arrow()`
materialization。Strategy `model.py:decide(context)` 在 backtest / historical-simulation 下已能通过
`context.input(name)` / `context.data(name)` 消费 Strategy Lock 中声明的 Data input，并把 decision artifact 写入
Run Workspace。已发布为 Feature Data Release 的 Factor input 也会以 `InputTableRef` 暴露；未发布的
Candidate/Frozen Factor 仍是 contract-only，不能被 `context.input(...).rows()` 直接读取。
...
当前仓库的 `kairospy.study_platform.open_study`、`StudySession.data` 和 `kairospy study create/start/freeze`
...
kairospy strategy open us-equity-momentum-long-only --from-study us-equity-momentum@1.0.0
kairospy strategy bind-factor --workspace us-equity-momentum-long-only --name primary --study-factor momentum_12_1
kairospy strategy set-model-code us-equity-momentum-long-only model.py --metadata model.metadata.yaml
kairospy strategy set-risk us-equity-momentum-long-only risk.yaml
kairospy strategy set-execution us-equity-momentum-long-only execution.yaml
kairospy strategy freeze us-equity-momentum-long-only --version 1.0.0
kairospy strategy inspect us-equity-momentum-long-only
kairospy strategy promote --snapshot us-equity-momentum-long-only@1.0.0 --to paper
```
...
当前 `set-model-code` 支持独立 JSON/YAML metadata 文件。最小 model metadata：
...
inputs:
  ...
  - universe
intent_schema:
  kind: target_exposure
  fields: [instrument_id, target_weight]
side_effects_allowed: false
```

绑定时只校验 model metadata 的输入边界并记录 hash，不执行 `model.py`。

当前 `set-risk` / `set-execution` 支持独立 JSON/YAML policy 文件。最小 execution policy：
...
当前仓库已有 `StrategySpec`、Strategy Registry 和 `kairospy strategy register-*/inspect/status/promote`
...
- Factor 输入的 `source_hash` 是 Study Lock 中记录的 factor code hash；带 metadata 的 Factor 还必须记录并校验
  `factor_contract_hash` 和 `parameters_hash`；
- 已通过 `study publish-factor` 发布为 Feature Data Release 的 Factor 输入，会在 Strategy input 中额外记录
  `materialization_status: published_feature`、`dataset`、`release_id`、`content_hash`、`manifest_hash` 和
  `factor_run_hash`，并可由 `context.input(name).rows()` 读取；
- 未发布的 Factor 输入保持 `materialization_status: contract_only`，可被 Strategy Lock 引用和审计，但不能直接
  materialize 为 InputTable；
- `strategy_eligible: false` 的 Candidate/Frozen Factor 不能绑定为 Strategy input；
...
kairospy run start --study us-equity-momentum --mode study
kairospy run start --snapshot us-equity-momentum-long-only@1.0.0 --mode backtest
kairospy run inspect --run-id run_...
kairospy run replay --run-id run_...
kairospy run compare --first run_... --second run_...
```
...
kairospy study add-factor --workspace us-equity-momentum --name momentum_12_1 \
  --file factors/momentum_12_1.py \
  --metadata factors/momentum_12_1.metadata.yaml
kairospy study factor-run us-equity-momentum momentum_12_1
kairospy study factor-inspect us-equity-momentum momentum_12_1
kairospy study publish-factor us-equity-momentum momentum_12_1 --as features.momentum.equity.us.1d
```
...
当前仓库尚未提供完整 `kairospy.study.factor` decorator；在 decorator 落地前，最小无装饰器协议和
...
`kairospy study add-factor --metadata <file>` 会校验 `inputs` 必须来自当前 Study 已声明的数据 alias、`parameters`
...
当前 `kairospy study factor-run <study_id> <factor_name>` 会执行无装饰器 `compute(inputs, params, context)` 协议；
...
当前 `kairospy study publish-factor <study_id> <factor_name> --as <dataset_id>` 只发布最新通过的 factor-run rows；
...
- `kairospy data download <data_key>`：已支持 credential-free 的 `tutorial-sma-data`，走 Data Catalog 和 Release 产物；
- `kairospy data register-provider --name <provider_name> --spec <spec.json|yaml>`：已支持最小 provider catalog，
  download spec 可通过 `source.provider` 复用 provider 定义；
- `kairospy data register-download --key <data_key> --spec <spec.json|yaml>`：已支持用户注册本地 `local_csv`
...
- `kairospy data write --file ... --as <dataset_id> --contract <contract>`：已支持 CSV 时间序列按 Data Contract 写入 Release；
- `kairospy data write`、注册本地 CSV download 和 `study publish-factor` 已写出最小 `data.quality_report`：
  区分 non-empty、required fields、primary time 等 gate，以及 missing values、duplicate primary time 等 diagnostic；
- `kairospy data write --live --connector ... --as <dataset_id> --contract <contract>`：已支持校验
  `freshness.max_age_seconds` 并生成带 `live_data_plane` 证据的 Live View manifest；
- `kairospy data live-binance/soak-binance`、`BoundedEventChannel`、`AsyncKairosRuntime`：已有 provider WebSocket 到 canonical async channel 的运行基线；
- `kairospy study open/add-data/add-factor/inspect/freeze`：已支持一个 Study workspace 绑定多个 Data Release 和 factor code hash；
- `kairospy study freeze` 已写出最小 Study readiness gate：检查 Study 至少绑定一个 Data Release、Data quality 达到 Q2
  Study 门槛，并把 factor metadata 缺失作为 diagnostic 写入 Study Lock 和 `readiness.json`；
- `kairospy study add-factor --metadata <metadata.json|yaml>`：已支持外部 Factor metadata contract；metadata 会校验
  Study 输入 alias、参数 JSON 序列化、输出字段、`primary_time`、`point_in_time` 和依赖声明，并在 Study Lock /
  Strategy input 中记录 `factor_contract_hash` 与 `parameters_hash`；
- `kairospy study factor-run <study_id> <factor_name>` 已提供最小 Candidate Factor run：执行
...
- `kairospy study publish-factor <study_id> <factor_name> --as <dataset_id>` 已可把最新通过的 factor-run rows
...
- `kairospy strategy open/bind-factor/set-risk/inspect/freeze`：已支持从 Frozen Study 创建 Strategy workspace，并复用 Study factor hash；
- Strategy bind/freeze 已开始检查 Factor metadata 语义边界：显式 `strategy_eligible: false` 的 Factor 不能绑定到
  ...
  必须匹配 Frozen Study Lock；
- `kairospy strategy set-risk <strategy_id> <risk.yaml>` / `kairospy strategy set-execution <strategy_id> <execution.yaml>`
  已把 risk/execution policy 作为 Strategy contract 读入 workspace，并在 Strategy Lock 中记录
  `risk_policy_hash` / `execution_policy_hash`；execution policy 需要声明 `decision_time`、`execution_time`
  和 `order_style`；
- `kairospy strategy set-model-code <strategy_id> model.py --metadata <model.metadata.yaml>` 已把用户区 `model.py`
...
- `kairospy run start --snapshot <strategy@version> --mode backtest --execute-strategy` 已支持用户
...
- `kairospy run start/inspect/replay/compare`：已支持从 Study 或 Strategy snapshot 创建 Run Workspace 和 Run Manifest；
...
- `kairospy.data.contracts` 已提供最小正式 artifact 模型：`DataSetContractArtifact`、`DataReleaseManifest` 和
...
- `kairospy data soak-binance --live-view-manifest ...` 已可把审计后的 soak 结果写回 Live View manifest，自动更新
...
- paper/live Strategy target Run Manifest 已记录 `runtime_contract.strategy_runtime_plan`，把 Strategy Lock target
  转成 strategy service plan 和 `plan_hash`；`kairospy strategy set-model --kind sma-cross-v1 ...` /
...
11. `data.dataset(name)`、`study.data(name)`、`study.factor(name)` 和 `run.start(...)` 的最小引用/启动 API
    ...
    `strategy.decide(context)` runtime 还没有完成；
12. Factor Code 外部 metadata/hash 协议已最小落地；decorator API、探索产物晋级和 Factor runtime profile 还没有完成；
...
- 增加 `kairospy data download <data_key>`；
- 增加 `kairospy data register-download --key <data_key> --spec <spec.yaml>`；
- 增加 `kairospy data write --file ... --as <dataset_id> --contract <contract>`；
- 增加 `kairospy data write --live --connector ... --as <dataset_id> --contract <contract>`；
...
- 增加 `kairospy strategy open --from-study`；
...
- 增加 `kairospy run start --snapshot <strategy_id>@<version> --mode <mode>`；
- 增加 `kairospy run start --study <study_id> --mode study`；
- 增加 `kairospy run inspect/replay/compare`；
- 生成独立 Run Workspace；
...
source:
  kind: local_csv
  path: custom-bars.csv
contract: custom-bars.contract.yaml
quality:
  minimum: Q2
```

注册只保存 spec 和 spec 来源路径；`data download custom-bars` 执行发布时才读取 `local_csv` 和 contract，
并生成正式 Data Release 证据。
...
```bash
kairospy data register-provider --name massive-provider --spec massive-provider.yaml
```
...
products:
  - dataset_id: reference.sentiment.provider
    ...
    contract: sentiment.contract.yaml
```
...
kairospy study add-data --workspace us-equity-momentum --name returns --dataset market.returns.equity.us.1d
kairospy study add-data --workspace us-equity-momentum --name universe --dataset market.universe.equity.us.1d
kairospy study add-data --workspace us-equity-momentum --name liquidity --dataset features.liquidity.equity.us.1d
kairospy study add-data --workspace us-equity-momentum --name estimates --dataset reference.estimates.equity.us
```
...
| 缺口 | 系统提示 | 用户下一步 |
|---|---|---|
| 缺历史 Data Release | `missing_history_release` | 运行 `kairospy data download` 或补 Data Source |
...
`kairospy study publish-factor` 后 refreeze Study/Strategy。
