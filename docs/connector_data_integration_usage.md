...
输出字段应描述 Data Product 和默认 Dataset，例如 `key`、`title`、`data_kind`、`provider`、`venue`、`default_dataset_name`、`aliases`。这里的 `data_kind` 是用户可理解的数据产品类型，不是 connector capability。
...
当前 `kairospy/integrations/connectors` 已经按 provider 分组，例如 `binance`、`massive`、`deribit`、`ibkr`。这符合用户心智，但代码里仍存在一个关键混淆：`connector` 同时表示“如何接入外部系统”和“如何把外部数据整理成 Data Product”。
...
| 名称 | 所属层 | 含义 |
|---|---|---|
| `ProviderConnector` | `integrations/connectors/` | 外部系统接入入口，用户文档中通常简称为 Provider |
| `ProviderService` | `integrations/connectors/` | provider 下的一组 service，例如 historical market data、live market data、execution、account、reference |
| `ProviderResource` | `integrations/connectors/` | 外部可访问 resource，例如 aggregate bars、klines、depth stream、order endpoint |
| `Transport` | `integrations/connectors/` | HTTP、WebSocket、FIX、gRPC、native process、shared memory 等传输实现 |
| `ProviderCodec` | `integrations/connectors/` | provider raw payload 与 provider-neutral source artifact 的编码/解码 |
...
```text
Data / owner product contract 稳定
Provider 接入可替换
...
kairos_v2 里一个 venue/provider 可以组合 market request、market stream、trading、user state。当前 `integrations/connectors/` 也应采用这个模型：
...
```text
integrations/connectors ProviderResource / Transport / ProviderCodec
  负责 provider 协议

integrations/connectors ProviderService
...
这些应进入 `integrations/connectors/` 的 service contract，不应进入 historical Data acquisition 的临时逻辑。
...
`ProviderConnector` 是外部接入层，位于 `kairospy/integrations/connectors`。它的职责是让 KairoSpy 可以可靠地接入外部 provider 或 venue。

### 6.1 职责

`ProviderConnector` 负责：

- provider identity、service/resource spec 和最小 support metadata。
...
现货、USD-M 合约、期权不应导致 provider connector 顶层分裂。它们是 service/resource 参数、support metadata 和 runtime binding 的差异，不是一个全局 capability graph。

## 实现边界：数据结构归属

`integrations/connectors` 应该有自己的数据结构，但不能把所有结构都定义在 `integrations/connectors` 里。推荐使用三层数据结构：
...
| 类型 | 归属 | 是否对外稳定 | 典型例子 |
|---|---|---|---|
| Provider DTO | `integrations/connectors/{provider}` | 否 | `MassiveAggregateBarPayload`、`BinanceKlinePayload`、`BinanceOrderResponse` |
| Provider Boundary Model | `integrations/connectors/artifacts.py` 等共享模块 | 是 | `SourceArtifact`、`ProviderEvent`、`ProviderHealth`、`ProviderEstimate` |
| Owner Product Model | `identity/`、`reference/`、`market/`、`execution/`、`portfolio/`、`integrations/ports/` | 是 | `InstrumentId`、`OrderRequest`、`OrderAck`、`ExecutionReport`、`Position` |
...
Provider DTO 可以贴近外部 API，不要求稳定，也不要求对 Data 或 owner 产品模型友好。它们允许保留 provider 特有字段、命名、枚举和异常语义。
...
### 7.3 Owner Product Model

涉及核心交易语义时，connector service boundary 应接收或返回 owner 产品模型和 `integrations/ports` 层结构。
...
这里 `OrderRequest`、`OrderAck`、`ExecutionReport` 属于 execution owner 的核心交易语义，应保持 provider-neutral。Binance/Massive/IBKR 原始请求和响应仍然属于 provider DTO，只存在于 connector 内部。

Reference 和 Account 也类似：

- instrument identity、venue id、account key 应使用 `identity/`、`reference/` 和 `integrations/ports` 层结构。
...
- 外部 API 原样字段、provider enum、provider 错误码：放在 `integrations/connectors/{provider}`。
- provider-neutral 接入证据、source artifact、health、estimate、sequence：放在 `integrations/connectors/artifacts.py`、`integrations/connectors/health.py` 等共享模块。
- 订单、成交、仓位、账户、instrument identity 等核心交易语义：放在 `identity/`、`reference/`、`execution/`、`portfolio/` 和 `integrations/ports/`。
- dataset contract、dataset identity、build record、snapshot、coverage、quality、publication：放在 `data`。

因此，`integrations/connectors` 既应该提供自己的数据结构，也应该在 service boundary 使用 owner 产品模型、Data model 或 provider boundary model。关键是不让 provider DTO 泄漏到 Data、Execution、Portfolio、Market、Reference 或 Strategy。
...
- 生成 lineage、collection、data product metadata。
...
第一版 `ExecutionService` 已对齐当前 `integrations/ports.ExecutionPort` / `OrderRecoveryPort` 的稳定形状：

- `place_order`：单腿订单提交，输入 KairoSpy `execution/` + `integrations/ports` 的 `OrderRequest`，输出 `OrderAck`。
- `place_combo_order`：组合订单提交，只在 execution port contract 和 provider service spec 明确支持时启用。
...
- `environment`、`service_spec`、`institution_id`、`venue_id`：声明交易环境、provider service 静态描述和稳定身份。
...
- `replace`：改单能力需要根据 execution port contract、provider service spec 和 readiness evidence 暴露。
...
当前不恢复顶层 `kairospy/connectors`。建议在 `kairospy/integrations/connectors` 内新增少量共享模块，并逐步让 provider 包变薄。不要新增过大的 `core/` 包，否则容易变成所有内部对象的堆放处。
...
现有 `integrations/connectors/*/datasets.py` 是历史 dataset wrapper 来源；最终目标中新增实现应优先使用 provider service + DataProductBuilder。
...
```text
integrations/connectors/massive/
  MassiveProviderConnector
  MassiveHistoricalMarketDataService
...
- 已新增 `integrations/connectors/provider_contracts.py`、`integrations/connectors/artifacts.py`，承载 ProviderConnector、ProviderService、ProviderResource、SourceArtifact 等基础边界。
- 已补齐 `integrations/connectors/services.py`、`integrations/connectors/resources.py`、`integrations/connectors/transports.py`、`integrations/connectors/codecs.py`，为 service/resource/transport/codec 拆分提供稳定薄边界。
...
- 已新增 `integrations/connectors/execution.py`，定义 `ExecutionService`、`ComboExecutionService` 和 `ExecutionServiceSpec`。
- 第一版 `ExecutionService` 对齐当前 `integrations/ports.ExecutionPort` / `OrderRecoveryPort`，使用 `execution/` + `integrations/ports` 的 `OrderRequest`、`ComboOrderRequest`、`OrderAck`、`VenueOrderRecovery`，不暴露 provider DTO。
...
- 已新增 `integrations/connectors/data_planes.py`，定义 `DataPlaneEndpoint`、`ProviderDataPlaneSpec`、`ProviderDataPlane`。
