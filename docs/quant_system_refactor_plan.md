...
| `runtime/` | 新增一级产品域 | `runtime/` | 交付 RunKernel、RunProfile、RunRequest、RunResult、RuntimeRunLauncher、LiveRuntimeBindingConfig、LiveRunDaemon、LiveRunKernelService、run lifecycle、live run lifecycle、artifact ref/hash |
...
- `runtime/live_config.py` 已提供 `LiveRuntimeBindingConfig`，并从 `RunConfig [live]`、`[bindings]`、`[evidence]` 解析 live readiness、promotion、account binding、recovery binding，再转成 `BoundRunProfile` evidence；它不读取 project-level `[runtime.live]`，不保存 credential，也不定义 connector capability model。
...
- `runtime/live_daemon.py` 已提供 `LiveRunDaemon`、`LiveRunDaemonPhase`、`LiveRunDaemonSnapshot` 和 `LiveRunKernelService`，把长驻 live run 的 start/status/stop/recover/critical-fault 语义落在 Runtime owner：每个实例必须有显式 `run_id`，daemon runtime state key 固定为 `live_run_daemon:<run_id>`；`LiveRunKernelService` 作为被 daemon 监督的 managed service 调用 `RunKernel.run(...)` 并把 `RunResult`、artifact hash/ref 写到 `live_run_kernel:<run_id>`。daemon 只管理 `KairosApplication` gates、`AsyncServiceSupervisor` 服务生命周期和 runtime store evidence；不调用 connector discovery、不持有 credential、不直接拥有策略、不写 governance artifact repository。
...
- 顶层 package export 不能通过 eager import 拉起下游 owner 或具体 connector。`integrations/__init__.py` 只稳定导出事件对象，并对 live port 入口做 lazy export；`infrastructure/configuration.py` 只负责项目配置发现、TOML 读取、路径和通用 `env:` 解析，provider/account credential 解析由 `integrations/config.py` 承担。这个规则防止基础设施、集成和运行时包在 import 阶段形成循环依赖，也让 owner 边界可被 architecture boundary tests 验证。
- `run start --config <RunConfig>` 已收口为 reusable RunConfig 启动入口。live 启动要求 `--confirm-live`、`execution.live_trading_enabled = true`、RunConfig evidence refs 和 live guards，然后走 `RuntimeRunLauncher`；当 RunConfig `[live] bind_provider = true` 时，surface 加载 reference catalog、通过 `integrations/live_ports.py` 构建 live ports，再交给 `LiveRuntimeComponents` 注入 durable outbox、account lock 和 recovery evidence；当 RunConfig `[bindings.live_views.*]` 存在时，surface 从 Data Product Live View 构建 provider runtime feed channel，并把它作为 `market_event_source` 注入 `LiveRuntimeComponents`。同步 `run start` 默认仍只记录 feed service bundle 的 created evidence，避免短生命周期 surface 命令隐式变成长驻真实行情进程；显式传入 `--supervise-live-services` 或 RunConfig live view binding `supervise_services = true` 时，surface 才会把 `market_source.managed_services` 交给 `RuntimeRunLauncher(managed_services=...)`，由 launcher 管理启停并在治理 artifact 写入前记录 stopped evidence。长驻语义由 `run live <start|recover|status|stop> --run-id <id> --config <RunConfig>` 显式暴露为用户可见的 live run surface；每个 live `run_id` 使用独立 runtime root、runtime database、stop request、service snapshot evidence 和 `LiveRunKernelService` strategy/artifact evidence。
...
- `tests/test_execution_state_contract.py` 已把 execution 状态机 contract 写成可回归测试：`execution/order_state.py` 必须覆盖所有 `DurableOrderStatus`，terminal 状态不能回退；`runtime/store/runtime_store.py` 的非法状态转移必须被 `require_order_transition()` 拦住，且 OrderView/IntentView 只能投影已提交的 durable order/outbox 事实，不能绕过 execution owner 自己生成新状态。
...
- `tests/test_connector_capability_boundary.py` 把这个边界写成回归测试：禁止在代码中重新引入 `ConnectorCapabilityModel`、`PortCapability` 或 capability graph；允许 Data Product、Market、Execution、Reference owner 内保留各自 support model；同时验证 `Context` 仍只暴露七个 View，不暴露 connector/capability 视图。
...
kairospy data ...
kairospy providers ...
kairospy run config validate configs/runs/backtest.toml
kairospy run config explain configs/runs/backtest.toml
kairospy run start --config configs/runs/backtest.toml
kairospy run start --config configs/runs/live.toml --confirm-live
kairospy run live status --run-id <run-id>
kairospy run live start --run-id <run-id> --config configs/runs/live.toml --confirm-live
```
...
具体 BacktestProfile / SimulationProfile / LiveProfile 的最小 adapter 已接到这个 contract 上；artifact writer 已通过 runtime protocol + governance adapter 绑定；runtime binding 已通过 `BoundRunProfile`、`IterableRunEventProvider`、`RunCommandSubmitterBinding`、`RuntimeRecoveryBinding` 接入；`EventSourceRunEventProvider`、`ExecutionPortCommandSubmitter`、`DurableOutboxCommandSubmitter`、`ExecutionRecoveryBinding`、`CompositeRecoveryBinding` 和 `ManagedServiceEvidenceProvider` 已把具体 event source、execution port、durable outbox、order recovery、live recovery 组合和长生命周期 service evidence 接到 binding adapter；`RuntimeRunLauncher` 已把 application startup gate、service evidence、可选 `managed_services` supervisor lifecycle 和 artifact writer factory 接入 run 启动用例，managed services 会在 artifact 写入前停止并刷新 final evidence；`runtime/run_config.py` 已提供 `RunConfig`、`RunConfigResolver` 和 `RunConfigValidationReport`；`runtime/live_config.py` 已把 RunConfig 的 live readiness/promotion/account/recovery refs 转成 `BoundRunProfile`；`runtime/run_instance.py` 已提供 `RunInstance` 和 `RunManifestBuilder`，`run start --config` 会冻结 project config hash、RunConfig hash、workspace snapshot hash、strategy hash、params hash、binding hash 和 guard evidence refs；`runtime/live_binding.py` 已把 live 运行组件组合成 market event provider、durable outbox command submitter 和 recovery chain；`runtime/live_daemon.py` 已把长驻 live run 的 start/status/stop/recover/critical-fault evidence 固定到 `run_id` 维度的 runtime store；`LiveRunKernelService` 已把 live run 内的策略调度收口成被 daemon 监督的 managed service；`integrations/live_ports.py` 已把显式 RunConfig provider binding 转成 live execution/account/order-recovery port 实例，并把 Data Product Live View / provider runtime feed 转成 live market EventSource channel；`run live <start|recover|status|stop> --run-id <id> --config <RunConfig>` 已把长驻 live run 暴露为独立 surface，并按 `run_id` 隔离 runtime root、runtime database、state key、stop request、service evidence 和 strategy/artifact evidence；`GovernedStrategyRunLoop` 已通过测试证明 profile strategy loop 能产出可重建的 final Context view hashes，并由 governance artifact 持久化为 cross-view evidence refs；BacktestProfile / SimulationProfile / LiveProfile 已通过 profile-specific reconstruction tests 证明同一 artifact explain contract 可以跨模式还原 `Context` evidence。下一步不是扩张 connector capability model，而是把真实 live profile runner 的订单、组合、ledger 因果链继续接入既有 artifact evidence。
...
- `LiveRunDaemon` 只负责长驻 live run 生命周期：每个实例由显式 `run_id` 标识；start/recover 会重新创建 `AsyncServiceSupervisor` 并通过 `KairosApplication` gates 进入 running，status/stop 会把 service snapshots、application status 和 stop request 写入实例专属 runtime store，critical managed service fault 会把 application 降级到 reduce-only 并持久化 live run evidence。策略调度不进入 daemon 内部，而是通过 `LiveRunKernelService.managed_service()` 作为受监督服务调用 `RunKernel` 并产出 artifact evidence。
...
### 13.1.1 Live Run 多实例边界

- `run_id` 是唯一的 Run 身份，live 模式不创建第二个身份字段。Backtest、Simulation、Paper、Live 都使用同一个字段，mode 决定它是一次离线运行、一次模拟运行，还是一个长驻 live run。
- live 模式下，`run_id` 同时代表可 start/recover/status/stop 的长驻实盘实例，负责 Data Product Live View / provider runtime feed 的服务生命周期、runtime store evidence、stop request 和故障状态。这样用户只需要理解“多个 live run = 多个 run_id”。
- `run start --config configs/runs/live.toml` 是一次 live profile run 的启动用例，默认不隐式变成长驻行情/交易进程；只有显式 `--supervise-live-services` 或 RunConfig `[live] supervise_services = true` 时，才把本次 run 的 market service bundle 纳入 launcher 生命周期。
- `run live <start|recover|status|stop> --run-id <id>` 是长驻 live run 控制面。`--run-id` 必填，不存在默认 singleton；每个 live run 使用独立 runtime root、runtime database、state key `live_run_daemon:<run_id>`、stop request 和 service snapshot evidence。
- 同一项目可以同时运行多个 live run。不同 `run_id` 之间不能共享 in-memory supervisor、runtime store 或 outbox 状态；如果多个 run 绑定同一 account/venue/instrument，冲突必须由 account lock、readiness/promotion gate 或后续 operator policy 显式处理，而不能靠隐藏全局变量解决。
- Connector 只在 `integrations/live_ports.py` 被显式 provider binding 构造，并通过 `runtime/live_binding.py` 注入 run binding。`run live` 不引入 connector capability model，也不把 provider DTO 暴露给 `Context`。
- `tests/test_live_run_run_id_contract.py` 已把这个边界写成回归测试：代码、文档和测试中不能重新引入 live-specific run identity 字段；`run live` 缺少 `--run-id` 必须被 CLI 拒绝；直接调用 `run_live` 时缺失、空白、前后空白或 path-like `run_id` 也必须被 product surface 拒绝；`run_live(start/recover)` 在装配 live service 前必须先要求 `--confirm-live` 和 `execution.live_trading_enabled = true`；`run_live(status)` 必须按不同 `run_id` 返回不同 runtime database 和 state key，且 missing run 的 status 不能创建 runtime database；`run_live(stop)` 只能写入指定 `run_id` 的 stop request，live 多实例只能通过不同 `run_id` 表达，不能通过路径逃逸绕开 runtime root。
...
3. `RunKernel` / `RunProfile` contract、三种最小 profile adapter、artifact writer boundary、runtime binding boundary、event source binding、execution port command submitter、durable outbox command submitter、execution recovery binding、composite live recovery binding、长生命周期 service evidence、RuntimeRunLauncher 启动用例、managed service supervisor lifecycle、artifact evidence factory、profile-specific Context reconstruction tests、`LiveRunDaemon` 长驻 live run contract、`LiveRunKernelService` strategy/artifact managed service contract、`run start --config` RunConfig surface 接入、RunConfig `[live]` / `[bindings]` / `[evidence]` 配置证据读取、`LiveRuntimeComponents` live 组件装配、`integrations/live_ports.py` provider port / market EventSource factory、RunConfig account binding 到 `LiveRuntimeComponents` 的 live execution/account/order-recovery 注入、RunConfig market binding 到 `LiveRuntimeComponents` 的 Data Product live view market source 注入、live 启动门禁接入、显式 market service supervision、`run live <start|recover|status|stop> --run-id <id>` 多实例控制面、per-run runtime state 隔离和 `run_id` 唯一身份命名合同已落地；下一步是把真实 live profile runner 的完整订单、组合、ledger 因果链继续接入既有 artifact evidence。
...
6. ExecutionEvent / Outbox / OrderView / IntentView 归属已固定在 `execution/` 与 `strategy/views.py` 的只读投影边界；`tests/test_execution_state_contract.py` 已补充状态转移 contract test，证明状态图由 `execution/order_state.py` 拥有，runtime store 只能调用 `require_order_transition()` 并投影已提交 durable facts，不能让 execution state machine 与 runtime coordinator 混成一个产品。
