...
./pyenv/bin/python -m trading --lang en-US tutorial sma
```
...
./pyenv/bin/python -m trading run inspect --artifact "$RUN_ARTIFACT"
./pyenv/bin/python -m trading run replay-sma --artifact "$RUN_ARTIFACT" --fixture
```
...
```text
trader study validate
trader study inspect
trader study conclude --decision accept|reject
```
