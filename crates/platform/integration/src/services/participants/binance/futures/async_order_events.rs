//! Async Binance Futures private order/fill channel.
...
pub(crate) struct BinanceFuturesAsyncOrderEventSource {
    binding_id: String,
    channel_id: String,
...
impl BinanceFuturesAsyncOrderEventSource {
    pub(crate) fn new(
        ...
        if binding_id.trim().is_empty() {
            return Err(IntegrationError::InvalidRequest(
                "Binance Futures binding id is required".into(),
            ));
        }
        ...
        if !(websocket_endpoint.starts_with("wss://") || websocket_endpoint.starts_with("ws://")) {
            return Err(IntegrationError::InvalidRequest(
                "Binance Futures user stream endpoint must start with ws:// or wss://".into(),
            ));
        }
        if event_queue_capacity == 0 {
            return Err(IntegrationError::InvalidRequest(
                "Binance Futures event queue capacity must be positive".into(),
            ));
        }
        Ok(Self {
...
impl AsyncOrderEventSource for BinanceFuturesAsyncOrderEventSource {
    async fn connect_channel(&mut self) -> Result<(), IntegrationError> {
        ...
        .await
        {
            ...
            Err(_) => {
                let error = IntegrationError::Unavailable(
                    "timed out creating Binance Futures listen key".into(),
                );
                self.record_failure(&error);
                return Err(error);
            }
        ...
        .await
        {
            ...
            Err(_) => {
                let error = IntegrationError::Unavailable(
                    "timed out connecting Binance Futures user stream".into(),
                );
                self.record_failure(&error);
                Err(error)
            }
        ...
        loop {
                    ...
                    Err(_) => {
                        ...
                        if let Err(error) = self.client.keepalive_listen_key_async(listen_key).await
                        {
                            let error = IntegrationError::ResyncRequired(format!(
                                "Binance Futures listen-key keepalive failed: {error}"
                            ));
                            self.record_failure(&error);
                            return Err(error);
                        }
            ...
            let text = match event {
                ...
                AsyncSocketEvent::Message(tokio_tungstenite::tungstenite::Message::Close(
                    ...
                    let error = IntegrationError::ResyncRequired(format!(
                        "Binance Futures user stream closed: {frame:?}"
                    ));
                    self.record_failure(&error);
                    return Err(error);
                }
                ...
                AsyncSocketEvent::Backpressure => {
                    let error = IntegrationError::Backpressure(
                        "Binance Futures order event queue overflowed; reconciliation is required"
                            .into(),
                    );
                    self.record_failure(&error);
                    return Err(error);
...
pub(super) fn parse_order_trade_update(
    ...
    match value.get("e").and_then(Value::as_str) {
        Some("listenKeyExpired") => {
            return Err(IntegrationError::ResyncRequired(
                "Binance Futures listen key expired".into(),
            ));
        }
    ...
    let row = value.get("o").ok_or_else(|| {
        IntegrationError::InvalidPayload("Binance Futures order update is missing o".into())
    })?;
        ...
        row.get(field)
            ...
            .ok_or_else(|| {
                IntegrationError::InvalidPayload(format!(
                    "Binance Futures order update is missing {field}"
                ))
            })
    };
    let event_time_millis = value.get("E").and_then(Value::as_u64).unwrap_or_default();
    let order_id = row.get("i").map(value_string).unwrap_or_default();
    ...
    let provider_event_id = Some(format!(
        "binance-futures:{order_id}:{}:{execution_type}:{status_text}:{event_time_millis}",
    ...
    Ok(Some(ExternalEventEnvelope {
        ...
        payload: ExternalExecutionEvent {
            ...
            execution_id: trade_id
                .map(|value| FillId::new(format!("binance-futures:{value}")))
                .transpose()
                .map_err(|error| IntegrationError::InvalidPayload(error.to_string()))?,
            fee_currency: row
                .get("N")
            ...
            occurred_at_unix_nanos: UnixNanos::from(event_time_millis.saturating_mul(1_000_000)),
            reason: row
                .get("r")
                .and_then(Value::as_str)
                .unwrap_or_default()
...
fn status(value: &str) -> OrderStatus {
    match value {
        "NEW" | "PENDING_NEW" => OrderStatus::Acknowledged,
...
fn order_type(value: Option<&str>) -> Option<OrderType> {
    match value {
        ...
        Some("STOP" | "TAKE_PROFIT") => Some(OrderType::StopLimit),
        Some("STOP_MARKET" | "TAKE_PROFIT_MARKET" | "TRAILING_STOP_MARKET") => {
            Some(OrderType::Stop)
        }
        _ => None,
    }
...
mod tests {
    ...
    use super::{parse_order_trade_update, BinanceFuturesAsyncOrderEventSource};

    #[test]
    fn normalizes_futures_order_trade_update_with_stable_identity() {
        let event = parse_order_trade_update(
            "execution.binance.usd-m.primary",
            "orders",
            2,
            UnixNanos::from(2_000_000_000),
            r#"{"e":"ORDER_TRADE_UPDATE","E":1000,"o":{"s":"BTCUSDT","c":"order-1","S":"BUY","o":"LIMIT","q":"0.25","p":"100","i":42,"t":7,"x":"TRADE","X":"PARTIALLY_FILLED","z":"0.1","l":"0.1","L":"99.5","n":"0.01","N":"USDT","r":"NONE"}}"#,
        )
        .unwrap()
        .unwrap();
        assert_eq!(event.channel_epoch, 2);
        ...
        assert_eq!(
            ...
            Some("binance-futures:42:7:TRADE:PARTIALLY_FILLED:1000")
        );
    }
    ...
    async fn async_source_creates_listen_key_and_receives_order_event() {
        ...
        let http_server = std::thread::spawn(move || {
            ...
            assert!(request.starts_with("POST /fapi/v1/listenKey "));
            assert!(request
                .to_ascii_lowercase()
                .contains("x-mbx-apikey: api-key"));
            let body = r#"{"listenKey":"test-listen-key"}"#;
            write!(
                stream,
        ...
        let ws_server = tokio::spawn(async move {
            ...
            socket
                .send(tokio_tungstenite::tungstenite::Message::Text(
                    r#"{"e":"ORDER_TRADE_UPDATE","E":1000,"o":{"s":"BTCUSDT","c":"order-1","S":"BUY","o":"MARKET","q":"0.25","p":"0","i":42,"t":7,"x":"TRADE","X":"FILLED","z":"0.25","l":"0.25","L":"100.5","n":"0.01","N":"USDT","r":"NONE"}}"#.into(),
                ))
                .await
                .unwrap();
        });
        ...
        .unwrap();
        let mut source = BinanceFuturesAsyncOrderEventSource::new(
            "execution.binance.usdm.test",
            client,
    ...
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
    async fn async_source_reconnects_with_a_new_listen_key_after_expiry() {
        let http_listener = std::net::TcpListener::bind(("127.0.0.1", 0)).unwrap();
        let http_address = http_listener.local_addr().unwrap();
        let http_server = std::thread::spawn(move || {
            for listen_key in ["expired-listen-key", "recovered-listen-key"] {
                let (mut stream, _) = http_listener.accept().unwrap();
                let mut request = [0_u8; 2_048];
                let read = stream.read(&mut request).unwrap();
                let request = String::from_utf8_lossy(&request[..read]);
        ...
        let ws_server = tokio::spawn(async move {
            ...
            socket
                .send(tokio_tungstenite::tungstenite::Message::Text(
                    r#"{"e":"listenKeyExpired","E":1000}"#.into(),
                ))
                .await
                .unwrap();
            ...
            socket
                .send(tokio_tungstenite::tungstenite::Message::Text(
                    r#"{"e":"ORDER_TRADE_UPDATE","E":2000,"o":{"s":"BTCUSDT","c":"order-2","S":"BUY","o":"MARKET","q":"0.5","p":"0","i":43,"t":8,"x":"TRADE","X":"FILLED","z":"0.5","l":"0.5","L":"101.5","n":"0.01","N":"USDT","r":"NONE"}}"#.into(),
                ))
                .await
                .unwrap();
        });
        ...
        let error = source.next_order_event().await.unwrap_err();
        assert!(matches!(
            error,
            crate::application::IntegrationError::ResyncRequired(_)
        ));
        source.reconnect_channel().await.unwrap();
        let event = source.next_order_event().await.unwrap();

        assert_eq!(event.channel_epoch, 2);
        assert_eq!(event.payload.order_id.as_str(), "order-2");
        assert_eq!(event.payload.status, OrderStatus::Filled);
        source.disconnect_channel().await.unwrap();
        ws_server.await.unwrap();
        http_server.join().unwrap();
