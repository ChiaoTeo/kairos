//! Source control use cases and contract mapping.

use kairos_conflux::Context;
use kairos_primitives::reference::{InstrumentId, ReferenceSourceId};
use kairos_reference_contract::{
    ReferenceControlError, ReferenceOptionCoverageResponse, ReferenceProviderStatus,
    ...
    ReferenceSourceScopeResponse, ReferenceSourceStatusResponse,
};
...
impl ReferenceApplication {
    ...
    pub(crate) async fn upsert_source_definition_from_contract(
        ...
        let source_id = ReferenceSourceId::new(binding.source_id())?;
        let desired_state = request.desired_state;
        self.upsert_source_definition_with_connections(
            domain_source_definition(request)?,
        ...
        let result = self
            .change_source_scope(
                ReferenceSourceScopeRequest {
                    source_id: ReferenceSourceId::new("massive-options")
...
mod tests {
    use kairos_reference_contract::{
        MassiveReferenceSource, ReferenceSourceBinding, ReferenceSourceDefinitionRequest,
    ...
    fn source_definition_contract_mapping_preserves_workflow_fields() {
        let definition = domain_source_definition(ReferenceSourceDefinitionRequest {
