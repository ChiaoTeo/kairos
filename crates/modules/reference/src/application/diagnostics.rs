use kairos_reference_contract::{
    ...
    ReferenceProviderProduct, ReferenceSourceDesiredState, ReferenceSourcePhase,
    ReferenceSourceRuntimeStatus, ReferenceSourceSyncPolicy,
};

use crate::application::runtime::ReferenceAppErrorSummary;
...
pub(super) fn publication_backlog_degraded(pending_publication_count: usize) -> bool {
...
fn registered_source_guidance(source: &ReferenceSourceRuntimeStatus) -> (String, String) {
    ...
    if matches!(
        source.provider_product,
        Some(ReferenceProviderProduct::Equity)
    ) && matches!(
        source.sync_policy,
...
fn source_definition_hint(source: &ReferenceSourceRuntimeStatus) -> String {
    ...
    if let Some(product) = source.provider_product {
        fields.push(format!(
            "provider_product={}",
            provider_product_as_str(product)
        ));
    }
...
fn provider_product_as_str(product: ReferenceProviderProduct) -> &'static str {
    match product {
...
mod tests {
    ...
    use kairos_reference_contract::{
        ReferenceCatalogIntegrityStatus, ReferenceProviderProduct, ReferenceSourceDesiredState,
    ...
    fn runtime_diagnostics_explain_registered_scoped_source() {
        ...
        status.provider_id = Some(ProviderId::new("massive").unwrap());
        status.provider_product = Some(ReferenceProviderProduct::Options);
        ...
        assert!(diagnostics[0].message.contains("provider_product=options"));
        assert!(
            diagnostics[0]
                .message
    ...
    fn runtime_diagnostics_explain_registered_credentialed_source_without_binding() {
        ...
        status.provider_id = Some(ProviderId::new("binance").unwrap());
        status.provider_product = Some(ReferenceProviderProduct::Equity);
        ...
        assert!(diagnostics[0].message.contains("provider_product=equity"));
