...
use crate::application::{
    ...
    QuoteObservation, RiskAuthorizationContext, SubmitOrder,
};
...
impl QueuedExecutionIntentPlanner {
    pub fn start<T>(planner: T, capacity: usize) -> Result<Self, String>
    where
        T: ExecutionIntentPlanner + 'static,
    {
        let mut planner: Box<dyn ExecutionIntentPlanner> = Box::new(planner);
...
impl ExecutionIntentPlanner for QueuedExecutionIntentPlanner {
...
impl QueuedExecutionOrderAdmission {
    pub fn start<T>(admission: T, capacity: usize) -> Result<Self, String>
    where
        T: ExecutionOrderAdmission + 'static,
    {
        let mut admission: Box<dyn ExecutionOrderAdmission> = Box::new(admission);
        let (sender, receiver) = std::sync::mpsc::sync_channel(capacity.max(1));
...
impl ExecutionOrderAdmission for QueuedExecutionOrderAdmission {
    fn dependency_watermarks(&self) -> DependencyWatermarks {
        self.watermarks
            .read()
            .map(|value| value.clone())
