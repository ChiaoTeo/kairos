...
use kairos_primitives::{Money, Price, Quantity, SignedQuantity};
...
pub(super) fn validate_reference_rules(
...
pub(super) fn validate_market_price(
    quotes: &[MarketQuote],
...
pub(super) fn validate_pair_constraints(
    intent: &ExecuteStrategyIntent,
    orders: &[SubmitOrder],
    quotes: &[MarketQuote],
) -> Result<(), String> {
...
pub(super) fn validate_quote_provisioning(orders: &[SubmitOrder]) -> Result<(), String> {
    let bid = orders
        .iter()
...
pub(super) fn validate_quote_freshness(
    orders: &[SubmitOrder],
    quotes: &[MarketQuote],
) -> Result<(), String> {
...
pub(super) fn decimal_quantity(value: Quantity) -> Result<Decimal, String> {
    Decimal::try_new(value.mantissa(), u32::from(value.scale()))
        .map_err(|_| "quantity cannot be represented as a decimal".to_string())
}

pub(super) fn decimal_signed_quantity(value: SignedQuantity) -> Result<Decimal, String> {
    Decimal::try_new(value.mantissa(), u32::from(value.scale()))
        .map_err(|_| "signed quantity cannot be represented as a decimal".to_string())
}

pub(super) fn quantity_from_decimal(value: Decimal) -> Result<Quantity, String> {
...
pub(super) fn decimal_price(value: Price) -> Result<Decimal, String> {
    Decimal::try_new(value.mantissa(), u32::from(value.scale()))
        .map_err(|_| "price cannot be represented as a decimal".to_string())
}

pub(super) fn decimal_money(value: Money) -> Result<Decimal, String> {
    Decimal::try_new(value.mantissa(), u32::from(value.scale()))
        .map_err(|_| "money cannot be represented as a decimal".to_string())
}

pub(super) fn money_from_decimal(value: Decimal) -> Result<Money, String> {
    let value = value.normalize();
    if value <= Decimal::ZERO || value.scale() > u32::from(kairos_primitives::MAX_DECIMAL_SCALE) {
...
#[cfg(test)]
pub(super) fn risk_amount(value: Decimal) -> Result<RiskAmount, String> {
    let value = value.normalize();
