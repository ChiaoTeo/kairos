//! Typed mmap dependency projections used by Execution planning and admission.
...
impl DependencyProjectionRuntime {
    pub(super) fn start(
        ...
        if let (Some(snapshot), Some(actor_id)) = (reference_snapshot, reference_actor_id) {
            let state = Arc::clone(&state);
            let stop = Arc::clone(&stop);
            workers.push(std::thread::spawn(move || {
                let mut last_watermark = None;
                while !stop.load(Ordering::Acquire) {
                    let result = read_reference_projection(&snapshot, &actor_id).map(|value| {
...
pub(super) fn read_reference_projection(
    ...
    .map_err(|error| error.to_string())?;
    let decoded = frame.decode().map_err(|error| error.to_string())?;
    let metadata = decoded.metadata();
    if metadata.completeness() != ViewCompleteness::COMPLETE {
        return Err("Reference mmap view is incomplete".into());
    }
    if frame.generation() != metadata.generation() {
        return Err("Reference mmap generation does not match its envelope".into());
    }
    let event_sequence = metadata
        .applied_revision()
        .ok_or_else(|| "Reference mmap view has no event-sequence watermark".to_string())?;
    if frame.envelope_metadata().applied_event_sequence != event_sequence {
        return Err("Reference mmap event sequence does not match its envelope".into());
    }
    let decimal = |value: Option<&kairos_protocol::generated::kairos::common::v_2::Decimal64>| {
        value.map(|value| {
    ...
    let markets = decoded
        ...
        .map(|value| ReferenceMarket {
            ...
            effective_to_unix_nanos: (value.effective_to_unix_nanos() != 0)
                .then(|| value.effective_to_unix_nanos()),
        })
        .collect();
