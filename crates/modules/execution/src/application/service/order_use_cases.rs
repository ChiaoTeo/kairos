...
impl ExecutionApplication {
    ...
    pub fn record_fill(
        ...
        if let Some(account_facts) = self.account_facts.as_mut() {
            let commitment = self
                .actor
                .commitment(next.order_id.as_str())
                .cloned()
                .ok_or_else(|| {
                    ExecutionError::Invalid("fill has no durable order commitment".into())
                })?;
            account_facts
                .publish_fill(&fill, &next, &commitment)
                .map_err(ExecutionError::Invalid)?;
            account_facts
                .publish_order(&next)
                .map_err(ExecutionError::Invalid)?;
        ...
        let (commitment, dependency_watermarks, risk_context) =
            if let Some(admission) = self.order_admission.as_mut() {
                let commitment = admission
                    .validate_order(&request, &active_commitments)
                    .map_err(ExecutionError::Invalid)?;
