...
use super::super::{MarketApplication, MarketError, MarketRpcActor};
use crate::domain::source::{
    FeedDescriptor, MarketFeedId, SourceEpoch, SourceFailureKind, SourceStatus, source_accepts,
    source_supports_selectors,
};
    ...
    fn activate_managed_sources(
        ...
        for market in markets {
            ...
            let mut candidates = self
                ...
                .source_plans
                .values()
                .filter(|plan| source_accepts(&plan.descriptor, &market))
                .cloned()
                .collect::<Vec<_>>();
                ...
                market.data_routes.iter().filter_map(|route| {
                    let provider_ready = self.actor.source_states().any(|source| {
                        source.status == crate::domain::source::SourceStatus::Ready
                            && source_accepts(&source.descriptor, market)
                            && source_supports_selectors(&source.descriptor, &selectors)
