...
pub(crate) struct ReplaySource {
    ...
    completed: bool,
    actor_snapshot: Option<crate::domain::snapshot::MarketSnapshot>,
}
...
impl ReplaySource {
    ...
    pub(crate) fn with_window(
        ...
        Self {
            ...
            paused: false,
            clock: MarketReplayClock::Maximum,
            speed_multiplier: 1,
            completed: false,
            actor_snapshot: None,
        }
    ...
    pub(crate) fn with_checkpoint(
        ...
        if checkpoint.is_file() {
            ...
            if value.actor_snapshot.is_some() {
                ...
                source.actor_snapshot = value.actor_snapshot;
            }
        }
    ...
    fn next_delay(&self) -> std::time::Duration {
        if self.clock == MarketReplayClock::Maximum {
    ...
    async fn persist_checkpoint(&self) -> Result<(), String> {
        ...
        let bytes = serde_json::to_vec_pretty(&Checkpoint {
            ...
            actor_snapshot: self.actor_snapshot.clone(),
        })
        .map_err(|error| error.to_string())?;
...
pub(crate) fn load_actor_checkpoint(
    path: &std::path::Path,
) -> Result<Option<crate::domain::snapshot::MarketSnapshot>, String> {
    if !path.is_file() {
        return Ok(None);
    }
...
async fn run(
    ...
    let mut subscriptions = BTreeMap::<ProviderSubscriptionId, MarketDescriptor>::new();
        ...
        tokio::select! {
            ...
            _ = tokio::time::sleep(source.next_delay()), if !subscriptions.is_empty() && !source.completed && !source.paused => {
                ...
                match acceptance.await {
                    Ok(Ok(snapshot)) => source.actor_snapshot = Some(snapshot),
                    Ok(Err(error)) => {
...
mod tests {
    ...
    async fn checkpoint_restores_actor_watermark_and_next_cursor_from_one_atomic_record() {
        ...
        source.actor_snapshot = Some(actor_snapshot);
        ...
        let snapshot = super::load_actor_checkpoint(&checkpoint).unwrap().unwrap();
        assert_eq!(snapshot.event_sequence.get(), 1);
        assert_eq!(snapshot.latest.len(), 1);
    }
}
