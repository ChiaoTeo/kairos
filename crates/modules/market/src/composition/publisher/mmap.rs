...
pub struct MmapMarketChangePublisher {
    root: PathBuf,
    slot_size: usize,
    actor_id: String,
...
impl MmapMarketChangePublisher {
    pub fn create(
    ...
    pub fn publish(&mut self, change: &MarketChange) -> Result<(), String> {
        if let Some(view) = change.view.as_ref() {
            ...
            if let MarketViewUpdate::Freshness(freshness) = view {
                ...
                let generation = change.sequence.get();
                let bytes =
    ...
    fn publish_orderbook(&mut self, sequence: u64, book: &crate::OrderBook) -> Result<(), String> {
        let key = MarketViewKey::new(
