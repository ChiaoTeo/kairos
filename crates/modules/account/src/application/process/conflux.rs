...
use crate::application::{
    AccountApplication, AccountCurrentView, AccountError, AccountFactProvenance, AccountRpcActor,
...
impl AccountApplication {
    ...
    fn apply_external_envelope(
        ...
        let applied = self.apply_event_with_provenance(
            ...
            AccountFactProvenance {
                source_id: kairos_primitives::integration::IntegrationSourceId::new(format!(
                    "{}:{}",
                    envelope.participant.id, envelope.connection_key
                ))
                .map_err(|error| AccountError::Invalid(error.to_string()))?,
                provider_event_id: envelope.participant_event_id.clone(),
                provider_sequence: envelope.participant_sequence.map(Into::into),
                provider_occurred_at_unix_nanos: Some(envelope.observed_at_unix_nanos),
                provider_received_at_unix_nanos: Some(envelope.received_at_unix_nanos),
            },
        )?;
        let state = self
            .conflux
    ...
    fn enrich_current_view(&self, view: &mut AccountCurrentView) {
        ...
        for segment in &mut view.segments {
            ...
            segment.snapshot_watermark = state.last_snapshot_at_unix_nanos.map(Into::into);
            ...
            segment.channel_epoch = state
                ...
                .max()
                .map(Into::into);
            segment.last_event_at_unix_nanos = state.last_event_at_unix_nanos.map(Into::into);
            segment.last_success_at_unix_nanos = state.last_success_at_unix_nanos.map(Into::into);
