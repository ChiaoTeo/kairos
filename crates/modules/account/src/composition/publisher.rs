...
use kairos_account_contract::{
    event_metadata, view_metadata, AccountViewKey, AccountViewKind, AccountViewPublisher,
    EncodeContext,
};
...
pub struct MmapAccountPublisher {
    current_writer: ReplacementSnapshotStorage,
    root: PathBuf,
    slot_size: usize,
...
impl MmapAccountPublisher {
    pub fn create(
        ...
        let effective_slot_size = if slot_size == 0 {
            DEFAULT_SLOT_SIZE
        } else {
            slot_size
        };
