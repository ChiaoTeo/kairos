...
use crate::application::{CommandOutcome, IntegrationError, TransferRequest, TransferResult};

pub(crate) fn transfer_params(
    ...
    Ok(BTreeMap::from([
        ("type".into(), transfer_type(request)?),
        ("asset".into(), request.asset.trim().to_ascii_uppercase()),
        ("amount".into(), format_decimal(request.amount)),
    ]))
}

pub(crate) fn normalize_transfer(payload: &serde_json::Value) -> CommandOutcome<TransferResult> {
    CommandOutcome::Confirmed(TransferResult {
        accepted: true,
...
fn format_decimal(value: crate::application::capabilities::ExternalDecimal) -> String {
    if value.scale == 0 {
