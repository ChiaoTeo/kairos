...
#[cfg(test)]
mod credential_security_tests {
    use super::CredentialRecord;

    #[test]
    fn credential_debug_output_redacts_all_secret_fields() {
        let record = CredentialRecord {
            credential_id: "live".into(),
            provider: "binance".into(),
            role: "trading".into(),
            api_key: "api-key-secret".into(),
            secret: "api-secret-secret".into(),
            passphrase: "passphrase-secret".into(),
        };
        let output = format!("{record:?}");
        assert!(!output.contains("api-key-secret"));
        assert!(!output.contains("api-secret-secret"));
