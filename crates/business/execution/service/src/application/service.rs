...
impl ExecutionApplication {
    ...
    pub fn remote_open_orders(
        ...
        self.order_query
            ...
            .map_err(|error| { warn!(event = "remote_open_orders_query_failed", component = "execution", error = %error, "remote open-orders query failed"); ExecutionError::Gateway(error) })
    }
    ...
    pub fn next_remote_execution_event(
        &mut self,
    ) -> Result<Option<VenueOrderUpdate>, ExecutionError> {
    ...
    pub fn take_execution_stream(&mut self) -> Option<Box<dyn ExecutionStreamConnection>> {
    ...
    pub fn consume_remote_execution_event(
        &mut self,
    ) -> Result<Option<(VenueOrderUpdate, ExecutionOrder)>, ExecutionError> {
    ...
    /// Apply one normalized venue fact received from the private order stream.
            ...
            match self.submit(order) {
                Ok(order) => {
                    self.commit_intent(IntentEvent {
                        ...
                        dependency_watermarks: accepted.dependency_watermarks.clone(),
                    })?;
                    accepted.order_ids.push(order.order_id);
                    accepted.status = IntentStatus::Executing;
                }
                Err(error) => {
                    warn!(event = "intent_failed", component = "execution", intent_id = %intent.intent_id, error = %error, "child order prevented intent execution");
    ...
    pub fn record_fill(
        ...
        info!(event = "fill_applied", component = "execution", fill_id = %fill.fill_id, order_id = %next.order_id, status = ?next.status, filled_quantity = next.filled_quantity_mantissa, "execution fill applied");
        Ok(next)
    }
    ...
    /// The gateway worker calls this through the venue event mailbox; it does
    ...
    pub fn apply_order_entry_event(
        ...
        info!(event = "order_submitted", component = "execution", order_id = %order.order_id, venue_order_id = ?order.venue_order_id, status = ?order.status, "order submission completed");
        Ok(order)
    }

    pub fn mark_gateway_error(
        &mut self,
        order_id: &str,
        ...
        self.mark_unknown_after_gateway_error(order_id, reason)
    }
    ...
    pub fn submit(&mut self, request: SubmitOrder) -> Result<ExecutionOrder, ExecutionError> {
        ...
        let Some(connection) = self.order_entry.as_mut() else {
            self.mark_gateway_error(&order.order_id, "order entry connection is not configured")?;
        ...
        let event = match connection.submit_order(&connection_request) {
            ...
            Err(error) => {
                ...
                self.mark_gateway_error(&order.order_id, error.clone())?;
                return Err(ExecutionError::Gateway(error));
            }
        };
