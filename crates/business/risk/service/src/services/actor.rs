...
use crate::services::persistence::RiskStateStore;
use tracing::info;
...
impl RiskActor {
        ...
        if let Some(store) = actor.store.as_mut() {
            if let Some(snapshot) = store.load()? {
                ...
                info!(
                    ...
                    component = "risk",
                    budget_count = actor.budgets.len(),
                    reservation_count = actor.reservations.len(),
                    generation = actor.generation,
                    event_sequence = actor.event_sequence,
                    "risk state restored from persistence"
                );
                return Ok(actor);
            }
        }
    ...
    pub fn replace_budgets(&mut self, budgets: Vec<Budget>) -> Result<(), String> {
        ...
        let previous = std::mem::replace(&mut self.budgets, next);
        let previous_generation = self.generation;
        let previous_event_sequence = self.event_sequence;
        if let Err(error) = self.changed() {
            self.budgets = previous;
            self.generation = previous_generation;
            self.event_sequence = previous_event_sequence;
            return Err(error);
        }
        Ok(())
    }
    ...
    pub fn reserve(&mut self, request: ReserveRisk) -> Result<Reservation, ActorError> {
        ...
        let allocations = assessment
            ...
            .collect::<Vec<_>>();
        let previous_budgets = self.budgets.clone();
        let previous_reservations = self.reservations.clone();
        let previous_generation = self.generation;
        let previous_event_sequence = self.event_sequence;
        ...
        if let Err(error) = self.changed() {
            ...
            self.reservations = previous_reservations;
            self.generation = previous_generation;
            self.event_sequence = previous_event_sequence;
            return Err(ActorError::State(error));
        }
