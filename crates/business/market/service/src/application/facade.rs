...
use crate::services::messages::{
    SourceCommand, SourceInput, SourceOrderBookUpdate, SourceRequestId,
};
use crate::services::sources::SourceActivator;
use crate::services::sources::SourceHandle;
...
impl MarketApplication {
    ...
    pub(crate) fn restore_with_source_capacity(
        ...
        Ok(Self {
            actor: MarketActor::restore(snapshot, max_dynamic_members, source_input_capacity)
                .map_err(MarketError::Invalid)?,
        })
    }
    ...
    async fn reconcile_source_commands(&mut self) -> Result<(), String> {
        ...
        let mut desired =
            BTreeMap::<SourceId, BTreeMap<BusinessSubscriptionKey, MarketDescriptor>>::new();
    ...
    fn next_request_id(&mut self) -> SourceRequestId {
        ...
        match input {
            ...
            SourceInput::ReplayObservation {
                ...
                match self.ingest(observation) {
                    Ok(_) => {
                        let _ = accepted.send(Ok(self.snapshot()));
                        return Ok(1);
                    }
                    Err(error) => {
    ...
    async fn request_orderbook_resync(
        &mut self,
        source_id: SourceId,
        epoch: SourceEpoch,
    ...
    fn apply_source_orderbook(&mut self, update: SourceOrderBookUpdate) -> Result<(), MarketError> {
        if update.snapshot {
            ...
            self.ingest_orderbook_delta(crate::domain::orderbook::OrderBookDelta {
...
pub(crate) fn source_accepts(source: &SourceDescriptor, market: &MarketDescriptor) -> bool {
    let source_id_matches = source.exchange_id.is_none()
        || market
            .source_id
            .as_deref()
            .is_none_or(|id| id.eq_ignore_ascii_case(source.id.as_str()));
    source_id_matches
...
mod tests {
    ...
    use crate::services::sources::{SourceActivator, SourceHandle};
    ...
    impl SourceActivator for CountingActivator {
        fn activate<'a>(
            &'a mut self,
            _market: &'a crate::MarketDescriptor,
    ...
    async fn subscription_projection_reports_route_and_provider_readiness() {
        ...
        let market = crate::domain::market::MarketDescriptor::new_with_asset_type(
        ...
        empty
            ...
            .unwrap();
        assert_eq!(
            empty.snapshot().subscriptions[0].status,
    ...
    async fn concurrent_same_route_intents_share_one_activation() {
        ...
        assert_eq!(application.snapshot().sources.len(), 1);
        application
            .shutdown_sources(Duration::from_secs(1))
            .await
            .unwrap();
    ...
    async fn orderbook_gap_requests_only_the_affected_market_resync() {
        ...
        application
            .ingest_orderbook_snapshot(
                crate::domain::orderbook::OrderBook::snapshot_with_source(
                    "binance-spot",
                    "btc-usdt",
                    "btc",
                    10,
        ...
        application
            .apply_source_input(SourceInput::OrderBook {
                ...
                epoch: SourceEpoch::new(1),
                update: crate::services::messages::SourceOrderBookUpdate {
                    market: Box::new(market.clone()),
    ...
    async fn stale_route_epoch_observation_cannot_mutate_actor_state() {
        ...
        application
            ...
            .await
            .unwrap();
        assert_eq!(application.snapshot().event_sequence.get(), 0);
        application
            .apply_source_input(SourceInput::Observation {
                source_id,
                epoch: SourceEpoch::new(2),
            ...
            .await
            .unwrap();
        assert_eq!(application.snapshot().event_sequence.get(), 1);
    }
