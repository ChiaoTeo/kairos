...
use crate::application::runtime::{
    build_application, ensure_database_parent, ReferenceRuntimeConfig,
};
use crate::application::{ReferenceApplication, ReferenceKind, ReferenceQuery};
...
pub fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = ReferenceRuntimeConfig {
        provider,
        endpoint,
        database,
        api_key,
    ...
    let mut application = build_application(&config, true)?;
    ...
    while !stopping {
        tokio::select! {
            accepted = listener.accept() => {
                ...
                stopping = handle_client(&mut application, stream).await?;
            }
            _ = interval.tick() => {
                let status = if application.refresh().is_ok() { "ready" } else { "degraded" };
...
async fn handle_client(
    ...
    let (status, body, stopping) = match path {
        ...
        control::PUBLISH => match application.publish() {
