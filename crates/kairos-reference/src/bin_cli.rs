...
use kairos_reference::application::runtime::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut application = build_application(&config, args.command.requires_publication())?;
    let value = execute(&mut application, args.command)?;
...
fn execute(
    ...
    let value = match command {
        ...
        Command::Publish => json!({ "generation": application.publish()? }),
