...
use crate::application::{
    ExecutionAuditEvent, ExecutionAuditQuery, ExecutionEvent, ExecutionOrderQuery,
    ExecutionSnapshot, ExecutionStream, RemoteExecutionEvent, RemoteOrder, RemoteOrderQuery,
};
use kairos_integration::application::{
    Connection, ExecutionStreamConnection, ExternalOrderQuery, OrderEntryConnection,
...
struct IntegrationOrderQuery(Box<dyn OrderQueryConnection>);
struct IntegrationExecutionStream(Box<dyn ExecutionStreamConnection>);

impl ExecutionStream for IntegrationExecutionStream {
    fn next_event(&mut self) -> Result<Option<RemoteExecutionEvent>, String> {
        self.0.next_execution_event().map(|event| {
            event.map(|value| RemoteExecutionEvent {
                order_id: value.order_id,
                symbol: value.symbol,
                status: value.status,
                fill_quantity: value.fill_quantity.map(decimal),
                fill_price: value.fill_price.map(decimal),
...
impl ExecutionOrderQuery for IntegrationOrderQuery {
    fn open_orders(&mut self, query: &RemoteOrderQuery) -> Result<Vec<RemoteOrder>, String> {
        self.0
            .open_orders(&external_query(query))
    ...
    fn history(&mut self, query: &RemoteOrderQuery) -> Result<Vec<RemoteOrder>, String> {
        self.0
            .order_history(&external_query(query))
            .map(|orders| orders.into_iter().map(remote_order).collect())
    }
    fn detail(&mut self, query: &RemoteOrderQuery) -> Result<Option<RemoteOrder>, String> {
        self.0
            .order_detail(&external_query(query))
            .map(|order| order.map(remote_order))
    }
}

fn external_query(query: &RemoteOrderQuery) -> ExternalOrderQuery {
    ExternalOrderQuery {
...
fn remote_order(order: kairos_integration::application::ExternalOrder) -> RemoteOrder {
    RemoteOrder {
        ...
        side: match order.side {
            kairos_integration::domain::OrderSide::Buy => crate::domain::OrderSide::Buy,
            kairos_integration::domain::OrderSide::Sell => crate::domain::OrderSide::Sell,
        },
        order_type: match order.order_type {
            kairos_integration::domain::OrderType::Market => crate::domain::OrderType::Market,
            _ => crate::domain::OrderType::Limit,
        },
        status: order.status,
...
fn decimal(value: kairos_integration::domain::DecimalValue) -> String {
    if value.scale == 0 {
...
pub fn compose_order_query(
    options: &ExecutionConnectionOptions,
) -> Result<Option<Box<dyn ExecutionOrderQuery>>, String> {
    let provider = options.provider.trim().to_ascii_lowercase();
    let product = options.product.trim().to_ascii_lowercase();
    let product_family = match product.as_str() {
        "equity" | "stocks" => {
            return Ok(Some(Box::new(IntegrationOrderQuery(
                Integration::new()
    ...
    let connection = integration
        ...
        .map_err(|error| error.to_string())?;
    Ok(Some(Box::new(IntegrationOrderQuery(connection))))
}

pub fn compose_execution_stream(
    options: &ExecutionConnectionOptions,
) -> Result<Option<Box<dyn ExecutionStream>>, String> {
    let provider = options.provider.trim().to_ascii_lowercase();
    if provider != "ibkr" {
        return Ok(None);
    }
    ...
    let connection = integration
        ...
        .map_err(|error| error.to_string())?;
    Ok(Some(Box::new(IntegrationExecutionStream(connection))))
}
