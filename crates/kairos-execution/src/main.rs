...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let state = instance
        .as_ref()
        .map(|value| value.state(&["execution", "execution-state.json"]))
        .transpose()?
        .unwrap_or(workspace.child(&["state", "execution", "execution-state.json"])?);
    let audit = instance
        .as_ref()
        .map(|value| value.state(&["execution", "execution-audit.sqlite"]))
        .transpose()?
        .unwrap_or(workspace.child(&["state", "execution", "execution-audit.sqlite"])?);
    ...
    let socket = instance
        .as_ref()
        .map(|value| value.socket("execution"))
        .transpose()?
        .unwrap_or(workspace.process_socket("execution")?);
