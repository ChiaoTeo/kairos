...
def test_account_current_view_rejects_incomplete_or_corrupt_mmap(tmp_path: Path) -> None:
    path = _account_path(tmp_path)
...
def test_account_current_view_reopens_after_publisher_restart(tmp_path: Path) -> None:
    path = _account_path(tmp_path)
    _write_shared_snapshot(path, _account_snapshot())
