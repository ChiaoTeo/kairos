...
from trading import BacktestRequest, BacktestRunner, Trader


class TraderApiTest(unittest.TestCase):
    def test_trader_backtest_returns_notebook_friendly_result_view(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            result = Trader(root).backtest(
