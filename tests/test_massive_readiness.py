...
from trading.adapters.massive.readiness import MassiveReadinessChecker
...
class MassiveReadinessTests(unittest.TestCase):
    def test_all_probes_use_private_host_and_do_not_put_key_in_url(self):
        transport = StubTransport()
        report = MassiveReadinessChecker(MassiveClient(MassiveConfig("secret"), transport)).check(
    ...
    def test_synthetic_forward_readiness_requires_the_opposite_call_put_quote(self):
        report = MassiveReadinessChecker(MassiveClient(MassiveConfig("secret"), IndexAndPairForbiddenTransport())).check(
            underlying="SPX", option_ticker="O:SPXW251103C06000000", date="2025-11-03")
