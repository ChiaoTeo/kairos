...
if pandas is not None:
    from studies.spxw_put_skew.study import StudyConfig, analyze_hypothesis, build_panel, execute_study
    from tests.test_options_study_end_to_end import internally_priceable_spxw_dataset
    from kairospy.study_platform.data_store import CollectionManifest, CollectionSession
...
class SpxwPutSkewStudyTests(unittest.TestCase):
    def test_panel_is_point_in_time_and_current_fixture_is_insufficient(self) -> None:
        ...
        _, readiness, guarded = execute_study(dataset, config, None)
    ...
    def test_invalid_configuration_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            StudyConfig(min_dte=45, max_dte=7)
        with self.assertRaises(ValueError):
            StudyConfig(high_skew_percentile=Decimal("1"))
    ...
    def test_massive_canonical_collection_is_a_verified_real_source(self) -> None:
        ...
        config = StudyConfig(min_dte=0, max_dte=7, forward_horizon_days=1, minimum_rank_history=2, minimum_observations=252, bootstrap_samples=20)
        _, readiness, _ = execute_study(dataset, config, CollectionManifest(1, dataset.manifest.dataset_id, (session,)))
