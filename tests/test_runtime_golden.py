...
from trading.application.runtime_golden import GOLDEN_SCENARIO_ID, run_runtime_golden
...
class RuntimeGoldenTests(unittest.TestCase):
    def test_formal_runtime_chain_survives_restart_with_fixed_audit_hash(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            result = run_runtime_golden(directory)
            self.assertEqual(result.scenario_id, GOLDEN_SCENARIO_ID)
            ...
            rerun = run_runtime_golden(directory)
            self.assertEqual(rerun.audit_hash, EXPECTED_AUDIT_HASH)

    def test_runtime_golden_cli_writes_product_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            self.assertEqual(main(["runtime", "golden", "--root", directory]), 0)
            artifact = Path(directory) / "artifacts" / GOLDEN_SCENARIO_ID / "manifest.json"
