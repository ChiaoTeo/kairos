...
def run_runtime_reference_artifact(root: str | Path) -> RuntimeReferenceArtifactResult:
    ...
    account = AccountRef(InstitutionId("simulated"), "runtime-reference", AccountType.CRYPTO_SPOT)
...
def _catalog() -> ReferenceCatalog:
    ...
    for account_id in ("runtime-reference", "failure-policy"):
        account = AccountRef(InstitutionId("simulated"), account_id, AccountType.CRYPTO_SPOT)
