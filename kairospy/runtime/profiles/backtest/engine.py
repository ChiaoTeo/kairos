...
from kairospy.analytics.pricing.option_valuation import OptionValuationService
from kairospy.research.capture.features import FeatureEngine, build_features
...
from kairospy.market.snapshots import MarketReplayDataset, MarketSnapshot, MarketSnapshotFeed, MarketSnapshotReplayFeed
...
class BacktestEngine:
    ...
    def run(self) -> BacktestResult:
        ...
        for market in self.feed.between(self.config.start, self.config.end):
            ...
            working_orders = tuple(order for order in orders.values() if not order.status.terminal)
            context = Context(
                MarketView.from_snapshot(market),
                PortfolioView.from_snapshot(snapshot),
        ...
        end_context = Context(
            MarketView.from_snapshot(last_market),
            PortfolioView.from_snapshot(portfolio.snapshot(last_market)),
