...
from kairospy.strategy.intents import CloseStructureIntent, Intent
from kairospy.execution.events import TradeSide
from kairospy.execution.orders import Order, OrderLeg, OrderStatus
from kairospy.reference.contracts import is_option_spec
...
def combo_quote(legs, market: MarketSnapshot, quantity: int) -> ComboQuote | None:
...
class ExecutionPlanner:
    ...
    def plan(self, intent: Intent, now: datetime) -> Order:
        ...
        order_legs = tuple(OrderLeg(leg.instrument_id, leg.side, leg.ratio) for leg in intent.legs)
        return Order(
            self.id_factory(), intent.intent_id, intent.strategy_id, structure_id, order_legs, intent.quantity,
