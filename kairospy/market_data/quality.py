...
def validate_option_observation(
    ...
    *,
    ...
    if bid is None or ask is None:
        issues.append(MarketQualityIssue("missing_two_sided_quote", "error", "bid and ask are required", observation.instrument_id))
    else:
        if bid < 0 or ask < 0:
        ...
        if bid <= ask and bid + ask > 0:
            ...
            if relative_spread > max_relative_spread:
                issues.append(MarketQualityIssue("wide_spread", "warning", f"relative spread {relative_spread} exceeds {max_relative_spread}", observation.instrument_id))
