...
def build_surface(
    ...
    as_of: datetime,
    observations: tuple[VolObservation, ...],
    *,
    ...
    if available_time.tzinfo is None:
        raise ValueError("surface available_time must be timezone-aware")
    if available_time < as_of:
        raise ValueError("surface available_time cannot precede as_of")
    rejected = []
    candidates = []
    ...
    return SurfaceSnapshot(
        surface_id, underlying_id, as_of, "svi", "1", input_hash, smiles, diagnostics,
        status, tuple(rejected), available_time,
    )
