...
class SmaFactorRuntime(CanonicalBarFactorRuntime):
    def __init__(self, config: SmaFactorConfig = SmaFactorConfig(), *, input_identity: str,
        ...
        self._spec = FactorSpec(
            ...
            "kairospy.analytics.features.sma:SmaFactorRuntime", implementation_hash(source),
        )
    ...
    def update(self, event: CanonicalEventEnvelope) -> FactorSnapshot | None:
        bar = self._bar(event)
        return self.update_bar(bar, available_time=event.available_time) if bar is not None else None

    def update_bar(self, bar: Bar, *, available_time=None) -> FactorSnapshot:
        ...
        self._snapshot = FactorSnapshot(
            ...
            available_time if available_time is not None else bar.end,
        )
