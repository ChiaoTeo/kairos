...
class GatewayRegistry:
    ...
    def with_builtins(cls) -> "GatewayRegistry":
        ...
        for route in (
            ...
            registry.register(route=route, product=ProductFamily.EARN, capability=IntegrationCapability.EARN, transport=TransportKind.REST, factory=BinanceSimpleEarnGateway().open)
        ...
        registry.register(route=ibkr, product=ProductFamily.EQUITY, capability=IntegrationCapability.ACCOUNT_READ, transport=TransportKind.REQUEST_API, factory=IBKRAccountGateway().open)
        registry.register(route=ibkr, product=ProductFamily.EQUITY, capability=IntegrationCapability.ACCOUNT_STREAM, transport=TransportKind.REQUEST_API, factory=IBKRAccountStreamGateway().open)
        registry.register(route=ibkr, product=ProductFamily.EQUITY, capability=IntegrationCapability.ORDER_ENTRY, transport=TransportKind.REQUEST_API, factory=IBKRExecutionGateway().open)
        registry.register(route=ibkr, product=ProductFamily.EQUITY, capability=IntegrationCapability.EXECUTION_STREAM, transport=TransportKind.REQUEST_API, factory=IBKRExecutionStreamGateway().open)
