from __future__ import annotations

from collections.abc import AsyncIterator
from uuid import uuid4

from kairospy.infrastructure.integrations.application.connections import (
    IntegrationConnectionSpec,
)
from kairospy.application.usecases.market.application.integration import MarketFeedSubscriptionRequest
from kairospy.infrastructure.integrations.domain import AccessScope, ProductFamily, TransportKind
from kairospy.infrastructure.integrations.services.connections.connection import Connection
from .stream import BinanceSpotMarketStream
from .normalizers import BinanceSpotNormalizers


class BinanceSpotPublicStreamGateway:
    """Create Binance Spot public market-stream connections."""

    def open(self, spec: IntegrationConnectionSpec) -> "BinanceSpotPublicStreamConnection":
        _validate_spec(spec)
        return BinanceSpotPublicStreamConnection(spec)


class BinanceSpotPublicStreamConnection(Connection):
    """Stateful Binance Spot public WebSocket connection."""

    def __init__(self, spec: IntegrationConnectionSpec) -> None:
        self.stream = BinanceSpotMarketStream()
        self.normalizers = BinanceSpotNormalizers()
        self._subscriptions: dict[str, BinanceSpotRemoteSubscription] = {}
        super().__init__(spec, components=(self.stream,))

    async def subscribe(self, request: MarketFeedSubscriptionRequest) -> "BinanceSpotRemoteSubscription":
        channel = _market_channel(request.selector)
        if channel not in {"ticker", "trade", "orderbook"}:
            raise ValueError(f"Binance Spot does not support market stream channel: {channel}")
        remote = BinanceSpotRemoteSubscription(
            subscription_id=f"binance-spot-{uuid4().hex}",
            stream=self.stream,
            symbol=str(request.market.source_symbol),
            channel=channel,
            stream_channel="depth" if channel == "orderbook" else channel,
            market=request.market,
            normalizers=self.normalizers,
        )
        self._subscriptions[remote.subscription_id] = remote
        return remote

    async def unsubscribe(self, subscription_id: str) -> None:
        remote = self._subscriptions.pop(subscription_id, None)
        if remote is not None:
            await remote.close()


class BinanceSpotRemoteSubscription:
    def __init__(
        self,
        subscription_id: str,
        stream: BinanceSpotMarketStream,
        symbol: str,
        channel: str,
        stream_channel: str,
        market: object,
        normalizers: BinanceSpotNormalizers,
    ) -> None:
        self.subscription_id = subscription_id
        self.stream = stream
        self.symbol = symbol
        self.channel = channel
        self.stream_channel = stream_channel
        self.market = market
        self.normalizers = normalizers

    def events(self) -> AsyncIterator[object]:
        return self._events()

    async def close(self) -> None:
        await self.stream.stop()

    async def _events(self) -> AsyncIterator[object]:
        async for payload in self.stream.events(self.symbol, self.stream_channel):
            yield self.normalizers.market_domain_event(
                payload,
                market=self.market,
                channel=self.channel,
            )


def _market_channel(selector: object) -> str:
    model = getattr(selector, "model", selector)
    name = getattr(model, "__name__", str(model)).lower()
    if "quote" in name:
        return "ticker"
    if "orderbook" in name:
        return "orderbook"
    if "trade" in name:
        return "trade"
    return name


def _validate_spec(spec: IntegrationConnectionSpec) -> None:
    if spec.product is not ProductFamily.SPOT:
        raise ValueError("Binance Spot public stream requires the spot product")
    if spec.access is not AccessScope.PUBLIC:
        raise ValueError("Binance Spot public stream requires public access")
    if spec.transport is not TransportKind.MARKET_STREAM:
        raise ValueError("Binance Spot public stream requires market-stream transport")


__all__ = [
    "BinanceSpotNormalizers",
    "BinanceSpotPublicStreamConnection",
    "BinanceSpotPublicStreamGateway",
    "BinanceSpotRemoteSubscription",
]
