...
class MassiveDriver:
    ...
    def _pages(self, path: str, params: RawPayload) -> RawPayloadRows:
    ...
    def _url(self, path: str, params: RawPayload) -> str:
    ...
    @staticmethod
    def _default_http_get(url: str) -> RawPayload:
        request = Request(url, headers={"Accept": "application/json"})
        with urlopen(request, timeout=30) as response:
            return json.loads(response.read().decode("utf-8"))


def _normalize_equity_row(row: RawPayload, *, active: bool) -> RawPayload:
    value = dict(row)
    if value.get("ticker"):
