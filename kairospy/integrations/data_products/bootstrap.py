...
from kairospy.integrations.extensions.provider_extensions import (
    CONFIG_PATH_KEY,
    provider_extension_specs,
    register_provider_extensions,
)
from kairospy.integrations.data_products import KNOWN_PRODUCTS, binance, deribit, massive


MassiveConfigResolver = Callable[[], object]
    ...
    for dataset in KNOWN_PRODUCTS:
        try:
            ...
            catalog.update_product_spec(
                ...
                reason="synchronize integration-provided data product contract",
            )
    catalog.save()
    return catalog
...
def default_provider_registry(
    ...
    *,
    ...
    progress=None,
    stop_event=None,
    massive_config_resolver: MassiveConfigResolver | None = None,
) -> ProviderRegistry:
    ...
    resolver = massive_config_resolver or massive_config_for_project
        ...
        for raw in _massive_equity_products(configured_config):
            ...
            connector = _ConfiguredMassiveEquityConnector(
                ...
                MassiveEquityDailyOhlcvProductConfig(
                    str(raw["logical_key"]),
                    str(raw["ticker"]),
                    str(raw.get("view", "vendor_adjusted")),
                ),
                resolver,
            )
            providers.register(connector, (spec,))
    ...
    try:
        massive.register(providers, root, resolver())
...
class _ConfiguredMassiveConnector:
    ...
    def __init__(self, root, config, massive_config_resolver: MassiveConfigResolver) -> None:
        self.root = root
        self.config = config
        self._massive_config_resolver = massive_config_resolver
    ...
    def acquire(self, request):
        ...
        return MassiveOptionEventsDatasetConnector(
            self.root,
            MassiveClient(self._massive_config_resolver()),
            self.config,
        ).acquire(request)
...
def massive_config_for_project():
...
__all__ = [
    "configured_product_specs",
    "default_provider_registry",
    "massive_config_for_project",
    "register_configured_products",
