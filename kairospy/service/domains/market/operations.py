...
from kairospy.data import DataSink, DataStore
from kairospy.core.market import MarketDataField, MarketSubscription
...
def _kind_from_subscription(subscription: MarketSubscription) -> str:
    families = {_field_family(field) for field in subscription.spec.fields}
    if "bar" in families:
        return "ohlcv"
    ...
    if "book" in families:
        return "orderbook"
    if "quote" in families or "ticker" in families:
        return "ticker"
    if families:
...
def _timeframe_from_subscription(subscription: MarketSubscription) -> str | None:
    intervals = {
        field.interval
        for field in subscription.spec.fields
        if isinstance(field, MarketDataField) and field.interval is not None
    }
    if len(intervals) > 1:
...
def _field_family(field: MarketDataField | object) -> str:
    if isinstance(field, MarketDataField):
        return field.family
