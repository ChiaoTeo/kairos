...
class StrategySpec:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.evidence_hash:
            raise ValueError("strategy evidence hash is required")
...
class EconomicIntent:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.intents:
            raise ValueError("economic intent must contain at least one strategy intent")
        for intent in self.intents:
            if intent.strategy_id != self.strategy_id:
                raise ValueError("all strategy intents must belong to the enclosing strategy")
    ...
    def create(
        ...
        *,
        ...
        decision_id = uuid5(NAMESPACE_URL, "kairospy:economic-intent:" + hashlib.sha256(encoded.encode()).hexdigest())
        return cls(
