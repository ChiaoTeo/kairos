"""Market and Reference interaction flow, independent of Textual widgets."""
...
from ..guided.catalog import MARKET_ADVANCED_ACTIONS, SECTION_ACTIONS
...
def handle_input(
    ...
    """Continue one typed Market or Reference input interaction."""
    ...
    if token.feature is Feature.MARKET:
        ...
        token.feature is Feature.REFERENCE
        and token.action == "search"
        and token.field is not None
    ):
...
def cancel_input(session: GuidedSession, token: ActionToken) -> None:
    """Clear only the Market or Reference prompt owned by the token."""
    ...
    elif token.feature is Feature.REFERENCE:
...
def handle_command(
    ...
    """Handle typed input commands owned by Market and Reference."""
    ...
    if command.startswith("reference:"):
        ...
        if kind not in {
            ...
            return None
        query = " ".join(arguments).strip()
        return (_run_reference_search(state, session, kind, query),)
...
def handle_success(
    ...
    """Interpret the result kinds already owned by this product slice."""

    kind = spec.route.kind
    if kind is ResultKind.MARKET:
        records = tuple(result or ())
        session.market.records = records
        if records:
            return _show_record_choices(session, "market", records)
        session.enter("market")
    ...
    if kind is ResultKind.MARKET_OBSERVATION:
        ...
        session.market.snapshot = result
    ...
    if kind is ResultKind.MARKET_ROUTES:
        ...
        preferred = next(
            (
                route
                for route in routes
                if route.get("provider") == session.market.provider
            ),
            None,
        )
        if not routes:
...
def _handle_reference_context(
    ...
    if session.context == ("reference", "selected"):
        record = state.selected_reference
    ...
    if len(session.context) > 1 and session.visible_records:
        ...
        if record is None:
            return None
        state.selected_reference = record
...
def _market_interaction(
    ...
    if (
        ...
        and state.selected_market is not None
    ):
        return ControlInteraction(
            title=record_label(state.selected_market),
            snapshot=observation_renderable(session.market.snapshot),
            actions=context_items(session, state),
            refreshing=True,
        )
