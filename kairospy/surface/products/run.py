...
from kairospy.modes.backtest import BacktestEngine, BacktestEngineDaemonTarget, SimulatedAccount, backtest_result_summary
from kairospy.context import DataContext
from kairospy.core.reference import MarketResolver
from kairospy.data import DataStore
from kairospy.modes.paper import PaperAccountConfig, PaperEngine, PaperEngineDaemonTarget, paper_result_summary, run_streaming_paper
from kairospy.runtime import AccountRegistry, AsyncIterableEventSource, IterableEventSource, list_run_daemons
from kairospy.runtime.account_journal import RunAccountJournal
from kairospy.runtime.line import RuntimeMode
from kairospy.runtime.daemon import RunDaemonControlPlane, RunDaemonTarget, RunExecutionContext
...
def _configured_daemon_target(
    ...
    configured = _configured_mode(mode, config_path, events=events)
...
class _StreamingPaperDaemonTarget:
    ...
    def run(self, context: RunExecutionContext) -> dict[str, object]:
        ...
        controller = _ConfiguredPaperSourceController(context)
...
class _ConfiguredPaperSourceController:
    ...
    def source(self) -> AsyncIterableEventSource:
        ...
        exchange_client = exchange(_exchange_name(venue), DriverName.ccxt)
...
def _configured_mode(mode: RuntimeMode, config_path: Path, *, events: Path | None = None) -> _ConfiguredMode:
    run_config = load_run_config(config_path)
    ...
    return _ConfiguredMode(run_config.run_id, engine, source)
...
def _configured_account(run_config, mode: RuntimeMode):
    ...
    if accounts:
        ...
        if mode is RuntimeMode.PAPER and len(configured_accounts) != 1:
            raise typer.BadParameter("configured paper event runs require exactly one [accounts.*] account")
