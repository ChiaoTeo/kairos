...
from kairospy.backtest.feed import InstrumentLifecycleSnapshot, MarketReplayDataset, MarketSnapshot, SettlementType, build_manifest
...
class MassiveMarketSnapshotBuilder:
    def __init__(self, lake_root: str | Path = "data", *,
        ...
        self.repository = MarketSnapshotStorageDriver(dataset_root if dataset_root is not None else self.lake_root / "curated")
    ...
    def build(self, source_dataset_id: str, output_dataset_id: str, start: datetime, end: datetime, *,
              ...
              risk_free_rate: Decimal = Decimal("0")) -> MarketReplayDataset:
        if start.tzinfo is None or end.tzinfo is None or not start < end:
            raise ValueError("curated market snapshot builder requires timezone-aware [start,end)")
        ...
        while cursor < end:
            ...
            slices.append(MarketSnapshot(cursor, tuple(snapshots), tuple(sorted(slice_references.items(), key=lambda item: item[0].value)),
        ...
        MarketSnapshotCollectionPublisher(self.repository).save_session(dataset, append=False)
        self._publish(source_dataset_id, dataset)
        return dataset

    def _publish(self, source_dataset_id: str, dataset: MarketReplayDataset) -> None:
        directory = self.repository.root / dataset.manifest.dataset_id
        logical_key = DatasetKey(f"curated.market_snapshots.options.us.{_dataset_underlying(source_dataset_id)}")
        ...
        write_json(directory / "schema.json", {
            "schema_id": "market_replay_dataset.v2", "schema_version": 2,
        ...
        write_json(directory / "lineage.json", {
            ...
            "producer": {"name": type(self).__name__, "transform": "point_in_time_market_snapshots", "version": 2},
        ...
        product = DataProductDefinition(logical_key, f"Point-in-time {_dataset_underlying(source_dataset_id).upper()} option snapshots",
        ...
        release = DatasetRelease(dataset.manifest.dataset_id, logical_key, dataset.manifest.end.isoformat(),
            "market_replay_dataset.v2", "2", "point_in_time_market_snapshots", "2", relative_path, "parquet",
        ...
        write_json(directory / "usage.json", {"primary_time": "timestamp", "default_view": "raw-as-received",
            "known_limitations": ["sampled_market_snapshots"]})
        catalog.register_product(product, enrich=True); catalog.register_release(release); catalog.save()


MassiveCuratedSliceBuilder = MassiveMarketSnapshotBuilder


def _contract(definition) -> InstrumentLifecycleSnapshot:
    ...
    return InstrumentLifecycleSnapshot(definition.instrument_id, spec.last_trade_at, spec.expiry,
