...
from kairospy.reference import ProviderId, ReferenceCatalog, ReferenceCatalogRepository
...
class MassiveEquityDailyOhlcvPipeline:
    ...
    def prepare(self, dataset_id: str, ticker: str, start: date, end: date, *, view: str = "vendor_adjusted") -> dict[str, object]:
        ...
        if not ticker or not start < end:
            raise ValueError("equity daily OHLCV requires ticker and [start,end)")
        if view not in {"raw", "vendor_adjusted"}:
            raise ValueError("equity daily OHLCV view must be 'raw' or 'vendor_adjusted'")
        ...
        if not rows:
            raise ValueError(f"Massive returned no {ticker} daily OHLCV")
        if len({item["event_date"] for item in rows}) != len(rows):
            raise ValueError("equity daily OHLCV contains duplicate trading dates")
...
class MassiveEquityHourlyOhlcvPipeline(MassiveEquityDailyOhlcvPipeline):
    def prepare(self, dataset_id: str, ticker: str, start: date, end: date, *, view: str = "vendor_adjusted") -> dict[str, object]:
        ticker = ticker.strip().upper()
        if not ticker or not start < end:
            raise ValueError("equity hourly OHLCV requires ticker and [start,end)")
        if view not in {"raw", "vendor_adjusted"}:
            raise ValueError("equity hourly OHLCV view must be 'raw' or 'vendor_adjusted'")
        adjusted = view == "vendor_adjusted"
        archive = self.source.fetch_pages(
        ...
        for raw in self.source.iter_results(archive):
            ...
            if not start <= trading_day < end:
                continue
            if not self.calendar.is_trading_day(trading_day):
                continue
            available_time = window_start + timedelta(hours=1)
            instrument_id = self._instrument_id(ticker, available_time)
            rows.append({
                "ticker": ticker, "instrument_id": instrument_id, "price_view": view,
                "event_date": trading_day,
                "window_start": window_start, "available_time": available_time,
                "open": Decimal(str(raw["o"])), "high": Decimal(str(raw["h"])),
                "low": Decimal(str(raw["l"])), "close": Decimal(str(raw["c"])),
                "volume": Decimal(str(raw.get("v", 0))),
                "transactions": int(raw.get("n", 0)),
        ...
        if not rows:
            raise ValueError(f"Massive returned no {ticker} hourly OHLCV")
        if len({item["window_start"] for item in rows}) != len(rows):
            raise ValueError("equity hourly OHLCV contains duplicate hourly windows")

        pa, pq = _pyarrow()
        target = self.root / "canonical/market/ohlcv/asset_class=equity/region=us/provider=massive/interval=1h" / f"view={view}" / f"dataset={dataset_id}"
        manifest_path = target / "manifest.json"
        row_hash = sha256(json.dumps(rows, default=_json_default, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
        if manifest_path.exists():
            existing = json.loads(manifest_path.read_text(encoding="utf-8"))
        ...
        manifest = {
            "manifest_version": 1, "dataset_id": dataset_id, "ticker": ticker,
            "source": "massive.stocks.hourly_aggregates", "view": view, "adjusted": adjusted,
            "interval": "1h", "boundary": "[start,end)",
            "start": start.isoformat(), "end": end.isoformat(), "rows": len(rows),
            "content_sha256": row_hash, "file": path.name, "file_sha256": _file_hash(path),
            "source_receipt": str((archive.directory / "receipt.json").relative_to(self.root)),
        }
        write_json(target / "lineage.json", {
            ...
            "resource": archive.receipt["resource"], "source_receipt": manifest["source_receipt"],
            "source": {"content_sha256": row_hash, "receipt": manifest["source_receipt"]},
            "visibility": "bar close plus provider publication latency", "view": view, "adjusted": adjusted,
            "parameters": {"ticker": ticker, "start": start.isoformat(), "end": end.isoformat(), "adjusted": adjusted, "interval": "1h"},
        })
        write_json(target / "coverage.json", {
            "start": datetime.combine(start, time.min, timezone.utc).isoformat(),
            "end": datetime.combine(end, time.min, timezone.utc).isoformat(),
            "boundary": "[start,end)",
            "rows": len(rows), "calendar": "US_SECURITIES", "ticker_count": 1,
            "tickers": [ticker], "view": view, "interval": "1h",
            "minimum_window_start": rows[0]["window_start"].isoformat(),
            "maximum_window_start": rows[-1]["window_start"].isoformat(),
        })
        write_json(target / "schema.json", {
            "schema_id": "market.ohlcv.equity.us.1h.v1", "schema_version": 1,
            "primary_key": ["instrument_id", "window_start", "price_view"],
            "primary_time": "available_time",
            "fields": [field.name for field in _schema(pa)],
        })
        write_json(target / "quality.json", {
            "publishable": True, "quality_level": "Q2", "duplicate_windows": 0, "invalid_ohlc": 0,
            "known_limitations": [
                "bounded single-ticker hourly dataset; not a full-market point-in-time universe",
                "provider ticker is still used as the provisional instrument identity",
                "hourly bars preserve provider aggregate windows and may include provider-defined eligible trades",
            ],
        })
        write_json(manifest_path, manifest)
        return manifest
...
def _pyarrow():
    try:
        ...
        raise RuntimeError("equity daily OHLCV requires the 'data' optional dependency") from error
    return pa, pq
