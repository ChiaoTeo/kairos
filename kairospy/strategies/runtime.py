...
from kairospy.domain.strategy_contract import EconomicIntent, StrategySpec

from .strategy_protocols import Strategy, StrategyContext
...
def _snapshot_payload(value):
    if is_dataclass(value): return _snapshot_payload(asdict(value))
    if isinstance(value,dict): return {str(key):_snapshot_payload(item) for key,item in value.items()}
    if isinstance(value,(tuple,list)): return [_snapshot_payload(item) for item in value]
    return value
