...
from ..storage.client import DataUnavailableError, DatasetClient
from ..contracts import DatasetStatus, QualityLevel
from .services import DatasetQualityService, QualityAssessment
...
WORKSPACE_DEFAULT_POLICY = DataPromotionPolicyProfile("workspace-default", QualityLevel.WORKSPACE)
...
DATA_PROMOTION_POLICY_PROFILES: Mapping[str, DataPromotionPolicyProfile] = MappingProxyType({
    WORKSPACE_DEFAULT_POLICY.name: WORKSPACE_DEFAULT_POLICY,
...
def evaluate_data_promotion_policy(
    ...
    *, profile: DataPromotionPolicyProfile | None = None,
    ...
    return DataPromotionPolicyResult(
        ...
        f"release {release_id} can be promoted; quality checks are diagnostics only",
    )
...
def _default_policy_profile(level: QualityLevel) -> DataPromotionPolicyProfile:
    ...
    return WORKSPACE_DEFAULT_POLICY
...
def _status_rank(status: DatasetStatus) -> int:
    return {
        ...
        DatasetStatus.APPROVED_FOR_WORKSPACE: 2,
