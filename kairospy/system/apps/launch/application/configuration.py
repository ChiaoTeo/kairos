...
from kairospy.system.apps.workspace.application import ResourceScopePaths
from .semantics import OptionBacktestConstraints
from .resource_diagnostics import (
    _current_resource_hashes,
    _structural_diagnostic,
    _workspace_account_issues,
    _workspace_resource_diagnostics,
    _workspace_resource_snapshots,
)
    ...
    def create(
        ...
        *,
        ...
        if isinstance(notifications, dict) and notifications.get("enabled", False):
            ...
            notifications["workspace_config_hash"] = NotificationAdminApplication(
                workspace
            ).config_hash(mode=config.mode)
        ...
        if not _safe_agent_resource_id(connection_id):
            ...
            try:
                ...
                if connection is None:
                    issues.append(
                        f"Agent model connection does not exist: {connection_id}"
                    )
                    return issues
