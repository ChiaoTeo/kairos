...
from kairospy.domain.account import AccountAlias, AccountBookRef, AccountDirectory, AccountIdentity


_DEFAULT_ACCOUNT_BOOKS: dict[str, tuple[str, ...]] = {
...
def _account_books(account: Mapping[str, object], values: Mapping[str, object]) -> tuple[AccountBookRecord, ...]:
    ...
    books = _DEFAULT_ACCOUNT_BOOKS.get(key, ("spot",))
    return tuple(AccountBookRecord(book, book) for book in books)
