...
from ..domain.datasets import MarketPartition, parse_market_dataset_id
...
class MarketDataOperationsService:
    ...
    def spec_from_subscription(self, subscription: DataSubscription) -> MarketDataSpec:
        dataset_id = subscription.spec.dataset_id
        ...
        return MarketDataSpec(
            symbol=str(subscription.spec.market.source_symbol),
            kind=kind,
...
def _kind_from_subscription(subscription: DataSubscription) -> str:
...
def _timeframe_from_subscription(subscription: DataSubscription) -> str | None:
    intervals = {
        selector.interval
