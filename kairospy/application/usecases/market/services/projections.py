...
from ..domain.subscriptions import DataSubscription, subscription_summary
...
class MarketProjectionService:
    ...
    def __init__(self, subscriptions: MarketSubscriptionProvider | None = None, *, window_size: int = 100) -> None:
        if window_size < 1:
    ...
    def subscriptions_view(self) -> MarketSubscriptionsView:
        ...
        if self.subscriptions is not None:
            subscriptions = tuple(subscription_summary(item) for item in self.subscriptions.subscriptions())
    ...
    def window_views(self) -> tuple[tuple[str, str, MarketWindow[object]], ...]:
        ...
        views.extend((MarketViewKeys.quotes(key), "quotes", _quote_window(key, self._quote_windows[key], self._quote_counts[key])) for key in sorted(self._quote_windows))
        views.extend((MarketViewKeys.rates(_rate_market_key(self._rate_windows[key][-1]), self._rate_windows[key][-1].basis or "default"), "rates", _rate_window(key, self._rate_windows[key], self._rate_counts[key])) for key in sorted(self._rate_windows))
        views.extend((MarketViewKeys.orderbooks(key), "orderbooks", _orderbook_window(key, self._book_windows[key], self._book_counts[key])) for key in sorted(self._book_windows))
        views.extend((MarketViewKeys.bars(_market_state_key(self._bar_windows[key][-1]), self._bar_windows[key][-1].timeframe), "bars", _bar_window(key, self._bar_windows[key], self._bar_counts[key])) for key in sorted(self._bar_windows))
        views.extend((MarketViewKeys.trades(key), "trades", _trade_window(key, self._trade_windows[key], self._trade_counts[key])) for key in sorted(self._trade_windows))
        views.extend((MarketViewKeys.option_greeks(key), "option_greeks", _option_greeks_window(key, self._option_greeks_windows[key], self._option_greeks_counts[key])) for key in sorted(self._option_greeks_windows))
        return tuple(views)


def quote_from_orderbook_top(book: OrderBookSnapshot) -> Quote | None:
    bid = book.bid1
    ask = book.ask1
...
def _append_window(items: tuple[TWindowItem, ...], item: TWindowItem, *, limit: int, replace_key: WindowReplaceKey[TWindowItem]) -> tuple[TWindowItem, ...]:
...
def _quote_window(key: str, items: tuple[Quote, ...], event_count: int) -> QuoteWindow:
    latest = items[-1]
    return QuoteWindow(subject_type="market", subject_id=key, market_id=latest.market_id, market_key=latest.market_key, instrument_id=latest.instrument_id, source=latest.source, updated_at=latest.time, items=items, event_count=event_count)


def _rate_window(key: str, items: tuple[RateObservation, ...], event_count: int) -> RateWindow:
    latest = items[-1]
    return RateWindow(subject_type="market" if latest.market_id is not None else "rate", subject_id=key, market_id=latest.market_id, market_key=_rate_market_key(latest), instrument_id=latest.instrument_id, source=latest.source, updated_at=latest.time, items=items, event_count=event_count, basis=latest.basis)


def _orderbook_window(key: str, items: tuple[OrderBookSnapshot, ...], event_count: int) -> OrderBookWindow:
    latest = items[-1]
    return OrderBookWindow(subject_type="market", subject_id=key, market_id=latest.market_id, market_key=latest.market_key, instrument_id=latest.instrument_id, source=latest.source, updated_at=latest.time, items=items, event_count=event_count)


def _bar_window(key: str, items: tuple[Bar, ...], event_count: int) -> BarWindow:
    latest = items[-1]
    return BarWindow(subject_type="market", subject_id=key, market_id=latest.market_id, market_key=latest.market_key, instrument_id=latest.instrument_id, source=latest.source, updated_at=latest.time, items=items, event_count=event_count, timeframe=latest.timeframe)


def _trade_window(key: str, items: tuple[TradePrint, ...], event_count: int) -> TradeWindow:
    latest = items[-1]
    return TradeWindow(subject_type="market", subject_id=key, market_id=latest.market_id, market_key=latest.market_key, instrument_id=latest.instrument_id, source=latest.source, updated_at=latest.time, items=items, event_count=event_count)


def _option_greeks_window(key: str, items: tuple[OptionGreeks, ...], event_count: int) -> OptionGreeksWindow:
    latest = items[-1]
    return OptionGreeksWindow(subject_type="market", subject_id=key, market_id=latest.market_id, market_key=latest.market_key, instrument_id=latest.instrument_id, source=latest.source, updated_at=latest.time, items=items, event_count=event_count)


def _window_summary(key: str, kind: str, window: MarketWindow[object]) -> MarketWindowSummary:
    return MarketWindowSummary(key=key, kind=kind, subject_type=window.subject_type, subject_id=window.subject_id, market_id=window.market_id, market_key=window.market_key, instrument_id=window.instrument_id, qualifier=_window_qualifier(window), item_count=window.size, event_count=window.event_count, updated_at=window.updated_at)
