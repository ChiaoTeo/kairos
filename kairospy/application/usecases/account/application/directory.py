"""Account directory capability used by account/system composition."""
...
class AccountBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.books:
            raise ValueError("account must expose at least one book")
        identities = {(str(book.identity.broker), str(book.identity.account_id), book.environment.value) for book in self.books}
        if len(identities) != 1:
            raise ValueError("account books must belong to one account identity and environment")
...
class AccountDirectory:
    ...
    @classmethod
    def from_contexts(cls, contexts: tuple[AccountContext, ...]) -> "AccountDirectory":
        ...
        for index, (_key, books) in enumerate(sorted(groups.items())):
            ...
            bindings.append(AccountBinding(alias, index, tuple(books)))
    ...
    def resolve_context(self, *, account_id: object | None = None, account_index: int | None = None, book: object | None = None, default: AccountContext | None = None) -> AccountContext:
        if account_index is not None:
            ...
            raise ValueError("multiple accounts are available; pass an account key")
        return binding.require_book(book) if book is not None else binding.default_book(None if default is None else default.book)


def _book_match_keys(book: AccountBookRef) -> set[str]:
    return {str(book.book), book.qualifier, ".".join(_key_part(part) for part in book.book_key.split(":")), ".".join(_key_part(part) for part in (book.broker, book.account_id, book.book, book.qualifier) if part)}


def _identity_match_keys(binding: AccountBinding) -> set[str]:
    return {key for context in binding.books for key in (str(context.identity.account_id), str(context.identity.value))}
