...
class RunDaemonControlPlane:
    def __init__(
        ...
        *,
        ...
        base = _runtime_base(self.mode, root=root)
        self.group_directory = base / _path_segment(self.run_id)
        self.history_directory = self.group_directory / "runs"
        self.current_path = self.group_directory / "current.json"
        self.run_instance_id = _path_segment(instance_id) if instance_id else None
        self.directory = (
            self._instance_directory(self.run_instance_id)
            if self.run_instance_id is not None
            else self._current_directory()
        )
        self.state_path = self.directory / "state.json"
        self.context_path = self.directory / "context.json"
        self.events_path = self.directory / "events.jsonl"
        self.command_path = self.directory / "command.json"
        self.lock_path = self.group_directory / "run.lock" if self.run_instance_id is not None else self.directory / "run.lock"
        self.log_path = self.directory / "daemon.log"

    def _instance_directory(self, instance_id: str) -> Path:
        return self.history_directory / _path_segment(instance_id)
    ...
    def _current_directory(self) -> Path:
        ...
        if isinstance(instance_id, str) and instance_id.strip():
            ...
            if directory.exists():
                self.run_instance_id = _path_segment(instance_id)
                return directory
        return self.group_directory

    def _write_current(self, status: Mapping[str, object] | None = None) -> None:
        ...
        temporary = self.group_directory / f".{self.current_path.name}.{uuid4()}.tmp"
    ...
    def update_context(self, values: Mapping[str, object]) -> dict[str, object]:
        ...
        self._write_current({"phase": state.get("phase", RunDaemonPhase.CREATED.value), "context": context})
    ...
    def start_background(
        ...
        *,
        ...
        if self.run_instance_id is not None:
            args.extend(("--run-instance-id", self.run_instance_id))
...
def list_run_instances(
    ...
    *,
    ...
    item = _runtime_mode(mode)
    base = _runtime_base(item, root=root)
    group = base / _path_segment(_required_text(run_id, "run_id"))
    history = group / "runs"
    if not history.exists():
        control = RunDaemonControlPlane(run_id, mode=item, root=base)
        return (control.status(stale_after_seconds=stale_after_seconds),) if control.state_path.exists() else ()
    statuses = [
        RunDaemonControlPlane(run_id, mode=item, root=base, instance_id=directory.name).status(
            stale_after_seconds=stale_after_seconds
        )
        for directory in sorted(path for path in history.iterdir() if path.is_dir())
    ]
    return tuple(sorted(statuses, key=lambda status: (status.updated_at, status.run_instance_id or "")))
...
def _runtime_base(mode: RuntimeMode, *, root: str | Path | None = None) -> Path:
    ...
    return Path(".kairos") / "runtime" / mode.value
