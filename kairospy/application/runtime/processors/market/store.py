...
class MarketStore:
    schemas = (
        ...
        ViewSchema(
            ...
            "system",
            fields=(
                ViewFieldSchema("event_count", "quote update count", "runtime sequence", "market event view state"),
                ViewFieldSchema("quotes", "latest quotes by market key", "event time", "market event view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market quote view state",
        ),
        ViewSchema(
            "market.rates",
            "system",
            fields=(
                ViewFieldSchema("event_count", "rate update count", "runtime sequence", "market event view state"),
                ViewFieldSchema("rates", "latest rates by rate or market id", "event time", "market event view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market rate view state",
        ),
        ViewSchema(
            "market.books",
            "system",
            fields=(
                ViewFieldSchema("event_count", "order book update count", "runtime sequence", "market event view state"),
                ViewFieldSchema("books", "latest order books by market key", "event time", "market event view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market book view state",
        ),
        ViewSchema(
            "market.bars",
            "system",
            fields=(
                ViewFieldSchema("event_count", "bar update count", "runtime sequence", "market event view state"),
                ViewFieldSchema("bars", "latest bars by market key and timeframe", "event time", "market event view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market bar view state",
        ),
        ViewSchema(
            "market.trades",
            "system",
            fields=(
                ViewFieldSchema("event_count", "trade update count", "runtime sequence", "market event view state"),
                ViewFieldSchema("trades", "latest trades by market key", "event time", "market event view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market trade view state",
        ),
        ViewSchema(
            "market.observations",
            "system",
            fields=(
                ViewFieldSchema("event_count", "market observation count", "runtime sequence", "market event view state"),
                ViewFieldSchema("observations", "latest observations by subject and kind", "event time", "market event view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market observation view state",
        ),
        ViewSchema(
            "market.fields",
            "system",
            fields=(
                ViewFieldSchema("event_count", "market field update count", "runtime sequence", "market field view state"),
                ViewFieldSchema("fields", "latest market facts by subject and field", "event time", "market field view state"),
            ),
            mutability="runtime_writable",
            evidence="runtime market field view state",
        ),
    )

    def __init__(self, data: MarketDataPort | None = None) -> None:
        self.data = data
