...
from kairospy.application.ports import DataSubscription, HistoricalMarketDataPort, MarketDataSubscriptionSpec, MarketDatasetStore
from kairospy.application.runtime.contracts import MarketRuntime, MarketRuntimeEnvelope
from kairospy.application.domain.market import MarketDataOperationsService, MarketDataResolver, MarketDataSpec
from kairospy.application.domain.market.datasets import parse_market_dataset_id
from kairospy.application.domain.market.sources import IterableMarketEventSource, market_event_from_row, parse_event_time
...
class ReplayMarketDataService(MarketDataOperationsService, MarketSubscriptionState, MarketRuntime):
    def __init__(
        self,
        store: MarketDatasetStore,
        *,
        resolver: MarketDataResolver | None = None,
        policy: ReplayMarketDataPolicy | None = None,
        historical_client: HistoricalMarketDataPort | None = None,
        historical_client_factory: Callable[[MarketDataSpec], HistoricalMarketDataPort] | None = None,
    ) -> None:
    ...
    def set_historical_client_factory(
        self,
        factory: Callable[[MarketDataSpec], HistoricalMarketDataPort] | None,
    ) -> None:
    ...
    def _historical_client(self, spec: MarketDataSpec) -> HistoricalMarketDataPort | None:
...
def runtime_envelope_from_row(row: Mapping[str, object], *, sequence: int, stream: str) -> MarketRuntimeEnvelope:
    if "time" not in row:
