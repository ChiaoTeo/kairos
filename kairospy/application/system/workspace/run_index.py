...
class RunIndex:
    ...
    def list(self) -> tuple[RunIndexEntry, ...]:
        return tuple(self._entries.values())

    def get(self, name: str) -> RunIndexEntry:
        try:
            return self._entries[name]
        except KeyError as error:
            raise ConfigError(f"unknown registered run: {name}") from error
    ...
    def register(self, name: str, config_path: str | Path) -> RunIndexEntry:
        if not name.strip():
            raise ConfigError("run name must be a non-empty string")
        path = Path(config_path).expanduser()
        resolved = path if path.is_absolute() else (self.root / path).resolve()
        entry = RunIndexEntry(name=name.strip(), config_path=resolved, registered_at=datetime.now(timezone.utc).isoformat())
        self._entries[entry.name] = entry
        self.save()
        return entry

    def unregister(self, name: str) -> RunIndexEntry:
        entry = self.get(name)
    ...
    def save(self) -> None:
        ...
        payload = {
            "schema_version": 1,
            "runs": {entry.name: entry.to_dict(root=self.root) for entry in self.list()},
        }
...
def _read_entries(path: Path, *, root: Path) -> dict[str, RunIndexEntry]:
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        return {}
    except json.JSONDecodeError as error:
        raise ConfigError(f"invalid run index JSON {path}: {error}") from error
    if not isinstance(payload, Mapping):
        raise ConfigError(f"run index must be a JSON object: {path}")
    runs = payload.get("runs", {})
    if not isinstance(runs, Mapping):
        raise ConfigError(f"run index runs must be a JSON object: {path}")
    entries: dict[str, RunIndexEntry] = {}
    for name, raw in runs.items():
        if not isinstance(raw, Mapping):
            raise ConfigError(f"run index entry must be an object: {name}")
        config = raw.get("config")
        if not isinstance(config, str) or not config.strip():
            raise ConfigError(f"run index entry {name} requires config")
        config_path = Path(config).expanduser()
        if not config_path.is_absolute():
            config_path = (root / config_path).resolve()
        entries[str(name)] = RunIndexEntry(
            name=str(name),
