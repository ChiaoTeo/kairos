...
class AccountRegistry:
    def __init__(self, accounts: Iterable[SystemAccount]) -> None:
    ...
    @property
    def accounts(self) -> tuple[SystemAccount, ...]:
        return tuple(self._accounts.values())

    def require(self, account_id: str) -> SystemAccount:
        try:
            return self._accounts[account_id]
        except KeyError as error:
            raise ValueError(f"unknown account: {account_id}") from error

    def require_index(self, account_index: int) -> SystemAccount:
        try:
...
__all__ = ["AccountRegistry", "SystemAccount"]
