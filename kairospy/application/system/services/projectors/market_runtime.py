...
from kairospy.application.usecases.market.application.runtime import RuntimeMarketService
from kairospy.application.usecases.market.application.ingestion import MarketIngestionService
from kairospy.application.usecases.market.application.projections import MarketProjectionService
...
class MarketProjector:
    def __init__(self, service: RuntimeMarketService) -> None:
        self.projection = MarketProjectionService(subscriptions=service)
        self.ingestion = MarketIngestionService()

    def on_event(self, event: RuntimeEnvelope) -> None:
        market_event = self.ingestion.event_from_envelope(event)
        if market_event is not None:
            self.projection.apply(market_event)
