...
from kairospy.application.system.trading import TradingSystemLauncher
...
def _target(mode: RuntimeMode, config_path: Path) -> tuple[object, Callable[[], object]]:
    if mode is RuntimeMode.BACKTEST:
        try:
            ...
            raise
        return configured, lambda: _TRADING_LAUNCHER.run_configured_backtest(configured)
    if mode is RuntimeMode.PAPER:
        try:
            configured = configured_paper(config_path)
        except PaperConfigurationError:
            raise
        return configured, lambda: _TRADING_LAUNCHER.run_configured_paper(configured)
    if mode is RuntimeMode.LIVE:
        try:
            configured = configured_live(config_path)
        except LiveConfigurationError:
            raise
        return configured, lambda: _TRADING_LAUNCHER.run_configured_live(configured)
