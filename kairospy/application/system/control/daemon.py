...
from kairospy.application.system.facade.trading import TradingSystemLauncher
from kairospy.config import ConfigError, load_run_config


_TRADING_LAUNCHER = TradingSystemLauncher()
_RUN_INSTANCE_ID_ENV = "KAIROS_RUN_INSTANCE_ID"
...
class _RunDaemonTarget:
    configured: object
    runner: Callable[[], object]

    @property
    def run_id(self) -> str:
        return str(getattr(self.configured, "run_id"))

    @property
    def run_directory(self) -> str:
        return str(getattr(self.configured, "run_directory", ""))


@dataclass(frozen=True, slots=True)
class _RunTargetDescriptor:
    run_id: str
    run_directory: str
...
class RunDaemonService:
    def __init__(self, root: str | Path = ".kairos/runs", *, target_resolver: "_RunTargetResolver | None" = None) -> None:
        self.root = Path(root).expanduser()
        self._targets = target_resolver or _RunTargetResolver(_TRADING_LAUNCHER)
        self._store = _RunDaemonStore()
    ...
    def run_foreground(
        ...
        *,
        ...
        with _RunGroupLock(group_directory):
        ...
        try:
            ...
            heartbeat = _RunHeartbeat(directory)
            heartbeat.start()
            try:
    ...
    def start_background(
        ...
        *,
        ...
        with log_path.open("ab") as output:
            env = os.environ.copy()
            env[_RUN_INSTANCE_ID_ENV] = str(identity["process_id"])
...
class _RunTargetResolver:
    def __init__(self, launcher: TradingSystemLauncher) -> None:
        self.launcher = launcher

    def describe(self, mode: RuntimeMode, config_path: Path) -> _RunTargetDescriptor:
        try:
            run_config = load_run_config(config_path)
            run_config.require_mode(mode.value)
        except ConfigError as error:
            raise ValueError(str(error)) from error
        mode_config = run_config.values.get(mode.value)
        if not isinstance(mode_config, Mapping):
            mode_config = {}
        return _RunTargetDescriptor(
...
class _RunDaemonStore:
    def claim_start(self, group_directory: Path, directory: Path, identity: Mapping[str, object]) -> None:
        ...
        if active is not None and active.get("run_instance_id") != identity["process_id"]:
            raise RunAlreadyActiveError(
                f"run already has an active instance: {identity['mode']}:{identity['run_id']} "
                f"({active.get('run_instance_id')})"
            )
...
def _target_with_instance_directory(target: _RunDaemonTarget, directory: Path, launcher: TradingSystemLauncher) -> _RunDaemonTarget:
    ...
    if isinstance(configured, ConfiguredBacktest):
        instance_configured = replace(configured, run_directory=directory)
        return _RunDaemonTarget(instance_configured, lambda: launcher.run_configured_backtest(instance_configured))
    if isinstance(configured, ConfiguredPaper):
        instance_configured = replace(configured, run_directory=directory)
        return _RunDaemonTarget(instance_configured, lambda: launcher.run_configured_paper(instance_configured))
    if isinstance(configured, ConfiguredLive):
        instance_configured = replace(configured, run_directory=directory, state_path=directory / "live_state.json")
        return _RunDaemonTarget(instance_configured, lambda: launcher.run_configured_live(instance_configured))
    return target
...
def _configured_run_directory(mode: RuntimeMode, mode_config: Mapping[str, object], *, root: Path, run_id: str) -> Path:
    runs_root_value = mode_config.get("runs_root")
    runs_root = Path(".kairos/runs").resolve() if runs_root_value is None else _resolve_workspace_path(runs_root_value, root=root)
    return runs_root / mode.value / run_id
...
def _identity(run_id: str, mode: RuntimeMode, *, process_id: str | None = None) -> dict[str, object]:
    return {
        "run_id": run_id,
        "mode": mode.value,
...
class _RunGroupLock:
    def __init__(self, group_directory: Path) -> None:
        self.path = group_directory / "run.lock"
        self._file = None

    def __enter__(self) -> "_RunGroupLock":
...
class _RunHeartbeat:
    def __init__(self, directory: Path, *, interval_seconds: float | None = None) -> None:
        ...
        self._thread = threading.Thread(target=self._run, name=f"kairos-run-heartbeat-{directory.name}", daemon=True)
...
__all__ = ["RunAlreadyActiveError", "RunDaemonResult", "RunDaemonService"]
