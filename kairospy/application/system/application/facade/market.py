...
from kairospy.application.usecases.market.application.replay import replay_rows, specs_from_subscription
from kairospy.application.support.launch.application.configuration import BacktestConfigurationError, configured_backtest
from kairospy.application.support.system.application.facade.resources import DriverName, ExchangeName, StorageFormat, data_store, public_market_access
...
from kairospy.domain.market import Bar, MarketSelector
from kairospy.domain.reference import ExchangeId, MarketRef, MarketResolver, MarketTypeId
from kairospy.infrastructure.persistence.application.market_data import DataStore

MarketDataMode = Literal["historical", "live"]


class MarketReplayWriter(Protocol):
    def __call__(self, rows: list[dict[str, object]]) -> object:
        ...
...
class MarketDataFacade:
    ...
    def download(
        ...
        *,
        ...
        client = public_market_access(exchange_name, driver_name)
        spec = MarketDataSpec(
            symbol=symbol,
            kind=kind,
            venue=exchange_name.value,
    ...
    def prefetch_backtest(
        ...
        *,
        ...
        data = _backtest_market_data_service(configured)
        collector = _SubscriptionCollector()
        context = _SubscriptionContext(configured.strategy.strategy_id, collector)
        ...
        for subscription in subscriptions:
            for spec in _historical_specs(
                ...
                path = None if dry_run else data.download(spec, public_market_access(exchange_name, driver_name), mode=mode)
                downloads.append(
                    {
    ...
    def stream_events(
        ...
        *,
        ...
        client = public_market_access(request.exchange_name, driver_name)
    ...
    def doctor(self, *, exchange_name: ExchangeName, driver_name: DriverName) -> dict[str, object]:
        public_market_access(exchange_name, driver_name)
...
class _SubscriptionCollector(MarketSubscriptionService):
...
class _SubscriptionContext:
    def __init__(self, strategy_id: str, collector: _SubscriptionCollector) -> None:
    ...
    def intent(self, intent: Intent) -> None:
        raise RuntimeError("market prefetch does not execute strategy intents")
    ...
    def subscribe(
        ...
        *,
        ...
        if selectors is None and isinstance(subject, str) and subject.startswith("market."):
            ...
            return self._collector.subscribe(
        ...
        return self._collector.subscribe(MarketDataSubscriptionSpec(market_ref, selectors, identity=identity))

    def unsubscribe(self, subscription: DataSubscription | str) -> None:
        self._collector.unsubscribe(subscription)
...
def _backtest_market_data_service(configured: object) -> BacktestMarketDataService:
