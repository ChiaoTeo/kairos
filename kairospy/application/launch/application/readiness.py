"""Launch readiness projection for Workspace resources."""
...
from kairospy.application.config.references import ConfigurationReferenceApplication
...
def project_resource_readiness(
    ...
    *,
    ...
    issues = tuple(str(item) for item in value.get("issues") or ())
    ...
    return ResourceReadiness(
        ...
        issues=issues,
        tested=tuple(str(item) for item in value.get("tested") or ()),
        not_tested=tuple(str(item) for item in value.get("not_tested") or ()),
