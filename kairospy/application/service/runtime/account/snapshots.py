...
class ApplyAccountSnapshotUseCase:
    def __init__(self, store: AccountSnapshotStore | None) -> None:
        self.store = store

    @classmethod
    def from_store(cls, store: object | None) -> "ApplyAccountSnapshotUseCase | None":
        if store is None:
            return None
...
__all__ = ["AccountSnapshotStore", "ApplyAccountSnapshotUseCase"]
