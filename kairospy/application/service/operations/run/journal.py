...
class RunAccountJournal:
    ...
    def record_backtest_result(self, result: object, *, run_id: str | None = None, mode: str | None = None) -> None:
        ...
        summary = {
            "run_id": run_id,
            "mode": mode or getattr(getattr(result, "account", None), "environment", ""),
            "account": getattr(getattr(result, "account", None), "value", None),
    ...
    def record_equity_point(
        ...
        *,
        ...
        initial_equity: object | None = None,
        ...
        net_profit = None if equity is None or initial_equity is None else equity - initial_equity
        total_return = None if net_profit is None or initial_equity in (None, Decimal("0")) else net_profit / initial_equity
        row = {
            "run_id": run_id,
            "mode": mode,
            "time": getattr(point, "time", None),
            "equity": equity,
            "cash": getattr(point, "cash", None),
            "net_profit": net_profit,
            "total_return": total_return,
            "positions": getattr(point, "positions", ()),
        }
        self.append_jsonl(self.equity_path, row)
        self.replace_jsonl(self.positions_path, _point_position_rows(point, run_id=run_id, mode=mode))
        current = self.read_current()
        current.update({
            "run_id": run_id,
            "mode": mode,
            "account": getattr(account, "value", None),
    ...
    def record_fill(self, fill: object, *, run_id: str | None = None, mode: str | None = None) -> None:
        run_id = run_id or self.run_id
        mode = mode or self.mode
        self.append_jsonl(self.fills_path, {"run_id": run_id, "mode": mode, **_jsonable(fill)})
...
def _equity_rows(result: object, *, run_id: str | None, mode: str | None) -> list[dict[str, object]]:
    rows = []
    initial = getattr(result, "initial_equity", None)
    for point in tuple(getattr(result, "equity_curve", ())):
        equity = getattr(point, "equity", None)
...
def _point_position_rows(point: object, *, run_id: str | None, mode: str | None) -> list[dict[str, object]]:
    rows = []
    for instrument_id, quantity in tuple(getattr(point, "positions", ())):
        rows.append({
            "run_id": run_id,
            "mode": mode,
            "time": getattr(point, "time", None),
