...
from kairospy.application.service.domains.market import MarketSubscription, STREAM_TICKER
from kairospy.core.reference import MarketResolver
from kairospy.infrastructure.data import DataStore
from kairospy.application.mode.backtest import BacktestEngine, SimulatedAccount
from kairospy.application.mode.paper import PaperAccountConfig, PaperEngine, run_streaming_paper
from kairospy.application.service.domains.market import (
    AsyncIterableEventSource,
    IterableEventSource,
)
from kairospy.application.runtime.control import RunDaemonTarget, RunExecutionContext
...
BrokerFactory = Callable[[str], object]
...
def configured_live_target(
    ...
    *,
    ...
    from kairospy.application.service.modes.live import configured_live_target as configured

    return configured(config_path, exchange_factory=exchange_factory, broker_factory=broker_factory)
...
def _int_value(value: object, source: str) -> int:
    if isinstance(value, bool):
        raise RunConfigurationError(f"{source} must be an integer")
    try:
        parsed = int(value)
    except Exception as error:
        raise RunConfigurationError(f"{source} must be an integer") from error
    if parsed < 0:
        raise RunConfigurationError(f"{source} cannot be negative")
    return parsed
...
def _optional_int(value: object, source: str) -> int | None:
    if value is None:
        return None
    parsed = _int_value(value, source)
    if parsed < 1:
        raise RunConfigurationError(f"{source} must be positive")
    return parsed
