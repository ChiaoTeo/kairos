...
class PaperAccountServiceView:
    account: AccountContext
    cash_currency: str
    initial_cash: Decimal


class PaperAccountService(AccountService):
    key = "account.service"
    schema = ViewSchema(
        key,
        "system",
        fields=(
            ViewFieldSchema("account", "paper account context", "runtime state", "paper account service"),
            ViewFieldSchema("cash_currency", "account cash currency", "runtime state", "paper account config"),
            ViewFieldSchema("initial_cash", "initial account cash", "run baseline", "paper account config"),
        ),
        mutability="runtime_writable",
        evidence="runtime paper account service",
    )
    ...
    def view(self) -> PaperAccountServiceView:
        return PaperAccountServiceView(self.account.context, self.account.cash_currency, self.account.initial_cash)
...
__all__ = ["PaperAccountService", "PaperAccountServiceView"]
