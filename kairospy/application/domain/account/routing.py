...
def account_book_route(
    ...
    *,
    ...
    broker_key = (broker or provider or str(book.broker)).strip().lower()
    defaults = _broker_book_params(broker_key, str(book.book), qualifier=book.qualifier)
    balance_params = {**defaults["balance"], **dict(base_params or {})}
    order_params = {**defaults["order"], **dict(base_params or {})}
    return AccountBookRoute(book, balance_params=balance_params, order_params=order_params, can_trade=_can_trade(str(book.book), broker=broker_key))
...
def _broker_book_params(broker: str, kind: str, *, qualifier: str) -> dict[str, Mapping[str, object]]:
    normalized = _normalize_kind(kind)
    if broker in {"binance", "okx", "okex"}:
        return _ccxt_params(normalized, qualifier=qualifier, broker=broker)
