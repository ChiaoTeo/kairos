...
from kairospy.application.usecases.account.application.runtime import InitialAssetBalance, SimulatedAccount, AccountSegmentRoute, account_segment_route
...
class BacktestAccountResources:
    ...
    def from_configured(cls, configured: ConfiguredBacktest) -> "BacktestAccountResources":
        ...
        account = SimulatedAccount(
            ...
            initial_balances=tuple(InitialAssetBalance(asset, quantity) for asset, quantity in account_config.initial_balances),
...
class PaperAccountResources:
    ...
    def from_configured(cls, configured: ConfiguredPaper) -> "PaperAccountResources":
        ...
        account = SimulatedAccount(
            ...
            product_family=_product_family(_primary_scope(configured.launch_accounts, default=str(paper.get("market", "spot")))),
            fee_rate=account_config.fee_rate,
        ...
        directory = _launch_account_directory(
            ...
            environment=Environment.PAPER,
            default_segment=account.product_family.value if account.product_family is not None else "spot",
        )
...
class LiveAccountResources:
    ...
    def from_configured(
        ...
        *,
        ...
        broker_client = broker_factory(account.segment, read_ref)
        trade_broker = broker_client if trade_ref == read_ref else broker_factory(account.segment, trade_ref)
        directory = _launch_account_directory(
            configured.launch_accounts,
            account_configs=configured.launch_account_configs,
            fallback=account_config,
        ...
        broker = read_brokers[account.segment]
        ...
        account_service = LiveAccountService(
            ...
            parser=parser,
            stream=private_streams.get(account.segment) if configured.private_sync.enabled else None,
...
def _from_configured_integration(
        ...
        for context in binding.segments:
            ...
            rest_connection = integration.connect(
                IntegrationConnectionSpec(
                    connection_id=f"live.{broker_name}.{product_label}.{segment.value}.private-rest",
                    route=route,
                    product=product,
            ...
            if product is ProductFamily.SPOT:
                stream_connection = integration.connect(
                    IntegrationConnectionSpec(
                        connection_id=f"live.{broker_name}.{product_label}.{segment.value}.private-stream",
                        route=route,
                        product=product,
                        access=AccessScope.PRIVATE,
            ...
            read_connections[segment] = rest_connection
            if product is None:
                earn_applications[segment] = EarnApplication(rest_connection)  # type: ignore[arg-type]
            if stream_connection is not None:
                private_connections[segment] = stream_connection
            if product is not None:
                execution_connection = integration.connect(
                    IntegrationConnectionSpec(
                        connection_id=f"live.{broker_name}.{product_label}.{segment.value}.execution-rest",
                        route=route,
                        product=product,
                        access=AccessScope.PRIVATE,
                        transport=TransportKind.REST,
    ...
    routes = _routes(directory, configured.launch_account_configs, fallback=account_config)
    account_readers = {
        segment: _AccountReader(access)
        for segment, access in read_connections.items()
    }
    ...
    return resource_type(account_service, execution, coordinator, resources, earn_applications.get(account.segment))
...
def _primary_scope(accounts: Mapping[str, LaunchAccountConfig], *, default: str) -> str:
    ...
    return first.segments[0] if first.segments else default
...
def _launch_account_directory(
    ...
    *,
    ...
    if not accounts:
        return AccountDirectory.from_contexts((AccountRuntimeContext(account_segment_from_name(ExternalAccountIdentity(fallback_broker, fallback.account_id), default_segment), environment),))
...
def _capabilities(
    ...
    *,
    ...
    return tuple(
        _capability(context.segment, trade=binding.trade and _account_can_trade_with_credential(binding, account_configs, fallback))
        for binding in directory.bindings
        for context in binding.segments
    )


def _capability(segment: AccountSegment, *, trade: bool = True) -> AccountCapability:
    return AccountProvisioningService().capability(segment, trade_enabled=trade)
...
def _product_family(segment_name: str) -> ProductFamily:
    return account_segment_from_name(ExternalAccountIdentity("config", "config"), segment_name).product_family or ProductFamily.SPOT
...
def configured_account_segment_route(
    segment: AccountSegment,
    *,
    broker: object | None = None,
    base_params: Mapping[str, object] | None = None,
) -> AccountSegmentRoute:
    broker_name = broker or segment.broker
    return account_segment_route(segment, broker=str(broker_name), base_params=base_params)
...
def _routes(
    ...
    *,
    fallback: ConfiguredAccount,
) -> tuple[AccountSegmentRoute, ...]:
    routes = []
    for binding in directory.bindings:
        can_trade = binding.trade and _account_can_trade_with_credential(binding, account_configs, fallback)
        for context in binding.segments:
            route = configured_account_segment_route(context.segment, broker=context.identity.broker)
            routes.append(
                AccountSegmentRoute(
                    route.segment,
...
def _fees(
    ...
    *,
    ...
    for binding in directory.bindings:
        ...
        for context in binding.segments:
            schedules.append(AccountProvisioningService().fee_schedule(context.segment, fee_rate=account_config.fee_rate))
...
def _live_brokers(
    ...
    *,
    ...
    primary: object,
    primary_book: AccountSegment,
    role: str,
    existing: Mapping[AccountSegment, object] | None = None,
) -> Mapping[AccountSegment, object]:
    factory = broker_factory  # type: ignore[assignment]
    brokers: dict[AccountSegment, object] = {}
    for binding in directory.bindings:
    ...
    if not brokers:
        brokers[AccountSegment(fallback.venue, fallback.account_id)] = primary
    return brokers
...
def _account_readers(
    clients: Mapping[AccountSegment, object],
    application,
) -> Mapping[AccountSegment, AccountConnection]:
    del application
    return {segment: _AccountReader(connection) for segment, connection in clients.items()}


def _account_private_streams(clients: Mapping[AccountSegment, object], application):
    del application
    return {segment: _AccountStreamAdapter(connection) for segment, connection in clients.items()}


@dataclass(frozen=True, slots=True)
class _MappedLiveAccountGatewayResolver:
    account_readers: Mapping[AccountSegment, object]
    private_streams: Mapping[AccountSegment, object]

    def resolve_account_reader(self, account: AccountSegment) -> object | None:
    ...
    def read_market_profile(self, request):
        reader = self.account_readers.get(request.context.segment)
        if reader is None:
            raise RuntimeError(f"no account reader configured for account: {request.context.segment.value}")
        return reader.read_market_profile(request)


@dataclass(frozen=True, slots=True)
class _MappedAccountLoginPort:
    readers: Mapping[AccountSegment, object]
    streams: Mapping[AccountSegment, object]

    def login(self, request: AccountLoginRequest) -> AccountLoginResult:
        reader = self.readers.get(request.context.segment)
        if reader is None:
            raise RuntimeError(f"no account reader configured for account: {request.context.segment.value}")
        ...
        stream = self.streams.get(request.context.segment)
        ...
        return AccountLoginResult(
            AccountSession(
                session_id=f"account.{request.context.segment.value}",
                account=request.context.segment,
                connection_ids=ids,
...
class _AccountStreamAdapter:
    ...
    def account_snapshots(self, context: AccountRuntimeContext, *, open_orders: tuple[object, ...] = ()):
...
class _MappedExecutionUpdateSource:
    ...
    def events(self, account: AccountRuntimeContext, *, symbol: str | None = None):
        ...
        if connection is None:
            raise RuntimeError(f"no execution connection configured for account: {account.segment.value}")
...
def _client_resolver(clients: Mapping[AccountSegment, object]):
    def resolve(account: AccountSegment) -> object | None:
