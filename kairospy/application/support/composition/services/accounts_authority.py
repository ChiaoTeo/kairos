...
from kairospy.application.support.system.application.workspace import AccountLease, AccountLeaseError, AccountLeaseManager
from kairospy.application.usecases.account.application.authorization import AccountAuthorizationService, AccountTradeAuthorizationRequest
from kairospy.domain.account import AccountBookRef, AccountCapability, AccountContext, AccountSnapshot, AccountState
...
class _AuthorizableAccountRuntime(Protocol):
    ...
    def fees(self, account: AccountBookRef | None = None) -> object:
        ...


class _AuthorizableExecutionRuntime(Protocol):
    def events(self) -> AsyncIterator[RuntimeEnvelope]:
        ...
...
class AccountTradeAuthority:
    def __init__(
        ...
        *,
        ...
        self._authorization = AccountAuthorizationService()
    ...
    def can_trade(self, account: AccountBookRef) -> bool:
        ...
        return self._authorization.authorize_trade(
            AccountTradeAuthorizationRequest(
                context or account,
                lock=record,
                lock_owned=record is not None and record.launch_instance_id == self.launch_instance_id,
                require_trade_lock=True,
            )
        ).allowed
...
class AuthorizingAccountRuntime:
    def __init__(self, port: _AuthorizableAccountRuntime, authority: AccountTradeAuthority) -> None:
        self.port = port
