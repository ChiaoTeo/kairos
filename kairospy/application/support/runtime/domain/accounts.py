...
class RuntimeAccountBinding:
    ...
    def __post_init__(self) -> None:
        if not self.alias.strip():
            raise ValueError("runtime account alias is required")
        if self.index < 0:
            raise ValueError("runtime account index cannot be negative")
        if not self.books:
            raise ValueError("runtime account must expose at least one book")
        identities = {(str(book.identity.broker), str(book.identity.account_id), book.environment.value) for book in self.books}
        if len(identities) != 1:
            raise ValueError("runtime account books must belong to one account identity and environment")
...
@dataclass(frozen=True, slots=True)
class RuntimeAccountDirectory:
    bindings: tuple[RuntimeAccountBinding, ...] = ()

    def __post_init__(self) -> None:
        aliases = [binding.alias for binding in self.bindings]
        indexes = [binding.index for binding in self.bindings]
        if len(aliases) != len(set(aliases)):
            raise ValueError("runtime account aliases must be unique")
        if len(indexes) != len(set(indexes)):
            raise ValueError("runtime account indexes must be unique")

    @classmethod
    def from_contexts(cls, contexts: tuple[AccountContext, ...]) -> "RuntimeAccountDirectory":
        ...
        for index, (_key, books) in enumerate(sorted(groups.items())):
            ...
            bindings.append(RuntimeAccountBinding(alias, index, tuple(books)))
        return cls(tuple(bindings))

    def require(self, key: str | int) -> RuntimeAccountBinding:
        if isinstance(key, bool):
            raise ValueError("runtime account key must be an alias or integer index")
        if isinstance(key, int):
            for binding in self.bindings:
                if binding.index == key:
                    return binding
            raise KeyError(f"unknown runtime account index: {key}")
        text = key.strip()
        if not text:
            raise ValueError("runtime account key cannot be empty")
        exact = tuple(
            binding
            for binding in self.bindings
        ...
        if len(exact) == 1:
            return exact[0]
        if len(exact) > 1:
            raise ValueError(f"multiple runtime accounts match: {text}")
        identity_matches = tuple(binding for binding in self.bindings if text in _identity_match_keys(binding))
        if len(identity_matches) == 1:
            return identity_matches[0]
        if len(identity_matches) > 1:
            raise ValueError(f"multiple runtime accounts match account identity: {text}")
        raise KeyError(f"unknown runtime account: {text}")
        ...
        *,
        ...
        if account_index is not None:
            ...
            if matches:
                ...
                raise ValueError("multiple runtime accounts are available; pass an account key")
        elif len(self.bindings) == 1:
            binding = self.bindings[0]
        else:
            raise ValueError("multiple runtime accounts are available; pass an account key")
...
def _identity_match_keys(binding: RuntimeAccountBinding) -> set[str]:
    keys: set[str] = set()
