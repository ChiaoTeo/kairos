...
from kairospy.application.usecases.market.subscriptions import DataSubscription
from kairospy.application.support.runtime.contracts import AccountCatalog, AccountRuntime, ExecutionRuntime, MarketRuntime, ReferenceRuntime
from kairospy.application.support.runtime.services.account.snapshots import AccountSnapshotStore, ApplyAccountSnapshotUseCase
...
class RuntimeServiceDependencies:
    ...
    account_directory: LaunchAccountDirectory | None = None
...
class RuntimeAccountViewProjectionService:
    ...
    def directory(self) -> LaunchAccountDirectory:
        ...
        if self.catalog is not None:
            return LaunchAccountDirectory.from_contexts(tuple(self.catalog.accounts()))
...
class TradingRuntimeExecutionService:
    port: ExecutionRuntime | None = None
...
class RuntimeApplicationServices:
    ...
    def from_dependencies(cls, dependencies: RuntimeServiceDependencies) -> "RuntimeApplicationServices":
        execution_projection = (
            ...
            else RuntimeExecutionProjectionService(
        ...
        account_views = (
            ...
            if dependencies.account is None
                ...
                dependencies.account_catalog,
                dependencies.account_directory,
            )
        )
        ...
        execution_updates = (
            None
            if dependencies.execution_coordinator is None
            else ApplyExecutionUpdateUseCase(dependencies.execution_coordinator, intents=dependencies.intents)
        )
        trading = (
            None
...
def _account_projection(
    account: AccountRuntime | None,
    catalog: AccountCatalog | None,
