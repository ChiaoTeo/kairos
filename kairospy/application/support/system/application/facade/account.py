...
from kairospy.application.support.system.domain.workspace import AccountRecord, write_credential_file
...
from kairospy.application.support.system.domain.pagination import PageRequest, paginate
...
class AccountFacade:
    ...
    def balance(
        ...
        *,
        ...
        for index, book in enumerate(selected_books, start=1):
            ...
            try:
                client = private_account_access(book_ref, DriverName.ccxt, credential=_read_credential_ref(account))
    ...
    def open_orders(
        ...
        *,
        ...
        account = _account(account_id)
        client = account_query_access(_account_book_ref(account), DriverName.ccxt, credential=_read_credential_ref(account))
    ...
    def snapshot(self, account_id: str, *, symbol: str | None, params: Mapping[str, object] | None) -> dict[str, object]:
        workspace = resolve_workspace()
        account = _account(account_id)
        client = account_bootstrap_access(_account_book_ref(account), DriverName.ccxt, credential=_read_credential_ref(account))
        payload = {
            "account": account.account_id,
    ...
    def _credential_profile_client(self, account: AccountRecord, ref: str):
        return private_account_access(_account_book_ref(account), DriverName.ccxt, credential=ref)
