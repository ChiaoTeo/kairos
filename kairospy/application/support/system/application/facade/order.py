...
from kairospy.application.support.system.application.control.facade import LaunchFacade
from kairospy.application.support.system.application.facade.resources import DriverName, account_query_access, execution_access
from kairospy.application.usecases.execution.application.orders import ExecutionOrderOptions, OrderCancelRequest, OrderSubmissionRequest
from kairospy.application.support.system.domain.workspace import AccountRecord, KairosWorkspace
from kairospy.application.support.system.domain.config import ConfigError
...
class OrderFacade:
    ...
    def place(
        ...
        *,
        ...
        result = self._order_execution_port(account).submit(
            OrderSubmissionRequest(
                ...
                options=ExecutionOrderOptions.from_mapping(params),
            )
        )
        payload = {"dry_run": False, "request": request, "result": asdict(result)}
    ...
    def cancel(
        ...
        *,
        ...
        if not submit:
            ...
            return payload
        _require_live_confirmation(account, confirm_live=confirm_live)
        result = self._order_execution_port(account).cancel(
            OrderCancelRequest(
                account=_account_book_ref(account),
    ...
    def replace(
        ...
        *,
        ...
        if not submit:
            ...
            return payload
        _require_live_confirmation(account, confirm_live=confirm_live)
        port = self._order_execution_port(account)
        cancel_result = port.cancel(
            OrderCancelRequest(
                account=_account_book_ref(account),
                order_venue_id=order_id,
                symbol=symbol,
        ...
        payload = {"dry_run": False, "request": request, "result": {"cancel": asdict(cancel_result), "create": asdict(create_result)}}
    ...
    def _order_query_client(self, account: AccountRecord):
        return account_query_access(_account_book_ref(account), DriverName.ccxt, credential=_read_credential_ref(account))

    def _order_execution_port(self, account: AccountRecord):
        credential = _trade_credential_ref(account)
        if credential is None:
            raise ValueError(f"account {account.account_id} has no trade credential")
        return execution_access(_account_book_ref(account), DriverName.ccxt, credential=credential)
