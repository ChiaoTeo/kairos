...
from kairospy.application.support.runtime.query import AccountQueryService, OrderQueryService
            ...
            rows.append(
                {
                    ...
                    "trade_state": _trade_state(lock, owned_by_system=owned_by_system),
                    "can_trade": bool(tradable_books) and (lock is None or owned_by_system),
        ...
        try:
            ...
            return SystemCommandResult.accepted(
                command,
                {
                    "account": account.account_id,
                    "account_key": account.account_key,
                    "trade_state": _trade_state(existing, owned_by_system=False),
                    "acquired": False,
    ...
    def _trade_release(self, command: SystemCommand) -> SystemCommandResult:
        ...
        return SystemCommandResult.accepted(
            command,
            {
                ...
                "account_key": account.account_key,
                "released": released,
                "trade_state": "available" if released else _trade_state(workspace.account_locks.get(account.account_key), owned_by_system=False),
            },
        )

    def _order_query(self, command: SystemCommand) -> SystemCommandResult:
        service = OrderQueryService(_ArtifactViewSource(self.directory))
        ...
        try:
            status = service.status(order_id)
