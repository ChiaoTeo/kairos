...
class ConfiguredPaper:
    ...
    source_name: str
    market_feed_resolver: MarketFeedResolver | None
...
def configured_paper(
    ...
    *,
    ...
    source: IterableMarketEventSource | None
    source_name: str
    market_feed_resolver_value: MarketFeedResolver | None
    if paper.get("events") is not None:
        ...
        source_name = str(paper.get("source_name") or source_path.stem)
        market_feed_resolver_value = None
    else:
        market_feed_resolver_value = (
        ...
        source = None
        source_name = str(paper.get("source_name") or "paper")
    return ConfiguredPaper(
        ...
        market_feed_resolver=market_feed_resolver_value,
...
def _missing_market_feed(spec: MarketDataSubscriptionSpec) -> MarketStreamConnection:
    ...
    raise PaperConfigurationError(f"no market feed resolver configured for paper subscription: venue={venue} market={market} symbol={symbol}")
