...
class IterableEventSource:
    ...
    def __init__(self, source_id: str, events: Iterable[Message]) -> None:
...
class AsyncEventSource:
    def __init__(self, source_id: str, events: AsyncIterable[Message], *, limit: int | None = None) -> None:
        if not source_id.strip():
            raise ValueError("source_id is required")
...
class ClockEventSource:
    ...
    async def events(self) -> AsyncIterator[Message]:
        for sequence, at in enumerate(self._ticks, start=1):
            yield Message(topic=f"clock.{self.kind}", payload=ClockTick(at, self.source_id, self.kind, self.metadata), published_at=at, producer=self.source_id, producer_sequence=sequence)
...
class CsvEventSource:
    ...
    async def events(self) -> AsyncIterator[Message]:
        ...
        for sequence, row in enumerate(sorted(rows, key=lambda item: item["_event_time"]), start=1):
            ...
            yield Message(topic=f"{self.domain}.{self.kind}", payload=payload, published_at=event_time, producer=self.source_id, producer_sequence=sequence)
