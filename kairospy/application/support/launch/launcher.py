...
from kairospy.application.support.launch.composition.backtest import BacktestComposition
from kairospy.application.support.launch.composition.artifacts import launch_output
from kairospy.application.support.launch.composition.common import reference_runtime
from kairospy.application.support.launch.composition.integrations import configured_market_feed_for_subscription
from kairospy.application.support.launch.composition.live import LiveComposition
from kairospy.application.support.launch.composition.paper import PaperComposition
from kairospy.application.support.launch.composition.accounts import configured_account_book_route
from kairospy.application.support.launch.config.backtest import BacktestConfigurationError, BacktestLaunchResult, ConfiguredBacktest, configured_backtest
from kairospy.application.support.launch.config.live import BrokerFactory, ConfiguredLive, LiveConfigurationError, LiveLaunchResult, MarketFeedFactory as LiveMarketFeedFactory, configured_live
from kairospy.application.support.launch.config.paper import ConfiguredPaper, PaperConfigurationError, PaperLaunchResult, MarketFeedFactory as PaperMarketFeedFactory, configured_paper
from kairospy.application.support.launch.config.common import ConfiguredAccount, ConfiguredCredential
...
from kairospy.application.support.launch.accounts.authority import AccountTradeAuthority, AuthorizingAccountRuntime, AuthorizingTradingExecutionService
from kairospy.application.support.runtime.services.execution.simulated import SimulatedExecutionService
from kairospy.application.support.system.artifacts.logging import LaunchOutputLog, write_launch_log_section
from kairospy.application.support.launch.accounts import LaunchAccountBinding, LaunchAccountDirectory
from kairospy.application.support.launch.host.resources import TradingRuntimeResources, TradingLaunchSpec
from kairospy.application.support.launch.host.runtime_host import TradingSystem, TradingSystemSession
...
from kairospy.application.support.system.session import SystemCommandDispatcher, SystemCommandFileQueue, SystemCommandResult
...
class TradingSystemLauncher:
    ...
    def launch_configured_backtest(self, configured: ConfiguredBacktest) -> BacktestLaunchResult:
        return BacktestComposition(
            launch_runtime=self._launch_runtime,
            write_account_status=self._write_account_status,
    ...
    def launch_configured_paper(self, configured: ConfiguredPaper) -> PaperLaunchResult:
        return PaperComposition(
            launch_runtime=self._launch_runtime,
            with_account_leases=self._with_account_leases,
            write_account_status=self._write_account_status,
            write_artifacts=self._write_artifacts,
        ).launch(configured)
        ...
        def build(feeds):
            def resolve(spec):
                return configured_market_feed_for_subscription(
                    ...
                    error_type=TradingConfigurationError,
                )
...
def _tradable_contexts(directory: LaunchAccountDirectory) -> tuple[AccountContext, ...]:
    return tuple(
        ...
        if configured_account_book_route(context.book, broker=context.book.broker).can_trade
    )
...
def _system_capabilities(directory: LaunchAccountDirectory) -> tuple[AccountCapability, ...]:
    ...
    for binding in directory.bindings:
        for context in binding.books:
            route = configured_account_book_route(context.book, broker=context.book.broker)
...
def _launch_account_can_trade(launch_account: LaunchAccountConfig, account: ConfiguredAccount, configured: object) -> bool:
    ...
    for book in books:
        ...
        if configured_account_book_route(ref, broker=ref.broker).can_trade:
            return True
    return False
