...
from kairospy.application.support.launch.config.backtest import BacktestConfigurationError, ConfiguredBacktest, configured_backtest
from kairospy.application.support.launch.config.live import ConfiguredLive, LiveConfigurationError, configured_live
from kairospy.application.support.launch.config.paper import ConfiguredPaper, PaperConfigurationError, configured_paper
from kairospy.application.support.launch.launcher import TradingSystemLauncher
from kairospy.application.support.system.session import SystemCommand, SystemCommandDispatcher, SystemCommandFileQueue
        ...
        if mode is RuntimeMode.PAPER:
            try:
                configured = configured_paper(
                    config_path,
                    market_feed_resolver_builder=self.launcher._market_feed_resolver_builder("paper"),
                    account_resolver=self.launcher._account_resolver(config_path),
                    strategy_ref=strategy_ref,
                )
        ...
        if mode is RuntimeMode.LIVE:
            try:
                configured = configured_live(
                    config_path,
                    market_feed_resolver_builder=self.launcher._market_feed_resolver_builder("live"),
                    account_resolver=self.launcher._account_resolver(config_path),
                    strategy_ref=strategy_ref,
                )
