...
from kairospy.orders import OrderSide
...
class BacktestEngine:
        ...
        run = runner.run(
            ...
            pre_events=(
                account_baseline_event(
                    ...
                    payload={"mode": self.runtime_mode.value},
                ),
            ),
    ...
    def _handle_intents(self, intents: tuple[object, ...], context: StrategyContext, hook: str) -> tuple[RuntimeEvent, ...]:
        if context.event is not None:
            self._mark(context.event.payload)
    ...
    def _record_equity(self, at: datetime) -> AccountRuntimeEvent:
        ...
        return AccountRuntimeEvent(
            ...
            payload={
                ...
                "equity": str(equity),
                "cash": str(cash),
                "source": AccountSource.SIMULATED.value,
            },
            snapshot=snapshot,
    ...
    def _mark(self, payload: object) -> None:
        if not isinstance(payload, dict):
            return
        instrument_id = self._mark_instrument_id(payload)
        raw_price = payload.get(self.account.price_field)
        if instrument_id is not None and raw_price is not None:
            self._marks[str(instrument_id)] = Decimal(str(raw_price))

    def _mark_instrument_id(self, payload: dict[str, object]) -> str | None:
        if payload.get("instrument_id") is not None:
            return str(payload["instrument_id"])
        ref = payload.get("market_id") or payload.get("market_key")
        if ref is None:
            return None
        try:
            return self.data.markets.resolve(ref).instrument_id
        except KeyError:
            return str(ref)
