...
def merge_datasets(existing: MarketReplayDataset, chunk: MarketReplayDataset) -> MarketReplayDataset:
    ...
    products = tuple(sorted(set((*existing.products, *chunk.products)), key=_stable_primitive_key))
    references = tuple(sorted(set((*existing.references, *chunk.references)), key=_stable_primitive_key))
    settlements = tuple(sorted(set((*existing.settlements, *chunk.settlements)), key=_stable_primitive_key))
...
def _stable_primitive_key(value: object) -> str:
