...
class TestWindowRegistry:
    __test__ = False
    def __init__(self,path: str|Path="data/validation/test_window_registry.jsonl") -> None:self.path=Path(path)
    ...
    def register(self,use: TestWindowUse) -> None:
        ...
        if use.decision_oos:
            conflicts=[old for old in self.uses() if old.consumed and old.experiment_id!=use.experiment_id and _overlap(old,use)]
