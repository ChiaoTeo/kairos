...
from .contracts import ExperimentValidationResult


class ValidationArtifactWriter:
    """Writes the governed, machine-readable artifact set for an experiment version."""

    def __init__(self, data_root: str | Path = DEFAULT_LAKE_ROOT) -> None:
        self.data_root = Path(data_root)

    def write(self, result: ExperimentValidationResult, *, report: str,
              extra_artifacts: dict[str, Any] | None = None,
              extra_audit: dict[str, Any] | None = None) -> Path:
        output = self.data_root / "validation" / result.registration.experiment_id / result.registration.version
        output.mkdir(parents=True, exist_ok=True)
        result_payload = result.to_dict()
        files = {
            "experiment_spec.json": {**result_payload["registration"], "spec_hash": result.spec_hash},
            ...
            "results.json": {
                "experiment_id": result.registration.experiment_id,
