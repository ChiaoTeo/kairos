...
def validate_events(
    ...
    *,
    known_exchange_codes: set[str] | None = None,
    known_condition_codes: set[str] | None = None,
) -> EventQualityReport:
    ...
    if list(events) != ordered:
        issues.append(
            EventQualityIssue("out_of_order", QualitySeverity.ERROR, "events are not sorted by available_time and source_order")
        )
    for event in events:
        key = (
            event.source,
            event.source_namespace,
            event.source_instrument_id,
            event.record_type,
            event.event_time,
            event.source_order,
        )
        ...
        if not calendar.is_trading_day(local_time.date()):
            ...
            if not session.opens_at <= local_time <= session.closes_at:
                issues.append(
                    EventQualityIssue("outside_regular_session", QualitySeverity.WARNING, "event occurs outside the regular US session", event.event_key)
                )
