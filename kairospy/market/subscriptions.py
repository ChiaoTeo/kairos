...
class MarketDataCapabilities:
    ...
    def require_market_data(self, kind: MarketDataKind) -> None:
        if kind not in self.market_data:
            raise ValueError(f"market data client does not support market data: {kind}")

    def require_product(self, product_type: ProductType) -> None:
        if product_type not in self.product_types:
            raise ValueError(f"market data client does not support product: {product_type}")
...
class SubscriptionPlanner:
    def __init__(
        ...
        capabilities: Mapping[VenueId | ProviderId, MarketDataCapabilities],
    ) -> None:
    ...
    def _symbol(self, requirement: MarketDataRequirement, instrument_id: InstrumentId, at: datetime) -> str:
        mappings = [
            item
            for item in self.catalog.mappings()
            if item.provider_id == requirement.provider_id
            and item.active_at(at)
            and item.target_type is MappingTargetType.INSTRUMENT
            and item.target_id == instrument_id.value
        ...
        listings = [
            item
            for item in self.catalog.active_listings(instrument_id, at)
            if item.venue_id.value == requirement.provider_id.value
        ]
        if len(listings) == 1:
