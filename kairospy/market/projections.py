...
if TYPE_CHECKING:
    from kairospy.market.canonical import CanonicalEventEnvelope, OrderBookLevelPayload
...
class CanonicalBarSeriesProjection:
    ...
    def apply(self, event: CanonicalEventEnvelope) -> Bar | None:
        from kairospy.market.canonical import BarPayload, MarketEventKind
...
class CanonicalOrderBookProjection:
    ...
    def apply(self, event: CanonicalEventEnvelope) -> OrderBookState | None:
        ...
        ordered_bids = tuple(
            OrderBookLevelPayload(price, quantity) for price, quantity in sorted(bids.items(), reverse=True)
        )
    ...
    def _from_snapshot(self, event: CanonicalEventEnvelope) -> OrderBookState:
        from kairospy.market.canonical import OrderBookSnapshotPayload
        ...
        return OrderBookState(
            ...
            event.available_time,
            valid,
            None if valid else "crossed_snapshot",
            self._version,
        )
